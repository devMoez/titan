# nix/nixosModules.nix — NixOS module for titan-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: Titan runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $Titan_HOME/.env
# and read by Titan at startup — no container recreation needed for env changes.
#
# Tool resolution: the Titan wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.titan-agent;
    effectivePackage =
    ...
    titan-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-titan-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "Titan-config-attrs";
      description = "Titan YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "Titan-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "Titan-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'Titan_DOC_EOF'\n${value}\nTitan_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "titan-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/Titan";
    ...
    # Titan user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "Titan-container-entrypoint" ''
      set -eu

      Titan_UID="''${Titan_UID:?Titan_UID must be set}"
      Titan_GID="''${Titan_GID:?Titan_GID must be set}"

      # ── Group: ensure a group with GID=$Titan_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$Titan_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="Titan"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$Titan_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$Titan_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$Titan_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$Titan_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="Titan"
        TARGET_HOME="/home/Titan"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$Titan_UID" -g "$Titan_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$Titan_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$Titan_UID:$Titan_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure Titan_HOME is owned by the target user.
      ...
      if [ -n "''${Titan_HOME:-}" ] && [ -d "$Titan_HOME" ]; then
        find "$Titan_HOME" \! -user "$Titan_UID" -exec chown "$Titan_UID:$Titan_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/Titan-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/Titan-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/Titan ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/Titan
        chmod 0440 /etc/sudoers.d/Titan
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$Titan_UID" --regid="$Titan_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $Titan_HOME/.env.
    ...
    # Default: /var/lib/Titan/workspace → /data/workspace.
  ...
  in {
    options.services.titan-agent = with lib; {
      enable = mkEnableOption "Titan Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = titan-agent;
        description = "The titan-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/Titan";
        description = "State directory. Contains .Titan/ subdir (Titan_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Titan config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $Titan_HOME/.env at activation time.
          Titan reads this file on every startup via load_Titan_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $Titan_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $Titan_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `Titan gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the Titan user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the Titan plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Titan discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          titan_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as Titan (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in titan-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the Titan CLI to environment.systemPackages and export
          Titan_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.Titan symlink to the service
            stateDir. These users are automatically added to the Titan group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.titan-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the Titan CLI to system PATH and export Titan_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.Titan/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.Titan_HOME = "${cfg.stateDir}/.Titan";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.titan-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the Titan user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.titan-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed Titan
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure Titan is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.Titan        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.Titan/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.Titan/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.Titan/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.Titan/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.Titan/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."titan-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.Titan
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.Titan ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.Titan ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.Titan -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.Titan/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.Titan/$_subdir"
            chmod 2770 "${cfg.stateDir}/.Titan/$_subdir"
            find "${cfg.stateDir}/.Titan/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.Titan/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.Titan/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.Titan/config.yaml
            chmod 0640 ${cfg.stateDir}/.Titan/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.Titan/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.Titan/.managed
          chmod 0644 ${cfg.stateDir}/.Titan/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.Titan/.container-mode <<'Titan_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    Titan_bin=${containerDataDir}/current-package/bin/Titan
    Titan_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.Titan/.container-mode
            chmod 0644 ${cfg.stateDir}/.Titan/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.Titan/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.Titan";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.Titan" ]; then
                  rm -f "${symlinkPath}"
                  echo "titan-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.Titan -> stateDir/.Titan for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.Titan";
                target = "${cfg.stateDir}/.Titan";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "titan-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.Titan/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.Titan/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.Titan/auth.json
              fi
            ''}
          ''}
          ...
          # Titan reads $Titan_HOME/.env at startup via load_Titan_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.Titan/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'Titan_NIX_ENV_EOF'
    ${envFileContent}
    Titan_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.Titan/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.Titan/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.Titan/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.titan-agent = {
          description = "Titan Agent Gateway";
          ...
          serviceConfig = {
            ...
            # $Titan_HOME/.env by the activation script. load_Titan_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/Titan"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the Titan group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.titan-agent = {
          description = "Titan Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              Titan_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              Titan_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env Titan_UID="$Titan_UID" \
                --env Titan_GID="$Titan_GID" \
                --env Titan_HOME=${containerDataDir}/.Titan \
                --env Titan_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/Titan gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
