...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      titan-agent = self'.packages.default;
      TitanVenv = titan-agent.TitanVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "Titan-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${TitanVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "Titan-cross-eval" { } (
          if failures != [] then
        ...
        package-contents = pkgs.runCommand "Titan-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${titan-agent}/bin/Titan || (echo "FAIL: Titan binary missing"; exit 1)
          test -x ${titan-agent}/bin/titan-agent || (echo "FAIL: titan-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${titan-agent}/bin/Titan version 2>&1 | grep -qi "Titan" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "Titan-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in Titan titan-agent Titan-acp; do
            test -x ${titan-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "Titan-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking Titan --help ==="
          ${titan-agent}/bin/Titan --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${titan-agent}/bin/Titan --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "Titan-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${titan-agent}/share/titan-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${titan-agent}/share/titan-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "Titan_BUNDLED_SKILLS" ${titan-agent}/bin/Titan || \
            (echo "FAIL: Titan_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: Titan_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "Titan-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${titan-agent}/share/titan-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${titan-agent}/share/titan-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "Titan_BUNDLED_PLUGINS" ${titan-agent}/bin/Titan || \
            (echo "FAIL: Titan_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: Titan_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-tui = pkgs.runCommand "Titan-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${titan-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${titan-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "Titan_TUI_DIR" ${titan-agent}/bin/Titan || \
            (echo "FAIL: Titan_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: Titan_TUI_DIR set in wrapper"
        ...
        # Verify Titan_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        Titan-node = pkgs.runCommand "Titan-node-version" { } ''
          set -e
          echo "=== Checking Titan_NODE in wrapper ==="
          grep -q "Titan_NODE" ${titan-agent}/bin/Titan || \
            (echo "FAIL: Titan_NODE not set in wrapper"; exit 1)
          echo "PASS: Titan_NODE present in wrapper"

          Titan_NODE=$(sed -n "s/^export Titan_NODE='\(.*\)'/\1/p" ${titan-agent}/bin/Titan)
          test -x "$Titan_NODE" || (echo "FAIL: Titan_NODE=$Titan_NODE not executable"; exit 1)
          echo "PASS: Titan_NODE executable at $Titan_NODE"

          NODE_MAJOR=$("$Titan_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All Titan_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify Titan_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "Titan-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(Titan_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking Titan_MANAGED guards ==="
          check_blocked "config set" ${titan-agent}/bin/Titan config set model foo
          check_blocked "config edit" ${titan-agent}/bin/Titan config edit
        ...
        extra-python-packages = let
          ...
          TitanWithExtra = titan-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "Titan-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${TitanWithExtra}/bin/Titan || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${TitanWithExtra}/bin/Titan || \
          ...
          if grep -q "PYTHONPATH" ${titan-agent}/bin/Titan; then
        ...
        extra-dependency-groups = let
          TitanWithGroups = titan-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "Titan-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${TitanWithGroups}"
          echo "venv: ${TitanWithGroups.TitanVenv}"
        ...
        in pkgs.runCommand "Titan-config-roundtrip" {
          ...
          merge_and_load() {
            local Titan_home="$1"
            export Titan_HOME="$Titan_home"
            ${configMergeScript} ${nixSettings} "$Titan_home/config.yaml"
            ${TitanVenv}/bin/python3 -c '
import json, sys
