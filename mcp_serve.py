"""
Titan MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Titan-specific extra)

Usage:
    Titan mcp serve
    Titan mcp serve --verbose
...
logger = logging.getLogger("Titan.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using Titan_HOME."""
    try:
        from Titan_constants import get_Titan_home
        return get_Titan_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("Titan_HOME", Path.home() / ".Titan")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from Titan_constants import get_Titan_home
        directory_file = get_Titan_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("Titan_HOME", Path.home() / ".Titan")
...
class EventBridge:
    ...
    This is the Titan equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from Titan_constants import get_Titan_home
            db_file = get_Titan_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("Titan_HOME", Path.home() / ".Titan")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Titan MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "Titan",
        instructions=(
            "Titan Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Titan MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
