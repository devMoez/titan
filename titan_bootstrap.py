"""Windows UTF-8 bootstrap for Titan entry points.
...
should be imported at the very top of every Titan entry point
(``Titan``, ``titan-agent``, ``Titan-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import Titan_bootstrap``
# (or ``from Titan_bootstrap import apply_windows_utf8_bootstrap``) at
