...
React + Ink terminal UI for Titan. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `Titan_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.Titan/.Titan_history` or under `Titan_HOME`.
...
```text
ui-tui/
  packages/Titan-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      Titan-ink.d.ts    type declarations for @Titan/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent TitanCLI subprocess for slash commands
```
