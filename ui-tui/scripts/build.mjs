...
await build({
  ...
  // Skip the prebuilt @Titan/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@Titan/ink': resolve(root, 'packages/Titan-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The Titan_cli launcher
