import type { RunExternalProcess } from '@Titan/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`Titan ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching Titan: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`Titan ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
