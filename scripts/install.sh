...
# Python-driven tool session (e.g. Titan terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/titan-agent.git"
REPO_URL_HTTPS="https://github.com/NousResearch/titan-agent.git"
Titan_HOME="${Titan_HOME:-$HOME/.Titan}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${Titan_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$Titan_INSTALL_DIR"
...
#   code at /usr/local/lib/titan-agent, command at /usr/local/bin/Titan,
#   data still at /root/.Titan (Titan_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/.Titan/titan-agent"
            echo "                   default (root, Linux): /usr/local/lib/titan-agent"
            echo "  --Titan-home PATH  Data directory (default: ~/.Titan, or \$Titan_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Titan installs the code under"
            echo "  /usr/local/lib/titan-agent and links the command into"
            echo "  /usr/local/bin/Titan (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$Titan_HOME"
            echo "  (default /root/.Titan).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$Titan_HOME/titan-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs Titan setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# Decide where the repo checkout + venv live, and where the `Titan` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $Titan_HOME/titan-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $Titan_HOME/titan-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/titan-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $Titan_HOME/titan-agent — then preserve it)
#
# Always no-op when the user set --dir or $Titan_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in Titan_HOME.
    if is_termux; then
        INSTALL_DIR="$Titan_HOME/titan-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$Titan_HOME/titan-agent/.git" ]; then
            INSTALL_DIR="$Titan_HOME/titan-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/titan-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/titan-agent"
        ROOT_FHS_LAYOUT=true
        log_info "Root install on Linux — using FHS layout"
        log_info "  Code:    $INSTALL_DIR"
        log_info "  Command: /usr/local/bin/Titan"
        log_info "  Data:    $Titan_HOME (unchanged)"
        return 0
    fi
...
get_Titan_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/Titan" ]; then
        echo "$link_dir/Titan"
    else
        echo "Titan"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/NousResearch/titan-agent/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/Titan-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/Titan-uv-installer.$$.sh")"
...
check_node() {
    ...
    if [ -x "$Titan_HOME/node/bin/node" ]; then
        export PATH="$Titan_HOME/node/bin:$PATH"
        local found_ver=$("$Titan_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Titan-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.Titan/node/..."
    ...
    # Place into ~/.Titan/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$Titan_HOME/node"
    mkdir -p "$Titan_HOME"
    mv "$extracted_dir" "$Titan_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$Titan_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$Titan_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$Titan_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$Titan_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$Titan_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.Titan/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Titan install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Titan Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="Titan-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Titan behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up Titan command..."

    if [ "$USE_VENV" = true ]; then
        Titan_BIN="$INSTALL_DIR/venv/bin/Titan"
    else
        Titan_BIN="$(which Titan 2>/dev/null || echo "")"
        if [ -z "$Titan_BIN" ]; then
            log_warn "Titan not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$Titan_BIN" ]; then
        log_warn "Titan entry point not found at $Titan_BIN"
    ...
    # Create a user-facing shim for the Titan command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $Titan_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$Titan_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/Titan"
    cat > "$command_link_dir/Titan" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$Titan_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/Titan"
    log_success "Installed Titan launcher → $command_link_display_dir/Titan"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "Titan command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v Titan' \
                >/dev/null 2>&1; then
        ...
        log_info "Titan not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Titan Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Titan Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Titan Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so Titan works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.Titan directory structure (config at top level, code in subdir)
    mkdir -p "$Titan_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.Titan/.env (top level, easy to find)
    if [ ! -f "$Titan_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$Titan_HOME/.env"
            log_success "Created ~/.Titan/.env from template"
        else
            touch "$Titan_HOME/.env"
            log_success "Created ~/.Titan/.env"
        fi
    else
        log_info "~/.Titan/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$Titan_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.Titan/config.yaml (top level, easy to find)
    if [ ! -f "$Titan_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$Titan_HOME/config.yaml"
            log_success "Created ~/.Titan/config.yaml from template"
        fi
    else
        log_info "~/.Titan/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$Titan_HOME/SOUL.md" ]; then
        cat > "$Titan_HOME/SOUL.md" << 'SOUL_EOF'
# Titan Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how Titan communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.Titan/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.Titan/"

    # Seed bundled skills into ~/.Titan/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.Titan/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.Titan/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$Titan_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$Titan_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.Titan/skills/"
        fi
    fi
}
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; Titan will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (Titan --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'Titan setup' after install."
        return 0
    fi
    ...
    # Run Titan setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m Titan_cli.main setup < /dev/tty
    else
        python -m Titan_cli.main setup < /dev/tty
    fi
}
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for Titan to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$Titan_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'Titan whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                Titan_CMD="$(get_Titan_command_path)"
                $Titan_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'Titan whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'Titan gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $Titan_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $Titan_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: Titan gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: Titan gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $Titan_CMD gateway > "$Titan_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.Titan/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: Titan gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: Titan gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $Titan_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $Titan_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $Titan_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}Titan${NC}              Start chatting"
    echo -e "   ${GREEN}Titan setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}Titan config${NC}       View/edit configuration"
    echo -e "   ${GREEN}Titan config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}Titan gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}Titan update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'Titan' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'Titan' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'Titan' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
            ...
            browser)
                ...
                if [ "$HAS_NODE" = true ]; then
                    ...
                    if [ -z "$DETECTED_BROWSER_EXECUTABLE" ]; then
                        ...
                        if [ -n "$npm_bin" ]; then
                            local agent_browser_dir="$Titan_HOME/node_modules"
                            mkdir -p "$agent_browser_dir"
                            "$npm_bin" install --prefix "$Titan_HOME" agent-browser 2>/dev/null || true
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Titan for pip install"
    ...
    Titan_CMD="$(command -v Titan 2>/dev/null || echo "")"
    if [ -n "$Titan_CMD" ]; then
        log_info "Running Titan setup..."
        "$Titan_CMD" setup
    else
        log_warn "Titan command not found on PATH"
        log_info "Try: python -m Titan_cli.main setup"
    fi
}
