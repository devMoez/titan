...
# Bootstrap Open WebUI against Titan Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.Titan/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-Titan.sh
...
#   Titan_API_MODEL_NAME='Titan Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Titan Agent WebUI}"
...
Titan_ENV_FILE="${Titan_ENV_FILE:-$HOME/.Titan/.env}"
Titan_API_PORT="${Titan_API_PORT:-8642}"
Titan_API_HOST="${Titan_API_HOST:-127.0.0.1}"
Titan_API_CONNECT_HOST="${Titan_API_CONNECT_HOST:-127.0.0.1}"
Titan_API_MODEL_NAME="${Titan_API_MODEL_NAME:-Titan Agent}"
Titan_API_BASE_URL="http://${Titan_API_CONNECT_HOST}:${Titan_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-Titan.sh"
LOG_DIR="$HOME/.Titan/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$Titan_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$Titan_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.Titan.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.Titan</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.Titan"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.Titan"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-Titan.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Titan Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-Titan.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.Titan/logs/openwebui.log
StandardError=append:%h/.Titan/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-Titan.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$Titan_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Titan API server is configured...'
  upsert_env API_SERVER_ENABLED true "$Titan_ENV_FILE"
  upsert_env API_SERVER_HOST "$Titan_API_HOST" "$Titan_ENV_FILE"
  upsert_env API_SERVER_PORT "$Titan_API_PORT" "$Titan_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$Titan_API_MODEL_NAME" "$Titan_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$Titan_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Titan gateway so API server settings take effect...'
  Titan gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${Titan_API_CONNECT_HOST}:${Titan_API_PORT}/health" >/dev/null; then
    log 'Titan API server did not answer on the first check. Trying to start gateway in the background...'
    nohup Titan gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${Titan_API_CONNECT_HOST}:${Titan_API_PORT}/health" >/dev/null
  ...
  log "Titan API endpoint: ${Titan_API_BASE_URL}"
