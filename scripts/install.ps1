# ============================================================================
# Titan Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$SkipSetup,
    [string]$Branch = "main",
    [string]$TitanHome = "$env:LOCALAPPDATA\Titan",
    [string]$InstallDir = "$env:LOCALAPPDATA\Titan\titan-agent"
)
...
$RepoUrlSsh = "git@github.com:NousResearch/titan-agent.git"
$RepoUrlHttps = "https://github.com/NousResearch/titan-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "│             ⚕ Titan Agent Installer                    │" -ForegroundColor Magenta
    Write-Host "├─────────────────────────────────────────────────────────┤" -ForegroundColor Magenta
...
function Install-Git {
    ...
    which Titan uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\Titan\git`` — never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Titan needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Titan copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\Titan\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``Titan_GIT_BASH_PATH`` (User scope) so Titan can find it in a fresh
    ...
    # Download PortableGit into $TitanHome\git.  Always works as long as
    # we can reach github.com — no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found — downloading PortableGit to $TitanHome\git\ ..."
    ...
    try {
        ...
        $release = Invoke-RestMethod -Uri $releaseApi -UseBasicParsing -Headers @{ "User-Agent" = "Titan-installer" }

        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected — PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Titan features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Titan needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``Titan_GIT_BASH_PATH`` (User env scope) so Titan can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $TitanHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $TitanHome\git\usr\bin\bash.exe
    $candidates += "$TitanHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$TitanHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("Titan_GIT_BASH_PATH", $candidate, "User")
            $env:Titan_GIT_BASH_PATH = $candidate
            Write-Info "Set Titan_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe — Titan may not find Git Bash."
    Write-Info "If needed, set Titan_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$TitanHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$TitanHome\node;$env:Path"
        Write-Success "Node.js $version found (Titan-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # Fallback: download binary zip to ~/.Titan/node/
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\Titan-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$TitanHome\node") { Remove-Item -Recurse -Force "$TitanHome\node" }
                Move-Item $extractedDir.FullName "$TitanHome\node"
                $env:Path = "$TitanHome\node;$env:Path"

                $version = & "$TitanHome\node\node.exe" --version
                Write-Success "Node.js $version installed to ~/.Titan/node/"
                $script:HasNode = $true
...
function Install-Repository {
    ...
    if (Test-Path $InstallDir) {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                Write-Info "terminals, running Titan processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                $zipUrl = "https://github.com/NousResearch/titan-agent/archive/refs/heads/$Branch.zip"
                $zipPath = "$env:TEMP\titan-agent-$Branch.zip"
                $extractPath = "$env:TEMP\titan-agent-extract"
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `Titan.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install titan-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and Titan.exe broken with
    ...
    # here, not 30 seconds later when the user runs `Titan`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `Titan dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable — `Titan dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `Titan dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up Titan command..."
    
    if ($NoVenv) {
        $TitanBin = "$InstallDir"
    } else {
        $TitanBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so Titan is globally available
    # On Windows, the Titan.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$TitanBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$TitanBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $TitanBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set Titan_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\Titan instead
    # of the Unix default ~/.Titan
    $currentTitanHome = [Environment]::GetEnvironmentVariable("Titan_HOME", "User")
    if (-not $currentTitanHome -or $currentTitanHome -ne $TitanHome) {
        [Environment]::SetEnvironmentVariable("Titan_HOME", $TitanHome, "User")
        Write-Success "Set Titan_HOME=$TitanHome"
    }
    $env:Titan_HOME = $TitanHome
    
    # Update current session
    $env:Path = "$TitanBin;$env:Path"
    
    Write-Success "Titan command ready"
}
...
function Copy-ConfigTemplates {
    ...
    New-Item -ItemType Directory -Force -Path "$TitanHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TitanHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TitanHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TitanHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TitanHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TitanHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TitanHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TitanHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TitanHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$TitanHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.Titan/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.Titan/.env"
        }
    } else {
        Write-Info "~/.Titan/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$TitanHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.Titan/config.yaml from template"
        }
    } else {
        Write-Info "~/.Titan/config.yaml already exists, keeping it"
    }
    ...
    # (the default PS5 behaviour), and Titan's prompt-injection scanner
...
"@
        ...
        Write-Success "Created ~/.Titan/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.Titan/"
    
    # Seed bundled skills into ~/.Titan/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.Titan/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.Titan/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$TitanHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.Titan/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'Titan setup tools' later."
        return
    }
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\Titan-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\Titan-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
                try {
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\Titan-npm-tui-$(Get-Random).log"
...
function Install-PlatformSdks {
    ...
    # ~/.Titan/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m Titan_cli.main setup
    } else {
        python -m Titan_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $TitanCmd = "$InstallDir\venv\Scripts\Titan.exe"
    if (-not (Test-Path $TitanCmd)) {
        $TitanCmd = "Titan"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$TitanHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'Titan whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$TitanHome\logs\gateway.log"
            Start-Process -FilePath $TitanCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$TitanHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: Titan gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: Titan gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$TitanHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$TitanHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$TitanHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$TitanHome\titan-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   Titan              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   Titan setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   Titan config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   Titan config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   Titan gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   Titan update       " -NoNewline -ForegroundColor Green
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/NousResearch/titan-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
