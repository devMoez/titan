"""Persistent slash-command worker — one TitanCLI per TUI session.
...
def _run(cli: TitanCLI, command: str) -> str:
...
def main():
    ...
    os.environ["Titan_SESSION_KEY"] = args.session_key
    os.environ["Titan_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = TitanCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
