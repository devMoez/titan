...
# 5. Symlinks the 'Titan' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Titan Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/Titan-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink Titan into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up Titan command..."

Titan_BIN="$SCRIPT_DIR/venv/bin/Titan"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$Titan_BIN" "$COMMAND_LINK_DIR/Titan"
echo -e "${GREEN}✓${NC} Symlinked Titan → $COMMAND_LINK_DISPLAY_DIR/Titan"
...
# ============================================================================
# Seed bundled skills into ~/.Titan/skills/
# ============================================================================

Titan_SKILLS_DIR="${Titan_HOME:-$HOME/.Titan}/skills"
mkdir -p "$Titan_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.Titan/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$Titan_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  Titan status        # Check configuration"
if is_termux; then
    echo "  Titan gateway       # Run gateway in foreground"
else
    echo "  Titan gateway install # Install gateway service (messaging + cron)"
fi
echo "  Titan cron list     # View scheduled jobs"
echo "  Titan doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m Titan_cli.main setup
fi
