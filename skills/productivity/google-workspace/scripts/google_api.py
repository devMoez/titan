#!/usr/bin/env python3
"""Google Workspace API CLI for Titan Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Titan-facing JSON contract and falls back to the Python client
...
from _Titan_home import get_Titan_home
...
def _gws_binary() -> str | None:
    override = os.getenv("Titan_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Titan Agent")
