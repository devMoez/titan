...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("Titan_HOME", os.path.expanduser("~/.Titan")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("Titan_HOME", Path.home() / ".Titan")) / "skills" / "red-teaming" / "godmode"
...
PREFILL_PATH = Titan_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Titan models — already uncensored, just needs clean prompt
    "Titan": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "Titan" in model_lower or "nous" in model_lower:
        return "Titan"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Titan config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.Titan/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Titan for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Titan for changes to take effect.")
