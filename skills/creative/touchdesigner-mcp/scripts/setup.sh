...
Titan_HOME_DIR="${Titan_HOME:-$HOME/.Titan}"
Titan_CFG="${Titan_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Titan config has twozero_td MCP entry ──
if [[ ! -f "$Titan_CFG" ]]; then
    echo -e " ${FAIL} Titan config not found at ${Titan_CFG}"
    manual_steps+=("Create ${Titan_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$Titan_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Titan config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Titan config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${Titan_CFG} manually"); }
    manual_steps+=("Restart Titan session to pick up config change")
fi
