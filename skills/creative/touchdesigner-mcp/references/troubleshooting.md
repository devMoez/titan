...
## 5. Titan Configuration

### Config location

`$Titan_HOME/config.yaml` (defaults to `~/.Titan/config.yaml` when `Titan_HOME` is unset)
...
Restart the Titan session for changes to take effect. The MCP connection is
