"""
Profile management for multiple isolated Titan instances.

Each profile is a fully independent Titan_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.Titan/profiles/<name>/`` by default.

The "default" profile is ``~/.Titan`` itself — backward compatible,
zero migration needed.

Usage::

    Titan profile create coder          # fresh profile + bundled skills
    Titan profile create coder --clone  # also copy config, .env, SOUL.md, skills
    Titan profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    Titan -p coder chat                 # or via flag
    Titan profile use coder             # set as sticky default
    Titan profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See Titan_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# Marker file written by `Titan profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `Titan
...
# `Titan skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Titan infrastructure directories that only the default profile
         (``~/.Titan``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_Titan_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.Titan) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "titan-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "Titan", "default", "test", "tmp", "root", "sudo",
})

# Titan subcommands that cannot be used as profile names/aliases
_Titan_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the Titan root, NOT to the current Titan_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where Titan_HOME points outside
    ``~/.Titan``, profiles live under ``Titan_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_Titan_home() / "profiles"


def _get_default_Titan_home() -> Path:
    """Return the default (pre-profile) Titan_HOME path.

    In standard deployments this is ``~/.Titan``.
    In Docker/custom deployments where Titan_HOME is outside ``~/.Titan``
    (e.g. ``/opt/data``), returns Titan_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_Titan_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Titan installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its Titan_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_Titan_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, Titan subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _Titan_SUBCOMMANDS:
        return f"'{canon}' conflicts with a Titan subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``Titan profile list`` for an unrelated profile.
    """
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_Titan_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``Titan update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.Titan)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.Titan/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `Titan update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`Titan profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `Titan update`.\n",
                encoding="utf-8",
            )
        except OSError:
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches Titan_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``Titan profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "Titan_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.Titan).\n"
            "To remove everything, use: Titan uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set Titan_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("Titan_HOME")
    try:
        os.environ["Titan_HOME"] = str(profile_dir)
        from Titan_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["Titan_HOME"] = old_home
        elif "Titan_HOME" in os.environ:
            del os.environ["Titan_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.Titan/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: Titan profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from Titan_HOME.

    Returns ``"default"`` if Titan_HOME is not set or points to ``~/.Titan``.
    Returns the profile name if Titan_HOME points into ``~/.Titan/profiles/<name>``.
    Returns ``"custom"`` if Titan_HOME is set to an unrecognized path.
    """
    from Titan_constants import get_Titan_home
    Titan_home = get_Titan_home()
    resolved = Titan_home.resolve()

    default_resolved = _get_default_Titan_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.Titan itself — its parent is ~/ and its
        # directory name is ".Titan", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: Titan profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.Titan itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.Titan). "
            "Specify a different name: Titan profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="Titan_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"Titan.{old_name}"
    new_host = f"Titan.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_Titan_home() / "honcho.json",
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a Titan_HOME path string.

    Called early in the CLI entry point, before any Titan modules
    are imported, to set the Titan_HOME environment variable.
    """
