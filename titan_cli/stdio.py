...
on Japanese locales, etc.).  Titan's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``Titan_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Titan is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``Titan_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("Titan_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Titan-managed Git install directories so
    ...
    # already-running shells don't see the change — which means Titan
    ...
    # install without Titan-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Titan's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Titan's
    ``Titan config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Titan (User env var in Windows Settings, or
    export in a PowerShell profile) and Titan picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Titan-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\Titan\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``Titan`` from right after install) retain their old PATH.

    Any subprocess Titan spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Titan-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "Titan", "git", "cmd"),
        os.path.join(local_appdata, "Titan", "git", "bin"),
        os.path.join(local_appdata, "Titan", "git", "usr", "bin"),
        # Titan venv Scripts directory — host of the Titan.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes Titan, but harmless to include.
        os.path.join(local_appdata, "Titan", "titan-agent", "venv", "Scripts"),
