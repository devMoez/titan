...
Usage:
    Titan                     # Interactive chat (default)
    Titan chat                # Interactive chat
    Titan gateway             # Run gateway in foreground
    Titan gateway start       # Start gateway as service
    Titan gateway stop        # Stop gateway service
    Titan gateway status      # Show gateway status
    Titan gateway install     # Install gateway service
    Titan gateway uninstall   # Uninstall gateway service
    Titan setup               # Interactive setup wizard
    Titan logout              # Clear stored authentication
    Titan status              # Show status of all components
    Titan cron                # Manage cron jobs
    Titan cron list           # List cron jobs
    Titan cron status         # Check if cron scheduler is running
    Titan doctor              # Check configuration and dependencies
    Titan honcho setup                    # Configure Honcho AI memory integration
    Titan honcho status                   # Show Honcho config and connection status
    Titan honcho sessions                 # List directory → session name mappings
    Titan honcho map <name>               # Map current directory to a session name
    Titan honcho peer                     # Show peer names and dialectic settings
    Titan honcho peer --user NAME         # Set user peer name
    Titan honcho peer --ai NAME           # Set AI peer name
    Titan honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    Titan honcho mode                     # Show current memory mode
    Titan honcho mode [hybrid|honcho|local]  # Set memory mode
    Titan honcho tokens                   # Show token budget settings
    Titan honcho tokens --context N       # Set session.context() token cap
    Titan honcho tokens --dialectic N     # Set dialectic result char cap
    Titan honcho identity                 # Show AI peer identity representation
    Titan honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    Titan honcho migrate                  # Step-by-step migration guide: OpenClaw native → Titan + Honcho
    Titan version             Show version
    Titan update              Update to latest version
    Titan uninstall           Uninstall Titan Agent
    Titan acp                 Run as an ACP server for editor integration
    Titan sessions browse     Interactive session picker with search

    Titan claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: Titan_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``Titan_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``Titan update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``Titan_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, Titan crashes on import and the user can't run
# ``Titan update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import Titan_bootstrap  # noqa: F401
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to Titan_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (Titan tools, Titan setup, Titan model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'Titan {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any Titan module import.
#
# Many modules cache Titan_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("Titan_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.Titan/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set Titan_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors Titan_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If Titan_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.Titan/profiles/coder or
    # /opt/data/profiles/coder).  If Titan_HOME points to the Titan root
    # instead (e.g. systemd hardcodes Titan_HOME=/root/.Titan), we must
    # still read active_profile — the user may have switched profiles via
    # `Titan profile use` and the gateway should honour that choice.
    # See issue #22502.
    Titan_home_env = os.environ.get("Titan_HOME", "")
    if profile_name is None and Titan_home_env:
        if Path(Titan_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the Titan root
    if profile_name is None:
        try:
            from Titan_constants import get_default_Titan_root

            active_path = get_default_Titan_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set Titan_HOME
    if profile_name is not None:
        try:
            from Titan_cli.profiles import resolve_profile_env

            Titan_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent Titan from starting
            print(
...
# Load .env from ~/.Titan/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from Titan_cli.config import get_Titan_home
from Titan_cli.env_loader import load_Titan_dotenv

load_Titan_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → Titan_REDACT_SECRETS env
# var BEFORE Titan_logging imports agent.redact (which snapshots the flag at
# module-import time). Without this, config.yaml's toggle is ignored because
# the setup_logging() call below imports agent.redact, which reads the env var
# exactly once. Env var in .env still wins — this is config.yaml fallback only.
try:
    if "Titan_REDACT_SECRETS" not in os.environ:
        import yaml as _yaml_early

        _cfg_path = get_Titan_home() / "config.yaml"
        if _cfg_path.exists():
            ...
            if isinstance(_early_sec_cfg, dict):
                ...
                if _early_redact is not None:
                    os.environ["Titan_REDACT_SECRETS"] = str(_early_redact).lower()
            del _early_sec_cfg
        del _cfg_path
except Exception:
    pass  # best-effort — redaction stays at default (enabled) on config errors

# Initialize centralized file logging early — all `Titan` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from Titan_logging import setup_logging as _setup_logging
...
try:
    from Titan_cli.config import load_config as _load_config_early
    from Titan_constants import apply_ipv4_preference as _apply_ipv4
...
from Titan_cli import __version__, __release_date__
from Titan_constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from Titan_cli.config import get_env_path, get_Titan_home, load_config
    from Titan_cli.auth import get_auth_status

    # Determine whether Titan itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_Titan_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from Titan_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_Titan_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Titan has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Titan to use their tokens.
    if _has_Titan_config:
        try:
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    db = None
    try:
        from Titan_state import SessionDB
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, Titan_bin
        cli_args: the original CLI arguments (everything after 'Titan')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    Titan_bin = container_info["Titan_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo Titan {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo Titan {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  Titan --tui --resume {target}")
    if title:
        print(f'  Titan --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @Titan/ink is missing or node_modules is behind package-lock.json.
    ...
    ink = root / "node_modules" / "@Titan" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
def _ensure_tui_node() -> None:
    ...
    ``Titan_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("Titan_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "Titan_HOME": Titan_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(Titan_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(Titan_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if Titan_cli_dir is None:
        Titan_cli_dir = Path(__file__).parent
    bundled = Titan_cli_dir / "tui_dist" / "entry.js"
    return bundled if bundled.is_file() else None


def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (Titan_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("Titan_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from Titan_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("Titan_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with Titan_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset Titan_TUI_DIR (e.g. `unset Titan_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if _tui_need_npm_install(tui_dir):
        npm = _node_bin("npm")
        if not os.environ.get("Titan_QUIET"):
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from Titan_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="Titan-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["Titan_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["Titan_PYTHON_SRC_ROOT"] = os.environ.get(
        "Titan_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("Titan_PYTHON", sys.executable)
    env.setdefault("Titan_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["Titan_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["Titan_MODEL"] = model
        env["Titan_INFERENCE_MODEL"] = model
    if provider:
        env["Titan_TUI_PROVIDER"] = provider
        env["Titan_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["Titan_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["Titan_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["Titan_TUI_SKILLS"] = value
    if query:
        env["Titan_TUI_QUERY"] = query
    if image:
        env["Titan_TUI_IMAGE"] = image
    if checkpoints:
        env["Titan_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["Titan_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["Titan_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["Titan_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["Titan_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["Titan_ACCEPT_HOOKS"] = "1"
    ...
    if resume_session_id:
        env["Titan_TUI_RESUME"] = resume_session_id
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``Titan_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``Titan kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``Titan kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("Titan_KANBAN_BOARD"):
        return
    try:
        from Titan_cli.kanban_db import get_current_board

        os.environ["Titan_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("Titan_TUI") == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'Titan sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Titan isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  Titan setup")
        print()

        from Titan_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'Titan setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from Titan_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["Titan_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.Titan/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["Titan_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["Titan_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["Titan_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from Titan_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from Titan_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from Titan_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Titan?")
        print()
    ...
    # has no creds.json.  Every subsequent `Titan gateway` then paid a 30s
    ...
    session_dir = get_Titan_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: Titan gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `Titan gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  Titan gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Titan Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  Titan gateway")
        ...
        print()
        print("  Or install as a service: Titan gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'Titan whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from Titan_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    print("⚕ Titan post-install bootstrap")
    print()
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``Titan model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from Titan_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    ...
    from Titan_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("Titan_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'Titan model' for "
                "available providers, or run 'Titan doctor' to diagnose config "
                "issues."
            )
    ...
    from Titan_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from Titan_cli.config import read_raw_config
    ...
    # a leftover OPENAI_BASE_URL in ~/.Titan/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.Titan/.env if the active provider is not 'custom'.
    ...
    """
    from Titan_cli.config import get_env_value, save_env_value, load_config
...
#
# Titan uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `Titan model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `Titan model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from Titan_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Titan only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``Titan model`` flow, then route aux tasks to them here.
    """
    from Titan_cli.config import load_config
    from Titan_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from Titan_cli.auth import _prompt_model_selection
    from Titan_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from Titan_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from Titan_cli.config import get_env_value

    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.Titan/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `Titan setup` from scratch. OpenRouter
    ...
    from Titan_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.Titan/.env by hand.
    ...
    from Titan_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from Titan_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from Titan_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from Titan_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `Titan auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from Titan_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_xai_oauth(_config, current_model=""):
    ...
    from Titan_cli.models import _PROVIDER_MODELS
    ...
    # live only in the pool (e.g. after ``Titan auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from Titan_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from Titan_cli.auth import _save_model_choice, deactivate_provider
    from Titan_cli.config import get_env_value, load_config, save_config
    ...
    from Titan_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `Titan model` to set a model.")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from Titan_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Titan URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from Titan_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from Titan_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from Titan_cli import azure_detect
    import getpass
    ...
    print("Anthropic-style API endpoints.  Titan will probe your")
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from Titan_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from Titan_cli.config import save_env_value, load_config, save_config
    from Titan_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from Titan_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from Titan_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Titan turns to `copilot --acp`.")
    print("  Titan currently starts its own ACP subprocess for each request.")
    print("  Titan uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set Titan_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``Titan setup`` / ``Titan model``.
    ...
    recover from a malformed paste without editing ``~/.Titan/.env`` by hand.
    ...
    from Titan_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from Titan_cli.config import save_env_value
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `Titan setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so Titan knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Titan typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Titan, enable billing on your "
    ...
    if provider_id == "lmstudio":
        from Titan_cli.auth import AuthError
        from Titan_cli.models import fetch_lmstudio_models
        ...
        from Titan_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from Titan_constants import display_Titan_home as _dhh_fn

            print(
                f"    Titan will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from Titan_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'Titan model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Titan CLI with a provider."""
    from Titan_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from Titan_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from Titan_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from Titan_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from Titan_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from Titan_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``Titan slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: Titan slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `Titan slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from Titan_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from Titan_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from Titan_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from Titan_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from Titan_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from Titan_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from Titan_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Titan home directory to a zip file."""
    if getattr(args, "quick", False):
        from Titan_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from Titan_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Titan backup from a zip file."""
    from Titan_cli.backup import run_import
...
def cmd_version(args):
    """Show version."""
    print(f"Titan Agent v{__version__} ({__release_date__})")
    ...
    try:
        from Titan_cli.banner import check_for_updates
        from Titan_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Titan Agent."""
    _require_tty("uninstall")
    from Titan_cli.uninstall import run_uninstall
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``Titan update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``Titan_cli/web_dist/`` (per vite.config.ts
    outDir: "../Titan_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "Titan_cli" / "web_dist"
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``Titan update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``Titan web``).
    ...
    # build path usable on a stock `py -m Titan_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (Titan web will not be available)")
        )
        _relay(r1)
        if fatal:
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``Titan dashboard`` processes other than ourselves.

    ``Titan dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``Titan update`` replaces files on disk, the running
    ...
    patterns = [
        "Titan dashboard",
        "Titan_cli.main dashboard",
        "Titan_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "Titan.*dashboard"`) keeps us consistent
            # with `Titan_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `Titan update`.
    ...
    print("  Preview now:  Titan curator run --dry-run")
    print("  Pause it:     Titan curator pause")
    print(
        "  Docs:         https://titan-agent.nousresearch.com/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``Titan update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``Titan update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: Titan curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``Titan dashboard`` processes.

    Called at the end of ``Titan update`` (default ``reason``) and also
    from ``Titan dashboard --stop`` (which overrides ``reason``).  The
    ...
    if killed:
        ...
        print("    Titan dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Titan Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/NousResearch/titan-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="Titan-update-")
        zip_path = os.path.join(tmp_dir, f"titan-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to titan-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"titan-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "Titan-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Titan behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes Titan completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Titan couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Titan couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Titan behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `Titan update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/titan-agent.git",
    "git@github.com:NousResearch/titan-agent.git",
    "https://github.com/NousResearch/titan-agent",
    "git@github.com:NousResearch/titan-agent",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/titan-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_Titan_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_Titan_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Titan repository.")
        print("  This means you may miss updates from NousResearch/titan-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/titan-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/titan-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``Titan update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from Titan_constants import get_default_Titan_root

    default_home = get_default_Titan_root()
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``Titan update`` is still progressing even if pip/uv itself is silent.
    """
...
def _Titan_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "Titan.exe",
        scripts_dir / "Titan-gateway.exe",
    ]


def _quarantine_running_Titan_exe(scripts_dir: Path) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``Titan.exe``.
    ...
    when ``Titan update`` runs, ``Titan.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``Titan.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next Titan invocation by ``_cleanup_quarantined_exes``.
    ...
    for shim in _Titan_exe_shims(scripts_dir):
        if not shim.exists():
            continue
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_Titan_exe`` if uv didn't write replacements."""
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``Titan.exe.old.*`` left by prior updates.

    Called early on every Titan invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``Titan update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `Titan update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``Titan.exe`` / ``Titan-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_Titan_exe`` for the rationale.
    """
    ...
    def _install(args: list[str]) -> None:
        ...
        if scripts_dir is not None:
            moved = _quarantine_running_Titan_exe(scripts_dir)
        try:
...
class _UpdateOutputStream:
    """Stream wrapper used during ``Titan update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.Titan/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``Titan update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``Titan update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.Titan/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``Titan update --gateway``) the update is already
    ...
    try:
        ...
        # Titan_cli.config.get_Titan_home to simulate setup failure.
        from Titan_cli.config import get_Titan_home as _get_Titan_home

        logs_dir = _get_Titan_home() / "logs"
...
def _cmd_update_check():
    """Implement ``Titan update --check``: fetch and report without installing."""
    from Titan_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "pip":
        from Titan_cli.config import recommended_update_command
        from Titan_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``Titan update`` without requiring a reinstall.  The
    ...
    ``Titan`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/Titan.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v Titan'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/Titan, code at /usr/local/lib/titan-agent).
    fhs_link = Path("/usr/local/bin/Titan")
    ...
    path_comment = (
        "# Titan Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of Titan_HOME before running the update.
    ...
    Titan_HOME directories.  The ``--backup`` flag on ``Titan update``
    ...
    try:
        from Titan_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_Titan_home so the user sees ~/.Titan/...
    try:
        from Titan_constants import get_Titan_home, display_Titan_home

        home = get_Titan_home()
        try:
            display_path = f"{display_Titan_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  Titan import {out_path}")
...
def cmd_update(args):
    """Update Titan Agent to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from Titan_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Titan Agent")
        return
...
def _cmd_update_pip(args):
    """Update Titan via pip (for PyPI installs)."""
    ...
    uv = shutil.which("uv")
    if uv:
        cmd = [uv, "pip", "install", "--upgrade", "titan-agent"]
    else:
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "titan-agent"]
    ...
    print("✓ Update complete! Restart Titan to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # the old bytecode (e.g. get_Titan_home added to Titan_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload Titan_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_Titan_home() added since the last release.
        try:
            import importlib
            import Titan_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit Titan_HOME so
        # it is not affected by sync_skills()'s module-level Titan_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's Titan_HOME env var points at the default or a named profile.
        try:
            from Titan_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        if needs_migration:
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: Titan config migrate")
            else:
                print()
                print("Skipped. Run 'Titan config migrate' later to configure.")
        else:
        ...
        # check `Titan curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``Titan update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from Titan_cli.tools_config import install_cua_driver
        ...
        # When running as ``Titan update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_Titan_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            # Discover all Titan-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. Titan-gateway.service or Titan-gateway-coder.service
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: Titan gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: Titan -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Titan gateway unit files are still installed.
        # When both Titan.service (from a pre-rename install) and the
        # current Titan-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `Titan update` surfaces the issue until the user migrates.
        try:
            from Titan_cli.gateway import (
                has_legacy_Titan_units,
                _find_legacy_Titan_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_Titan_units():
                print()
                print("⚠ Legacy Titan gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_Titan_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (Titan.service) fight the current")
                print("  Titan-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    Titan gateway migrate-legacy")
                print()
        ...
        print("  Titan model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``Titan -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from Titan_constants import display_Titan_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `Titan profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_Titan_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → Titan -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.Titan)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  Titan profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     Titan -p {name} chat")
                else:
        ...
        if dist_name:
            ...
            print(f"  (run `Titan profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        from Titan_cli.profiles import profile_exists
        ...
        from Titan_cli.profiles import rename_profile
        ...
        from Titan_cli.profiles import export_profile
        ...
        from Titan_cli.profiles import import_profile
        ...
        import tempfile
        from Titan_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="Titan_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  Titan -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      Titan -p {plan.manifest.name} chat")
        ...
        from Titan_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `Titan profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"Titan -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from Titan_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("Titan_requires"):
            print(f"Requires:     Titan {data['Titan_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from Titan_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.Titan_requires:
        print(f"  Requires: Titan {mf.Titan_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``Titan dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``Titan dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No Titan dashboard processes running.")
        return 0

    print(f"{len(pids)} Titan dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No Titan dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `Titan update`.
    ...
    if "Titan_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["Titan_WEB_DIST"])
            if "Titan_WEB_DIST" in os.environ
            else PROJECT_ROOT / "Titan_cli" / "web_dist"
        )
    ...
    from Titan_cli.web_server import start_server

    embedded_chat = args.tui or os.environ.get("Titan_DASHBOARD_TUI") == "1"
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from Titan_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter Titan log files."""
    from Titan_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from Titan_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``Titan -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``Titan_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``Titan -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``Titan`` or only flags → defaults to ``chat``.
        return False
...
def main():
    ...
    try:
        from Titan_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``Titan.exe.old.*`` quarantine files left by previous
    # ``Titan update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_Titan_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    from Titan_cli._parser import build_top_level_parser
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: Titan-cli)",
    )
    model_parser.add_argument(
    ...
    # =========================================================================
    from Titan_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://titan-agent.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `Titan model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy Titan.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Titan gateway unit files "
            "(e.g. Titan.service) left over from older installs. Profile "
            "units (Titan-gateway-<profile>.service) and unrelated "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider (default: nous). See `Titan proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Titan Agent with an interactive wizard. "
        "Run a specific section: Titan setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'Titan setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Titan.",
    )
    ...
    slack_manifest.add_argument(
        ...
        "writes to $Titan_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Titan")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Titan CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: Titan-cli)"
    )
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate Titan with Spotify via PKCE"
    )
    ...
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set Titan_SPOTIFY_CLIENT_ID)"
    )
    auth_spotify.add_argument(
        "--redirect-uri",
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Titan Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.Titan/scripts/. Default mode: "
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.Titan/scripts/. Pass empty string to clear. "
    ...
    # =========================================================================
    from Titan_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.Titan/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.Titan/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Titan Agent setup",
    )
    ...
    doctor_parser.add_argument(
        ...
        help=(
            ...
            "advisory will no longer trigger startup banners. Run `Titan "
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Titan setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Titan Agent. Use 'Titan debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    Titan debug share              Upload debug report and print URL
    Titan debug share --lines 500  Include more log lines
    Titan debug share --expire 30  Keep paste for 30 days
    Titan debug share --local      Print report locally (no upload)
    Titan debug share --no-redact  Disable upload-time secret redaction
    Titan debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'Titan debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Titan home directory to a zip file",
        description="Create a zip archive of your entire Titan configuration, "
        "skills, sessions, and data (excludes the titan-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/Titan-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.Titan/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo Titan uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from Titan_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Titan backup from a zip file",
        description="Extract a previously created Titan backup into your "
        "Titan home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Titan Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from Titan_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.Titan/skills/.bundled_manifest) "
            "so future 'Titan update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from Titan_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from Titan_cli.skills_hub import skills_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/Titan-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `Titan plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from Titan_cli.plugins_cmd import plugins_command
    ...
    # subcommand — ``Titan --help``, ``Titan version``, ``Titan logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from Titan_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from Titan_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            mem_dir = get_Titan_home() / "memories"
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_Titan_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_Titan_home()}/memories/\n")
        else:
            from Titan_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'Titan tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # Titan tools disable <name...> [--platform cli]
    ...
    # Titan tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from Titan_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from Titan_cli.tools_config import tools_command
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `Titan computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `Titan tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: Titan computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: Titan computer-use install")
            return
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Titan as an MCP server",
        description=(
            "Manage MCP server connections and run Titan as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'Titan mcp add' to connect to a new server, or\n"
            "'Titan mcp serve' to expose Titan conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Titan as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `Titan mcp add ...` to fall through to
    # interactive chat.
    ...
    def cmd_mcp(args):
        from Titan_cli.mcp_config import mcp_command
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_Titan_home() / "sessions"
            ...
            # Launch Titan --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            from Titan_cli.relaunch import relaunch
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Titan",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Titan",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.Titan/ (by default a "
        "single restore-point archive is written to ~/.Titan/backups/ "
        "before apply; restorable with 'Titan import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from Titan_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Titan Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'Titan config migrate' separately for those.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Titan Agent",
        description="Remove Titan Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Titan Agent as an ACP (Agent Client Protocol) server",
        description="Start Titan Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    acp_parser.add_argument(
        ...
        action="store_true",
        dest="acp_version",
        help="Print Titan ACP version and exit",
    )
    acp_parser.add_argument(
        "--check",
        action="store_true",
    ...
    acp_parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Titan provider/model setup for ACP terminal auth",
    )
    acp_parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.Titan/node/ "
    ...
    def cmd_acp(args):
        """Launch Titan Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Titan instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `Titan update` skill sync)",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a Titan profile distribution. SOURCE can be a git URL "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Titan Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `Titan --tui` via PTY/WebSocket). "
            "Alternatively set Titan_DASHBOARD_TUI=1."
        ),
    )
    dashboard_parser.add_argument(
        "--skip-build",
    ...
    # for `Titan dashboard` cmdlines and SIGTERM them directly — the same
    # path `Titan update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Titan dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running Titan dashboard processes and exit",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Titan log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    Titan logs                    Show last 50 lines of agent.log
    Titan logs -f                 Follow agent.log in real time
    Titan logs errors             Show last 50 lines of errors.log
    Titan logs gateway -n 100     Show last 100 lines of gateway.log
    Titan logs --level WARNING    Only show WARNING and above
    Titan logs --session abc123   Filter by session ID
    Titan logs --component tools  Only show tool-related lines
    Titan logs --since 1h         Lines from the last hour
    Titan logs --since 30m -f     Follow, starting from 30 min ago
    Titan logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``Titan -c Pokemon Agent Dev`` → ``Titan -c 'Pokemon Agent Dev'``
    ...
    from Titan_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'Titan -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (Titan hooks list, cron
    ...
    if args.command in _AGENT_COMMANDS or (
        ...
        try:
            from Titan_cli.plugins import discover_plugins
    ...
    if getattr(args, "oneshot", None):
        from Titan_cli.oneshot import run_oneshot
