"""
Titan Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.Titan/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Titan PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing titan-agent or Titan PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Titan Agent" comment and following line
                if '# Titan Agent' in line or '# titan-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('Titan' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing Titan
                if 'Titan' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the Titan wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "Titan",
        Path("/usr/local/bin/Titan"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains Titan_cli reference)
                content = wrapper.read_text()
                if 'Titan_cli' in content or 'titan-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``Titan gateway run`` processes
    ...
    try:
        from Titan_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from Titan_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``Titan_HOME`` and ``Titan_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\Titan\git\cmd``, ``%LOCALAPPDATA%\Titan\git\bin``,
#      ``%LOCALAPPDATA%\Titan\git\usr\bin``, ``%LOCALAPPDATA%\Titan\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\Titan\git\`` and Node to
#      ``%LOCALAPPDATA%\Titan\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``Titan dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\Titan\gateway-service\`` and sometimes
...
def _Titan_path_markers(Titan_home: Path) -> list[str]:
    """Path-entry substrings that identify Titan-owned User-PATH entries."""
    root = str(Titan_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare titan-agent install dir.
    markers = [root + "\\titan-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if Titan_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "Titan".  We don't want to catch
    # unrelated entries like "cTitan-foo" or "ephermeral", so we look for
    # backslash-Titan as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(Titan_home: Path) -> list[str]:
    """Strip Titan-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _Titan_path_markers(Titan_home)
            kept: list[str] = []
...
def remove_Titan_env_vars_windows() -> list[str]:
    """Delete Titan_HOME and Titan_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("Titan_HOME", "Titan_GIT_BASH_PATH"):
                try:
...
def _is_default_Titan_home(Titan_home: Path) -> bool:
    """Return True when ``Titan_home`` points at the default (non-profile) root."""
    try:
        from Titan_constants import get_default_Titan_root
        return Titan_home.resolve() == get_default_Titan_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its Titan_HOME directory.

    We shell out to ``Titan -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current Titan_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m Titan_cli.main` so we don't depend on a `Titan`
    #    wrapper that may be half-removed mid-uninstall.
    Titan_invocation = [_sys.executable, "-m", "Titan_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                Titan_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's Titan_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.Titan/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.Titan/ for future reinstall
    """
    project_root = get_project_root()
    Titan_home = get_Titan_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie Titan_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_Titan_home(Titan_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Titan Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {Titan_home / 'config.yaml'}")
    print(f"  Secrets: {Titan_home / '.env'}")
    print(f"  Data:    {Titan_home / 'cron/'}, {Titan_home / 'sessions/'}, {Titan_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their Titan_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Titan data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Titan code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in Titan_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\Titan\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(Titan_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Titan-owned PATH entries in User environment")

        log_info("Removing Titan_HOME / Titan_GIT_BASH_PATH User env vars...")
        removed_env = remove_Titan_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Titan-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing Titan command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.Titan/, just remove the titan-agent subdir
            if Titan_home in project_root.parents or project_root.parent == Titan_home:
    ...
    #     under Titan_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(Titan_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(Titan_home)
    ...
    # 5. Optionally remove ~/.Titan/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile Titan_HOME dirs live under
        ...
        try:
            if Titan_home.exists():
                shutil.rmtree(Titan_home)
                log_success(f"Removed {Titan_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {Titan_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {Titan_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  irm https://raw.githubusercontent.com/NousResearch/titan-agent/main/scripts/install.ps1 | iex", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/NousResearch/titan-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Titan Agent! ⚕")
    print()
