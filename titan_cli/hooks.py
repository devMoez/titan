"""Titan hooks — inspect and manage shell-script hooks.

Usage::

    Titan hooks list
    Titan hooks test <event> [--for-tool X] [--payload-file F]
    Titan hooks revoke <command>
    Titan hooks doctor

Consent records live under ``~/.Titan/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.Titan/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``Titan hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: Titan hooks {list|test|revoke|doctor}")
        print("Run 'Titan hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.Titan/config.yaml.")
        print("See `Titan hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `Titan hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `Titan hooks test` and `Titan hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Titan wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `Titan hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `Titan hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `Titan hooks doctor`.")
