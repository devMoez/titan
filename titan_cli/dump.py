"""
Dump command for Titan CLI.

Outputs a compact, plain-text summary of the user's Titan setup
...
from Titan_cli.config import get_Titan_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``Titan dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from Titan_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(Titan_home: Path) -> int:
...
def _cron_summary(Titan_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = Titan_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"Titan_home:      {display_Titan_home()}")
    ...
    toolsets = config.get("toolsets", ["Titan-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(Titan_home)}")
    lines.append(f"  skills:             {_count_skills(Titan_home)}")
