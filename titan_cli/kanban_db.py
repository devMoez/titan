...
``<root>`` is the **shared Titan root** (the parent of any active
...
``Titan -p <profile>`` joins the same board as the dispatcher that
...
the profile's Titan home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``Titan_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``Titan_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``Titan kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.Titan``. In Docker / custom
deployments where ``Titan_HOME`` points outside ``~/.Titan`` (e.g.
``/opt/Titan``), ``<root>`` is ``Titan_HOME``. Legacy env-var
overrides still work:

* ``Titan_KANBAN_DB`` — pin the database file path directly.
* ``Titan_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``Titan_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``Titan_KANBAN_DB``,
``Titan_KANBAN_WORKSPACES_ROOT``, and ``Titan_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/Titan-kanban-v1-spec.pdf`` for the full
...
# enough that kebab-case names like ``atm10-server`` or ``titan-agent``
...
def kanban_home() -> Path:
    """Return the shared Titan root that anchors the kanban board.

    Resolution order:

    1. ``Titan_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_Titan_root()``, which already returns ``<root>``
       when ``Titan_HOME`` is ``<root>/profiles/<name>``, and returns
       ``Titan_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``Titan_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("Titan_KANBAN_HOME", "").strip()
    if override:
        return Path(override).expanduser()
    from Titan_constants import get_default_Titan_root
    return get_default_Titan_root()
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``Titan kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``Titan_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``Titan kanban boards
    ...
    """
    env = os.environ.get("Titan_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``Titan kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``Titan_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("Titan_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``Titan_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("Titan_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``Titan kanban log`` unambiguous even when multiple
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``Titan_KANBAN_DB`` env → ``Titan_KANBAN_BOARD`` env →
    ...
    # See Titan_state._WAL_INCOMPAT_MARKERS for detection logic.
    from Titan_state import apply_wal_with_fallback
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``Titan kanban init`` and the
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from Titan_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``Titan --skills ...`` alongside the built-in
    ...
    # `Titan --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `Titan kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``Titan kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``Titan kanban complete <ready-task>
...
def complete_task(
    ...
    *,
    ...
    completion (``Titan kanban complete <id>``) works without requiring
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Titan
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``Titan kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Titan profile.
    ...
    if not rows:
        return False
    try:
        from Titan_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
    ...
    *,
    ...
    for row in ready_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        # Skip ready tasks whose assignee is not a real Titan profile.
        # `_default_spawn` invokes ``Titan -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Titan
...
def _resolve_Titan_argv() -> list[str]:
    """Resolve the ``Titan`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``shutil.which("Titan")`` — the console-script shim, the same form
       that shows up in ``ps`` output and existing logs. Preferred so live
       systems' diagnostics stay familiar.
    2. ``sys.executable -m Titan_cli.main`` — fallback for setups where
       Titan is launched from a venv and the ``Titan`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_Titan_bin`` for the same reason. Kept
    local (not imported from gateway) because ``Titan_cli`` sits below
    ...
    if Titan_bin:
        return [Titan_bin]
    # Fallback to the module form. ``Titan_cli.main`` is the actual
    # console-script target declared in pyproject.toml, NOT a top-level
    # ``Titan`` package — there is no ``Titan`` package to import.
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``Titan -p <profile> chat -q ...`` subprocess.
    ...
    ``Titan_KANBAN_DB`` / ``Titan_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject Titan_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `Titan -p <name>` the
    # _apply_profile_override() runs *before* Titan_constants is imported.
    # If Titan_HOME is absent from the child's env, get_Titan_home() falls
    # back to Path.home() / ".Titan" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from Titan_cli.profiles import resolve_profile_env
    try:
        env["Titan_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via Titan_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # Titan_HOME never had profiles created.
        pass
    if task.tenant:
        env["Titan_TENANT"] = task.tenant
    env["Titan_KANBAN_TASK"] = task.id
    env["Titan_KANBAN_WORKSPACE"] = workspace
    if task.current_run_id is not None:
        env["Titan_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["Titan_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Pin the shared board + workspaces root the dispatcher resolved, so
    # that even when the worker activates a profile (`Titan -p <name>`
    # rewrites Titan_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_Titan_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["Titan_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["Titan_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["Titan_KANBAN_BOARD"] = resolved_board
    # Titan_PROFILE is the author the kanban_comment tool defaults to.
    # `Titan -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["Titan_PROFILE"] = profile_arg
    ...
    # `Titan kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`Titan` executable not found on PATH. "
            "Install Titan Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``Titan kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled Titan_PROFILE values like "Titan-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Titan root exists

    Reads profile paths directly so this module has no import dependency on
    ``Titan_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from Titan_constants import get_default_Titan_root
        default_root = get_default_Titan_root()
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``Titan kanban assignees`` for the terminal.
