"""``Titan logs`` — view and filter Titan log files.
...
Usage examples::
    ...
    Titan logs                    # last 50 lines of agent.log
    Titan logs -f                 # follow agent.log in real time
    Titan logs errors             # last 50 lines of errors.log
    Titan logs gateway -n 100    # last 100 lines of gateway.log
    Titan logs --level WARNING    # only WARNING+ lines
    Titan logs --session abc123   # filter by session ID substring
    Titan logs --component tools  # only tool-related lines
    Titan logs --since 1h         # lines from the last hour
    Titan logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_Titan_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Titan runs — try 'Titan chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_Titan_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_Titan_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_Titan_home()}/logs/")
        return

    print(f"Log files in {display_Titan_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'Titan chat' to generate logs)")
