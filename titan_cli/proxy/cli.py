"""CLI handlers for the ``Titan proxy`` subcommand."""
...
from Titan_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "Titan proxy requires aiohttp. Install one of:\n"
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        print(
            ...
            f"Run `Titan login {adapter.name}` first.",
            file=sys.stderr,
        )
        return 2
    ...
    print(
        f"Starting Titan proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Titan proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: Titan proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``Titan proxy <subcommand>``."""
    ...
    print(
        "Titan proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  Titan proxy start [--provider nous] [--host 127.0.0.1] [--port 8645]\n"
