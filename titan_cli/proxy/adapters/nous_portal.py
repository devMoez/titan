...
Reads the user's Nous OAuth state from ``~/.Titan/auth.json``, refreshes
...
already handles both — see :func:`Titan_cli.auth.refresh_nous_oauth_from_state`.
"""
...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def get_credential(self) -> UpstreamCredential:
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nous Portal. Run `Titan login nous` first."
                )
            ...
            if not agent_key:
                raise RuntimeError(
                    ...
                    "Try `Titan login nous` to re-authenticate."
                )
    ...
    # to Titan_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
