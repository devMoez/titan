"""`Titan checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.Titan/checkpoints/``.  Actions:

    Titan checkpoints               # same as `status`
    Titan checkpoints status        # total size, project count, breakdown
    Titan checkpoints list          # per-project checkpoint counts + workdir
    Titan checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    Titan checkpoints clear [-f]    # nuke the entire base (asks first)
    Titan checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    Titan checkpoints
    Titan checkpoints prune --retention-days 3 --max-size-mb 200
    Titan checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: Titan checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``Titan checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `Titan checkpoints` → status
