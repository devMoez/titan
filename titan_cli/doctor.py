"""
Doctor command for Titan CLI.

Diagnoses issues with Titan Agent setup.
"""
...
from Titan_cli.config import get_project_root, get_Titan_home, get_env_path
...
_DHH = display_Titan_home()  # user-facing display path (e.g. ~/.Titan or ~/.Titan/profiles/coder)

# Load environment variables from ~/.Titan/.env so API key checks work
_env_path = get_env_path()
load_Titan_dotenv(Titan_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from Titan_cli.colors import Colors, color
from Titan_cli.models import _Titan_USER_AGENT
from Titan_cli.vercel_auth import describe_vercel_auth
from Titan_constants import OPENROUTER_MODELS_URL
...
from Titan_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.Titan/.env contains provider auth/base URL settings."""
...
def _is_kanban_worker_env_gate(item: dict) -> bool:
    ...
    if item.get("name") != "kanban":
        return False
    if os.environ.get("Titan_KANBAN_TASK"):
        return False
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("Titan_KANBAN_TASK"):
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from Titan_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `Titan`.
    os.environ.setdefault("Titan_INTERACTIVE", "1")

    # Handle `Titan doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.Titan/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Titan Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`Titan doctor --ack {hit.advisory.id}`."
                )
    ...
    # Check ~/.Titan/.env (primary location for user config)
    env_path = Titan_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'Titan setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'Titan setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'Titan setup' to create one")
                issues.append("Run 'Titan setup' to create .env")
    
    # Check ~/.Titan/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = Titan_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from Titan_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from Titan_cli.providers import (
            ...
            if provider and provider != "auto":
                if catalog_provider is None or (
                    ...
                    issues.append(
                        ...
                        f"Fix: run 'Titan config set model.provider <valid_provider>'"
                    )
            ...
            if runtime_provider and runtime_provider not in {"auto", "custom", "openrouter"}:
                try:
                    from Titan_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if pconfig and getattr(pconfig, "auth_type", "") == "api_key":
                        ...
                        if not configured:
                            check_fail(
                                ...
                                "(check ~/.Titan/.env or run 'Titan setup')",
                            )
                            issues.append(
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'Titan setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'Titan config set model.provider <name>'"
                            )
                except Exception:
                    pass
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from Titan_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from Titan_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'Titan setup' to migrate config")
                else:
                    issues.append("Run 'Titan doctor --fix' or 'Titan setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'Titan doctor --fix'")
        except Exception:
            pass
    ...
    if _safe_which("codex"):
        ...
        # Native OAuth uses Titan' own device-code flow — the Codex CLI is
        # only needed if you want to import existing tokens from
        # ~/.codex/auth.json.  Downgrade to info so users running
        # `Titan auth openai-codex` aren't told they're missing something.
        check_info(
    ...
    for subdir_name in expected_subdirs:
        subdir_path = Titan_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Titan a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Titan Agent Persona\n\n"
                "<!-- Edit this file to customize how Titan communicates. -->\n\n"
    ...
    memories_dir = Titan_home / "memories"
    ...
    state_db_path = Titan_home / "state.db"
    ...
    wal_path = Titan_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'Titan doctor --fix' to checkpoint")
    ...
    # =========================================================================
    # Check: Command installation (Titan bin symlink)
    # =========================================================================
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "Titan"
        ...
        _cmd_link = _cmd_link_dir / "Titan"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(Titan not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/Titan → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/Titan points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/Titan → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/Titan — run 'Titan doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/Titan exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/Titan not found",
                    "(Titan command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/Titan → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/Titan symlink — run 'Titan doctor --fix'")
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            check_fail("vercel SDK not installed", "(pip install 'titan-agent[vercel]')")
            issues.append("Install the Vercel optional dependency: pip install 'titan-agent[vercel]'")
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `Titan doctor` invocation.
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'Titan config set model.provider <provider>' "
    ...
    # never the legitimate source for `Titan doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'Titan setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: Titan skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = Titan_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: Titan memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                check_fail("Honcho API key or base URL not set", "run: Titan memory setup")
                issues.append("No Honcho API key — run 'Titan memory setup'")
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                check_fail("Mem0 API key not set", "(set MEM0_API_KEY in .env or run Titan memory setup)")
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: Titan memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: Titan memory setup")
        except Exception as _e:
    ...
    # =========================================================================
    try:
        from Titan_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "Titan -p" in content:
                            _m = _re.search(r"Titan -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'Titan doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
