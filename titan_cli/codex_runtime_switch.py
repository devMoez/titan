...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Titan'
...
The actual runtime resolution happens in Titan_cli.runtime_provider's
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in ("off", "default", "disable", "Titan"):
        return "auto", []
...
def apply(
    ...
    *,
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate Titan' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into Titan for
        # browser/web/delegate_task/vision/memory tools (#7 fix).
        # Failures are non-fatal — the runtime change still proceeds.
        try:
            from Titan_cli.codex_runtime_plugin_migration import migrate
            mig_report = migrate(config)
            # Tools/MCP servers (excluding the Titan-tools callback,
            # which is internal plumbing — surface separately).
            user_servers = [
                s for s in mig_report.migrated if s != "Titan-tools"
            ]
            if user_servers:
                msg_lines.append(
            ...
            # Permissions + Titan tool callback are always-on production
            ...
            if "Titan-tools" in mig_report.migrated:
                msg_lines.append(
                    "Titan tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default Titan runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "Titan tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default Titan runtime.")
