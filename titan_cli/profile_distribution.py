"""Profile distributions — shareable, packaged Titan profiles via git.

A distribution is a Titan profile published as a git repository (or
...
* ``Titan profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``Titan skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``Titan profile``, not a parallel tree):

    Titan profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "titan-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            Titan_requires=str(data.get("Titan_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.Titan_requires:
            out["Titan_requires"] = self.Titan_requires
        if self.author:
...
def check_Titan_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Titan {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``Titan profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Titan profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Titan distribution."
        )

    # Version check up-front so we fail fast
    check_Titan_requires(manifest.Titan_requires, Titan_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.Titan).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="Titan_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `Titan profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`Titan profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="Titan_dist_update_") as tmp:
        plan = plan_install(
