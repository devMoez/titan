"""Shell completion script generation for Titan CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_Titan_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Titan Agent bash completion
# Add to ~/.bashrc:
#   eval "$(Titan completion bash)"

_Titan_profiles() {{
    local profiles_dir="$HOME/.Titan/profiles"
...
_Titan_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_Titan_profiles)" -- "$cur"))
        return
    fi
...
complete -F _Titan_completion Titan
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef Titan
# Titan Agent zsh completion
# Add to ~/.zshrc:
...
_Titan_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.Titan/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.Titan/profiles 2>/dev/null)}}")
    fi
...
_Titan() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_Titan_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'Titan command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Titan Agent fish completion",
        "# Add to your config:",
        "#   Titan completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __Titan_profiles",
        "    echo default",
        "    if test -d $HOME/.Titan/profiles",
        "        ls $HOME/.Titan/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c Titan -f -s p -l profile"
        " -d 'Profile name' -xa '(__Titan_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__Titan_profiles)' -d 'Profile name'"
                )
