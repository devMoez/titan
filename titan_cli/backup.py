"""
Backup and import commands for Titan CLI.

`Titan backup` creates a zip archive of the entire ~/.Titan/ directory
(excluding the titan-agent repo and transient files).

`Titan import` restores from a backup zip, overlaying onto the current
Titan_HOME root.
"""
...
from Titan_constants import get_default_Titan_root, get_Titan_home, display_Titan_home
...
_EXCLUDED_DIRS = {
    "titan-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to Titan root) should be skipped."""
...
def run_backup(args) -> None:
    """Create a zip backup of the Titan home directory."""
    Titan_root = get_default_Titan_root()

    if not Titan_root.is_dir():
        print(f"Error: Titan home directory not found at {Titan_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"Titan-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"Titan-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_Titan_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(Titan_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(Titan_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(Titan_root)

            if _should_exclude(rel):
                continue

            # Skip the output zip itself if it happens to be inside Titan root
            try:
    ...
    print(f"\nRestore with: Titan import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Titan backup.
    ...
    # Look for telltale files that a Titan home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Titan backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.Titan/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a Titan dir name
        if prefix in {".Titan", "Titan"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_Titan_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (Titan_root / "config.yaml").exists()
        has_env = (Titan_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Titan configuration.")
        ...
        for member in members:
            ...
            target = Titan_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(Titan_root.resolve())
            except ValueError:
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # Titan_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: Titan profile list  (after installing Titan)")

        # Guidance
        print()
        if not (Titan_root / "titan-agent").is_dir():
            print("Note: The titan-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: Titan update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  Titan -p {pname} gateway install")

        print("Done. Your Titan configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and Titan backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to Titan_HOME).
...
# — `Titan update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(Titan_home: Optional[Path] = None) -> Path:
    home = Titan_home or get_Titan_home()
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(Titan_home)
    if not root.exists():
        return []
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(Titan_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for Titan backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_Titan_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, Titan_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``Titan_root`` to ``out_path``.
...
def _pre_update_backup_dir(Titan_home: Optional[Path] = None) -> Path:
    home = Titan_home or get_Titan_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of Titan_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<Titan_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``Titan update``) should continue even if the backup fails.
    """
    Titan_root = Titan_home or get_default_Titan_root()
    if not Titan_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(Titan_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, Titan_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `Titan claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of Titan_HOME under ``backups/`` before a
    ``Titan claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``Titan import <archive>``.  Writes to
    ``<Titan_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `Titan import` and the
