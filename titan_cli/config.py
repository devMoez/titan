"""
Configuration management for Titan Agent.

Config files are stored in ~/.Titan/ for easy access:
- ~/.Titan/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.Titan/.env         - API keys and secrets

This module provides:
- Titan config          - Show current configuration
- Titan config edit     - Open config in editor
- Titan config set      - Set a specific value
- Titan config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.Titan/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``Titan logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  Titan config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `Titan plugins enable
    # observability/langfuse` or `Titan tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "Titan_LANGFUSE_ENV",
    "Titan_LANGFUSE_RELEASE",
    "Titan_LANGFUSE_SAMPLE_RATE",
    "Titan_LANGFUSE_MAX_CHARS",
    "Titan_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from Titan_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("Titan_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_Titan_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Titan is running in package-manager-managed mode.

    Two signals: the Titan_MANAGED env var (set by the systemd service),
    or a .managed marker file in Titan_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade titan-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Titan was installed: 'nixos', 'homebrew', 'git', or 'pip'."""
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "pip":
        ...
        uv = shutil.which("uv")
        if uv:
            return "uv pip install --upgrade titan-agent"
        return "pip install --upgrade titan-agent"
    return "Titan update"
...
def format_managed_message(action: str = "modify this Titan installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("Titan_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Titan installation is managed by NixOS "
            f"(Titan_MANAGED={env_hint}).\n"
            "Edit services.titan-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Titan installation is managed by Homebrew "
            f"(Titan_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade titan-agent"
        )

    return (
        f"Cannot {action}: this Titan installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Titan."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from Titan_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, Titan_bin
    or None if container mode is not active, we're already inside the
    container, or Titan_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("Titan_DEV") == "1":
        return None
    ...
    container_mode_file = get_Titan_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "titan-agent")
    exec_user = info.get("exec_user", "Titan")
    Titan_bin = info.get("Titan_bin", "/data/current-package/bin/Titan")
...
# Re-export from Titan_constants — canonical definition lives there.
from Titan_constants import get_Titan_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_Titan_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_Titan_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the Titan group can
    share state with the gateway service.

    The mode can be overridden via the Titan_HOME_MODE environment variable
    (e.g. Titan_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse Titan_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("Titan_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Titan runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("Titan_CONTAINER") or os.environ.get("Titan_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set Titan_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into Titan_HOME if the user doesn't have one yet."""
...
def ensure_Titan_home():
    """Ensure ~/.Titan directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_Titan_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_Titan_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"Titan_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Titan-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, Titan auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Titan sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Titan
        ...
        # bundled Titan image, which drops to the `Titan` user via gosu).
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Titan sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Titan should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``Titan chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.Titan/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: Titan sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Titan truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``Titan kanban specify`` (single id or --all). Set a
        ...
        # model; override via `Titan model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `Titan --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `Titan -c` muscle memory.  Default off so existing
        # users aren't surprised.  Titan_TUI_RESUME=<id> always wins.
        ...
        # e.g. `model · 68% · ~/projects/Titan`. Per-platform overrides go under
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.Titan/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.Titan/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Titan feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Titan auto-pauses the goal and
    ...
    # always goes to ~/.Titan/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${Titan_SKILL_DIR} and ${Titan_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `Titan curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot ~/.Titan/skills/ into
        # ~/.Titan/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `Titan curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for Titan-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Titan Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (Titan_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.Titan/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or Titan_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `Titan doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``Titan_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Titan to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via Titan_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `Titan -p <assignee> chat -q ...` for
    ...
    "code_execution": {
        ...
        #     titan-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.Titan/logs/.
    ...
    # update model picker lists without shipping a titan-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://titan-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `Titan model` invocation; network failures
    ...
    # Session storage — controls automatic cleanup of ~/.Titan/state.db.
    ...
    "sessions": {
        ...
        # default of ``Titan sessions prune``.
        "retention_days": 90,
    ...
    # ``Titan update`` behaviour.
    "updates": {
        # Run a full ``Titan backup``-style zip of Titan_HOME before every
        # ``Titan update``.  Backups land in ``<Titan_HOME>/backups/`` and
        # can be restored with ``Titan import <path>``.  Off by default —
        # on large Titan_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<Titan_HOME>/lsp/bin/`` on first use.
    ...
    # `Titan tools`. These settings tune the backing Responses API call.
    "x_search": {
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'Titan model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Titan auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @Titan:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. Titan_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #Titan)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: Titan-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'titan-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Titan API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Titan API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Titan API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # Titan_TOOL_PROGRESS and Titan_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.Titan.config`` entries, then checks
    ...
    try:
        ...
        # should never break `Titan update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Titan versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``Titan_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'Titan setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Titan won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'Titan doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("Titan_HOME", "~/.Titan")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("Titan_TOOL_PROGRESS")
            old_mode = get_env_value("Titan_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from Titan_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from Titan_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("Titan_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from Titan_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$Titan_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_Titan_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `Titan plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `Titan curator status` has no
    ...
    #   3. Creates `~/.Titan/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_Titan_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_Titan_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: Titan config set <key> <value>")
    ...
    # metadata.Titan.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _Titan_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.Titan/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.Titan/config.yaml.
    ...
    (which change ``Titan_HOME`` and therefore ``get_config_path()``)
    don't collide.
    """
    with _CONFIG_LOCK:
        ensure_Titan_home()
...
#   openai-codex (OAuth — Titan auth) — OpenAI Codex
#   nous         (OAuth — Titan auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.Titan/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.Titan/.env.
    ...
    interactive menu render (`Titan tools`, `Titan setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `Titan tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Titan env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.Titan/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.Titan/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.Titan/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.Titan/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Titan — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Titan vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.Titan/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Titan Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  Titan config edit     # Edit config file", Colors.DIM))
    print(color("  Titan config set <key> <value>", Colors.DIM))
    print(color("  Titan setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: Titan config set <key> <value>")
            print()
            print("Examples:")
            print("  Titan config set model anthropic/claude-sonnet-4")
            print("  Titan config set terminal.backend docker")
            print("  Titan config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'Titan config migrate' to add them")
        ...
        print("  Titan config           Show current configuration")
        print("  Titan config edit      Open config in editor")
        print("  Titan config set <key> <value>   Set a config value")
        print("  Titan config check     Check for missing/outdated config")
        print("  Titan config migrate   Update config with new options")
        print("  Titan config path      Show config file path")
        print("  Titan config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``Titan config`` UI so users
