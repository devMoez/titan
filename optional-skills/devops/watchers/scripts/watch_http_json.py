...
Usage (via cron with --no-agent):
    ...
    Titan cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$Titan_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Titan-Watcher/1.0"})
