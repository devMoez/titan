...
Titan kanban watch --tenant <project-slug>

# Snapshot of the board
Titan kanban list --tenant <project-slug>
Titan kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
Titan kanban stats --tenant <project-slug>

# Visual dashboard (browser)
Titan dashboard

# Inspect a specific task (includes comments + events)
Titan kanban show <task-id>

# Follow a single task's event stream
Titan kanban tail <task-id>
```

Verify available subcommands with `Titan kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `Titan kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `Titan kanban block <id>` then `Titan kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `Titan kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `Titan kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `Titan profile list` (profile exists?), `Titan status` (gateway/dispatcher up?), and verify tenant. |
...
Titan kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
Titan kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(Titan kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
Titan kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
Titan kanban block <task-id>
Titan kanban archive <task-id>

# Diagnose what happened
Titan kanban show <task-id>      # task body, comments, recent events
Titan kanban tail <task-id>      # follow the live event stream
Titan kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `Titan kanban create` for the director
...
while true; do
    clear
    Titan kanban list --tenant <slug>
    echo "---"
    Titan kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `Titan kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `Titan profile list` and start
