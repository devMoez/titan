...
Maps each role archetype to the Titan skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public titan-agent
...
## Titan skills relevant to video production

### Visual / rendering skills (`titan-agent/skills/creative/`)
...
### Design / pre-production skills (`titan-agent/skills/creative/`)
...
### Audio / media skills (`titan-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`titan-agent/skills/devops/`)
...
These are **not** Titan skills but external CLIs / APIs that profiles invoke.
...
## Built-in Titan tools for media review

These are native Titan tools — not invoked via terminal but through their own
...
key is present in `~/.Titan/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Titan profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.Titan/.env` → environment variable.
...
When a new Titan-public video skill ships:
