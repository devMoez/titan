...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/titan-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    Titan_meta = (fm.get("metadata") or {}).get("Titan") or {}
    tags = Titan_meta.get("tags") or []
    related = Titan_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `Titan skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Titan Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Titan ships with a large built-in skill library copied into `~/.Titan/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Titan also syncs bundled skills on `Titan update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.Titan/skills/` tree, it is still shipped with Titan; restore it with `Titan skills reset <name> --restore`.",
        "",
