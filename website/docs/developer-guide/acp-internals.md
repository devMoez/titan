...
The ACP adapter wraps Titan' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
Titan acp / Titan-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.Titan/.env
  -> configure stderr logging
  -> construct TitanACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'titan-agent[acp]==<version>' Titan-acp`, pointed at the `titan-agent` PyPI release.
...
- `allow_once` -> Titan `once`
- `allow_always` -> Titan `always`
- reject options -> Titan `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Titan tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["Titan-acp"])
...
Instead it reuses Titan' runtime resolver:

- `acp_adapter/auth.py`
- `Titan_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Titan provider/credentials. It also always advertises a terminal setup auth method (`Titan-setup`, args `--setup`) so first-run registry clients can open Titan' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.Titan/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `Titan-acp` toolset definition
- `Titan_cli/main.py` — `Titan acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `Titan-acp` script
