...
description: "How Titan stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `Titan_cli/cron.py` | CLI `Titan cron` subcommands |
...
Jobs are stored in `~/.Titan/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `Titan cron` commands are run or during active CLI sessions.
...
```python
# ~/.Titan/scripts/check_competitors.py
...
2. **Environment variable** — `Titan_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.Titan/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `Titan cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `Titan cron` CLI provides direct job management:

```bash
Titan cron list                    # Show all jobs
Titan cron create                  # Interactive job creation (alias: add)
Titan cron edit <job_id>           # Edit job configuration
Titan cron pause <job_id>          # Pause a running job
Titan cron resume <job_id>         # Resume a paused job
Titan cron run <job_id>            # Trigger immediate execution
Titan cron remove <job_id>         # Delete a job
```
