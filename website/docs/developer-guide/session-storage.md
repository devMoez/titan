...
Titan Agent uses a SQLite database (`~/.Titan/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `Titan_state.py`


## Architecture Overview

```
~/.Titan/state.db (SQLite, WAL mode)
...
Multiple Titan processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.Titan/state.db
...
Default path: `~/.Titan/state.db`

This is derived from `Titan_constants.get_Titan_home()` which resolves to
`~/.Titan/` by default, or the value of `Titan_HOME` environment variable.
