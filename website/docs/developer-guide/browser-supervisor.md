...
[PR #12550](https://github.com/NousResearch/titan-agent/pull/12550) proposed a
...
(`Titan-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
One `asyncio.Task` running in a background daemon thread per Titan `task_id`.
...
- `toolsets.py` — register `browser_dialog` in `browser`, `Titan-acp`, `Titan-api-server`, core toolsets (gated on CDP reachability)
...
- `Titan_cli/config.py` — add `browser.dialog_policy` and `browser.dialog_timeout_s` to `DEFAULT_CONFIG`
