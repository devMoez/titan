...
Titan Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.Titan/state.db`) — structured session metadata with FTS5 full-text search
2. **JSONL transcripts** (`~/.Titan/sessions/`) — raw conversation transcripts including tool calls (gateway)
...
Titan stores session history so it can resume conversations, but it does not
...
Titan explicitly injects for that turn.
...
For example, if a user sends an image and asks Titan to make a meme from it,
Titan may inspect that image once with vision and run an image-processing
...
`Titan sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`Titan` or `Titan chat`) |
...
Titan -c "my project"   # → resumes "my project #3"
```
...
Titan --resume 20250305_091523_a1b2c3d4
Titan -r 20250305_091523_a1b2c3d4

# Resume by title
Titan --resume "refactoring auth"

# Or with the chat subcommand
Titan chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `Titan sessions list`.

### Conversation Recap on Resume

When you resume a session, Titan displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Titan session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.Titan/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `Titan -r "<title>"` from the shell) and pick up where the platform left off.
...
Titan automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `Titan sessions list` or `Titan sessions browse`.
...
```bash
Titan sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Titan creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`Titan -c "my project"`), it automatically picks the most recent session in the lineage.
...
Titan provides a full set of session management commands via `Titan sessions`:
...
# Filter by platform
Titan sessions list --source telegram

# Show more sessions
Titan sessions list --limit 50
```
...
Titan sessions export backup.jsonl

# Export sessions from a specific platform
Titan sessions export telegram-history.jsonl --source telegram

# Export a single session
Titan sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
Titan sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
Titan sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
Titan sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
Titan sessions prune

# Custom age threshold
Titan sessions prune --older-than 30

# Only prune sessions from a specific platform
Titan sessions prune --source telegram --older-than 60

# Skip confirmation
Titan sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`Titan insights`](/docs/reference/cli-commands#Titan-insights).
...
When Titan cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Titan uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Titan in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.Titan/state.db` | All session metadata + messages with FTS5 |
| Gateway transcripts | `~/.Titan/sessions/` | JSONL transcripts per session + sessions.json index |
| Gateway index | `~/.Titan/sessions/sessions.json` | Maps session keys to active session IDs |
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Titan process in the same `Titan_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.Titan/config.yaml`:
...
Titan sessions delete <session_id>
