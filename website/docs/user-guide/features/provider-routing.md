...
When using [OpenRouter](https://openrouter.ai) as your LLM provider, Titan Agent supports **provider routing** — fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
Add a `provider_routing` section to your `~/.Titan/config.yaml`:
...
- **CLI mode** — configured in `~/.Titan/config.yaml`, loaded at startup
