...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Titan Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Titan Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Titan](/docs/guides/use-voice-mode-with-Titan).

## Prerequisites

Before using voice features, make sure you have:

1. **Titan Agent installed** — `pip install titan-agent` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `Titan model` or set your preferred provider credentials in `~/.Titan/.env`
3. **A working base setup** — run `Titan` to verify the agent responds to text before enabling voice

:::tip
The `~/.Titan/` directory and default `config.yaml` are created automatically the first time you run `Titan`. You only need to create `~/.Titan/.env` manually for API keys.
:::
...
pip install "titan-agent[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
pip install "titan-agent[messaging]"

# Premium TTS (ElevenLabs)
pip install "titan-agent[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "titan-agent[all]"
```
...
Voice mode is available in both the **classic CLI** (`Titan chat`) and the **TUI** (`Titan --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.Titan/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
Titan                # Start the interactive CLI
```
...
1. Start the CLI with `Titan` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.Titan/config.yaml` (default: `ctrl+b`).
:::
...
```bash
Titan gateway        # Start the gateway (connects to configured platforms)
Titan gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@Titanbyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.Titan/.env`:
```bash
...
```bash
Titan gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.Titan/.env`
...
- Review gateway logs: `tail -f ~/.Titan/logs/gateway.log`
