...
description: "Connect Titan Agent to external tool servers via MCP — and control exactly which MCP tools Titan loads"
---

# MCP (Model Context Protocol)

MCP lets Titan Agent connect to external tool servers so the agent can use tools that live outside Titan itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted Titan to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

## What MCP gives you

- Access to external tool ecosystems without writing a native Titan tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want Titan to see
...
2. Add an MCP server to `~/.Titan/config.yaml`:
...
4. Ask Titan to use the MCP-backed capability.
...
Titan will discover the MCP server's tools and use them like any other tool.
...
HTTP MCP servers are remote endpoints Titan connects to directly.
...
- you do not want Titan spawning a local subprocess for that integration

## Basic configuration reference

Titan reads MCP config from `~/.Titan/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, Titan skips the server entirely |
...
## How Titan registers MCP tools

Titan prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — Titan sees the tool and chooses it during normal reasoning.

## MCP utility tools

When supported, Titan also registers utility tools around MCP resources and prompts:
...
- Titan only registers resource utilities if the MCP session actually supports resource operations
- Titan only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to Titan, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, Titan skips the server completely and does not even attempt a connection.
...
You can also separately disable Titan-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, Titan does not create an empty runtime MCP toolset for that server.
...
Titan discovers MCP servers at startup and registers their tools into the normal tool registry.

### Dynamic Tool Discovery

MCP servers can notify Titan when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When Titan receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, Titan does not blindly pass your full shell environment.
...
Then verify your config and restart Titan.
...
Because Titan now only registers those wrappers when both are true:
...
When `supports_parallel_tool_calls` is `true`, Titan may execute multiple tools from that server at the same time within a single tool-call batch, just like it does for built-in read-only tools (web_search, read_file, etc.).
...
MCP servers can request LLM inference from Titan via the `sampling/createMessage` protocol. This allows an MCP server to ask Titan to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running Titan as an MCP server

In addition to connecting **to** MCP servers, Titan can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use Titan's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through Titan
- You want a single MCP server that bridges to all of Titan's connected messaging platforms at once
- You already have a running Titan gateway with connected platforms
...
Add Titan to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed Titan in a specific location:

```json
{
  "mcpServers": {
    "Titan": {
      "command": "/home/user/.Titan/titan-agent/venv/bin/Titan",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a Titan-specific channel browser:
...
The MCP server includes a live event bridge that polls Titan's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
```bash
Titan mcp serve              # Normal mode
Titan mcp serve --verbose    # Debug logging on stderr
```

### How it works

The MCP server reads conversation data directly from Titan's session store (`~/.Titan/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same `send_message` infrastructure as the Titan agent itself.
