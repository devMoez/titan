...
Titan runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Titan
...
1. Titan captures a baseline of current diagnostics for the file.
...
…). Titan auto-detects the binary on PATH or in
`<Titan_HOME>/lsp/bin/`.
...
`node_modules` tree — Titan installs both packages together when you
run `Titan lsp install typescript` or auto-install fires on first
use.

## CLI

```
Titan lsp status          # service state + per-server install status
Titan lsp list            # registry, optionally --installed-only
Titan lsp install <id>    # eagerly install one server
Titan lsp install-all     # try every server with a known recipe
Titan lsp restart         # tear down running clients
Titan lsp which <id>      # print resolved binary path
```

`Titan lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <Titan_HOME>/lsp/bin
...
When `install_strategy: auto`, Titan installs binaries into
`<Titan_HOME>/lsp/bin/`. NPM packages land in
`<Titan_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Titan-owned and is
...
Servers are kept alive for the life of the Titan process. There's
...
**`Titan lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<Titan_HOME>/lsp/bin/`. Run
`Titan lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `Titan lsp status`**
...
When `Titan lsp status` shows a `Backend warnings` section, install
...
`~/.Titan/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.Titan/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `Titan lsp restart` to clear the set;
