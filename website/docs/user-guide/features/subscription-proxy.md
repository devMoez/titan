...
chat completions — use your Titan-managed provider subscription as their
...
| Use case | "Use Titan as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Titan stores
the refresh token in `~/.Titan/auth.json` — the same place all Titan
...
```
Starting Titan proxy for Nous Portal
...
Model:      Titan-4-70B    # or Titan-4.3-36B, Titan-4-405B
```
...
```
Titan proxy upstream adapters

  [nous    ] Nous Portal — ready (bearer expires 2026-05-15T06:43:21Z)
```

If you see `not logged in`, run `Titan login nous`. If you see
...
INFERENCE_TEXT_MODEL=Titan-4-70B
```
...
```bash
Titan proxy start --host 0.0.0.0 --port 8645
```
...
`Titan_cli/proxy/adapters/<provider>.py` and registering it in
