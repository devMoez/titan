...
sidebar_position: 8
title: "Context Files"
description: "Project context files — .Titan.md, AGENTS.md, CLAUDE.md, global SOUL.md, and .cursorrules — automatically injected into every conversation"
---

# Context Files

Titan Agent automatically discovers and loads context files that shape how it behaves. Some are project-local and discovered from your working directory. `SOUL.md` is now global to the Titan instance and is loaded from `Titan_HOME` only.

## Supported Context Files

| File | Purpose | Discovery |
|------|---------|-----------| 
| **.Titan.md** / **Titan.md** | Project instructions (highest priority) | Walks to git root |
| **AGENTS.md** | Project instructions, conventions, architecture | CWD at startup + subdirectories progressively |
| **CLAUDE.md** | Claude Code context files (also detected) | CWD at startup + subdirectories progressively |
| **SOUL.md** | Global personality and tone customization for this Titan instance | `Titan_HOME/SOUL.md` only |
| **.cursorrules** | Cursor IDE coding conventions | CWD only |
| **.cursor/rules/*.mdc** | Cursor IDE rule modules | CWD only |

:::info Priority system
Only **one** project context type is loaded per session (first match wins): `.Titan.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. **SOUL.md** is always loaded independently as the agent identity (slot #1).
:::
...
- `~/.Titan/SOUL.md`
- or `$Titan_HOME/SOUL.md` if you run Titan with a custom home directory

Important details:

- Titan seeds a default `SOUL.md` automatically if one does not exist yet
- Titan loads `SOUL.md` only from `Titan_HOME`
- Titan does not probe the working directory for `SOUL.md`
...
Titan is compatible with Cursor IDE's `.cursorrules` file and `.cursor/rules/*.mdc` rule modules. If these files exist in your project root and no higher-priority context file (`.Titan.md`, `AGENTS.md`, or `CLAUDE.md`) is found, they're loaded as the project context.

This means your existing Cursor conventions automatically apply when using Titan.
...
1. **Scan working directory** — checks for `.Titan.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules` (first match wins)
