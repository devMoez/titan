...
|------------|--------|-------|
| **SuperGrok OAuth** (preferred) | Browser login at `accounts.x.ai`, refreshed automatically | `Titan auth add xai-oauth` — see [xAI Grok OAuth (SuperGrok Subscription)](../../guides/xai-grok-oauth.md) |
| **`XAI_API_KEY`** | Paid xAI API key | Set in `~/.Titan/.env` |
...
Off by default. Enable in `Titan tools`:
...
The tool surfaces this when both auth paths fail. Either set `XAI_API_KEY` in `~/.Titan/.env` or run `Titan auth add xai-oauth` and complete the browser login. Then restart your session so the agent re-reads the tool registry.
...
1. **Toolset not enabled.** Run `Titan tools` and confirm `🐦 X (Twitter) Search` is checked.
2. **No xAI credentials.** The check_fn returns False, so the schema stays hidden. Run `Titan auth status` to confirm xai-oauth login state, and check that `XAI_API_KEY` is set (if you're using the API-key path).
