...
Titan Agent is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, Titan checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.Titan/config.yaml`:
...
1. **CLI flag**: Start a session with `Titan --yolo` or `Titan chat --yolo`
2. **Slash command**: Type `/yolo` during a session to toggle it on/off
3. **Environment variable**: Set `Titan_YOLO_MODE=1`
...
YOLO mode is available in both CLI and gateway sessions. Internally, it sets the `Titan_YOLO_MODE` environment variable which is checked before every command execution.
...
Some commands are so catastrophic — irreversible filesystem wipes, fork bombs, direct block-device writes — that Titan refuses to run them **regardless** of:
...
Configure the timeout in `~/.Titan/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.Titan/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.Titan/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` Titan/gateway | Self-termination prevention |
...
The `Titan_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.Titan/config.yaml`:
...
:::tip
Use `Titan config edit` to review or remove patterns from your permanent allowlist.
:::

## User Authorization (Gateway)

When running the messaging gateway, Titan controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.Titan/.env`:
...
For more flexible authorization, Titan includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `Titan pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.Titan/config.yaml`:
...
Titan pairing approve telegram ABC12DEF

# Revoke a user's access
Titan pairing revoke telegram 123456789

# Clear all pending codes
Titan pairing clear-pending
```

**Storage:** Pairing data is stored in `~/.Titan/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, Titan applies strict security hardening to every container.
...
Container resources are configurable in `~/.Titan/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.Titan/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `Titan_HOME`. Skills declare these in frontmatter:
...
When loaded, Titan checks if these files exist in the active profile's `Titan_HOME` and registers them for mounting:
...
Paths are relative to `~/.Titan/`. Files are mounted to `/root/.Titan/` inside the container.
...
| **terminal** (local) | Blocks explicit Titan infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- Titan infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
Titan integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
Tirith ships prebuilt binaries for Linux (x86_64 / aarch64) and macOS (x86_64 / arm64). On platforms with no prebuilt binary (Windows, etc.), tirith is silently skipped — pattern-matching guards still run, and the CLI does not surface an "unavailable" banner. To use tirith on Windows, run Titan under WSL.
...
4. **Store secrets securely** — keep API keys in `~/.Titan/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.Titan/logs/` for unauthorized access attempts
10. **Keep updated** — run `Titan update` regularly for security patches
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.Titan/.env`:
...
TERMINAL_SSH_USER=Titan
TERMINAL_SSH_KEY=~/.ssh/titan_agent_key
```
