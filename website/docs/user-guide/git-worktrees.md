...
title: "Git Worktrees"
description: "Run multiple Titan agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Titan Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Titan so each session has a clean, isolated working directory.

## Why Use Worktrees with Titan?

Titan treats the **current working directory** as the project root:

- CLI: the directory where you run `Titan` or `Titan chat`
...
git worktree add ../repo-feature feature/Titan-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/Titan-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Titan there:

```bash
cd ../repo-feature

# Start Titan in the worktree
Titan
```
...
git worktree add ../repo-experiment-a feature/Titan-a
git worktree add ../repo-experiment-b feature/Titan-b
```
...
- Works on its own branch (`feature/Titan-a` vs `feature/Titan-b`).
...
- Titan checkpoint data under `~/.Titan/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Titan experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/Titan-checkpoints-docs`, `feature/Titan-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Titan from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `Titan -w` (Automatic Worktree Mode)

Titan has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `Titan/Titan-<hash>`).
...
```bash
Titan -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `Titan -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Titan session its own clean checkout.
