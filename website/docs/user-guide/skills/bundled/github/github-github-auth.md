...
- Give it a name like "titan-agent"
- Select scopes:
...
- Give it a title like "titan-agent-&lt;machine-name>"
...
elif [ -f ~/.Titan/.env ] && grep -q "^GITHUB_TOKEN=" ~/.Titan/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.Titan/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
