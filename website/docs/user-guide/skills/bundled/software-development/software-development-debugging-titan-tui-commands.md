---
title: "Debugging Titan Tui Commands — Debug Titan TUI slash commands: Python, gateway, Ink UI"
sidebar_label: "Debugging Titan Tui Commands"
description: "Debug Titan TUI slash commands: Python, gateway, Ink UI"
---
...
Debug Titan TUI slash commands: Python, gateway, Ink UI.
...
| Path | `skills/software-development/debugging-Titan-tui-commands` |
| Version | `1.0.0` |
| Author | Titan Agent |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `debugging`, `titan-agent`, `tui`, `slash-commands`, `typescript`, `python` |
...
# Debugging Titan TUI Slash Commands
...
Use this skill when you encounter issues with slash commands in the Titan TUI, particularly when commands aren't showing in autocomplete, aren't working properly in the TUI, or need to be added/updated.
...
<!-- ascii-guard-ignore -->
```
Python backend (Titan_cli/commands.py)     <- canonical COMMAND_REGISTRY
       │
       ▼
...
3. **Check if the command exists in the Python backend:**
   ```bash
   search_files --pattern "CommandDef" --file_glob "*.py" --path Titan_cli/
   search_files --pattern "commandname" --path Titan_cli/commands.py --context 3
   ```
...
4. If the command runs server-side, add a handler in `TitanCLI.process_command()` in `cli.py`:
   ```python
...
1. **Command shows in TUI but not in autocomplete.** The command is defined in the TUI codebase but missing from `COMMAND_REGISTRY` in `Titan_cli/commands.py`. Autocomplete data ships from Python.
...
1. Rebuild the TUI:
   ```bash
   cd /home/bb/titan-agent && npm --prefix ui-tui run build
   ```
...
4. Execute the command and confirm:
   ...
   - Any persisted config updates correctly (`read_file ~/.Titan/config.yaml`)
