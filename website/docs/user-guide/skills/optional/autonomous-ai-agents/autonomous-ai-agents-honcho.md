...
description: "Configure and use Honcho memory with Titan -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
| | |
|---|---|
| Source | Optional — install with `Titan skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Titan

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Titan profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/Titan#running-honcho-locally-with-Titan

### Verify

```bash
Titan honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Titan profile's AI peer
...
Honcho models conversations as interactions between **peers**. Titan creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Titan instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Titan run |
| `global` | Single session across all directories |

Manual override: `Titan honcho map my-project-name`
...
Each Titan profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block Titan.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `Titan.coder` host block in `honcho.json`
...
```bash
Titan honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Titan when Honcho memory is active.
...
Config file: `$Titan_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `Titan honcho setup`. Ensure `memory.provider: honcho` is in `~/.Titan/config.yaml`.

### Memory not persisting across sessions
Check `Titan honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `Titan profile create <name> --clone`. For existing profiles: `Titan honcho sync`.
...
| Command | Description |
|---------|-------------|
| `Titan honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `Titan honcho status` | Show resolved config, connection test, peer info for active profile |
| `Titan honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `Titan honcho disable` | Disable Honcho for the active profile |
| `Titan honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `Titan honcho peers` | Show peer identities across all profiles |
| `Titan honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `Titan honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `Titan honcho sessions` | List known directory-to-session-name mappings |
| `Titan honcho map <name>` | Map current working directory to a Honcho session name |
| `Titan honcho identity` | Seed AI peer identity or show both peer representations |
| `Titan honcho sync` | Create host blocks for all Titan profiles that don't have one yet |
| `Titan honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Titan + Honcho |
| `Titan memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `Titan memory status` | Show active memory provider and config |
| `Titan memory off` | Disable external memory provider |
