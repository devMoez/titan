...
| | |
|---|---|
| Source | Optional — install with `Titan skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Titan Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.Titan/.env`:
...
```bash
CANVAS="python $Titan_HOME/skills/productivity/canvas/scripts/canvas_api.py"
