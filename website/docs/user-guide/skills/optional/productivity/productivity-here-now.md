...
| | |
|---|---|
| Source | Optional — install with `Titan skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${Titan_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${Titan_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${Titan_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client Titan
```
...
```bash
PUBLISH="${Titan_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client Titan
```
...
```bash
DRIVE="${Titan_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `Titan`)    |
