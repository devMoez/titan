...
| | |
|---|---|
| Source | Optional — install with `Titan skills install official/devops/watchers` |
...
4. **Putting the state dir where the agent's sandbox can't write.** `$Titan_HOME/watcher-state/` is always writable. Docker/Modal backends may not see arbitrary host paths.
