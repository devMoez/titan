...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Titan Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Titan to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.Titan/.env`:
...
All standard Titan commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.Titan/logs/gateway.log`
...
4. Consider enabling verbose logging: `Titan_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Titan automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
Titan_LOG_LEVEL=debug Titan gateway
```
...
```bash
Titan chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
