...
This tutorial walks you through setting up a Telegram bot powered by Titan Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Titan Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.Titan/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Titan itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Titan Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_Titan_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Titan Agent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Titan Gateway...
...
# View live logs
journalctl --user -u Titan-gateway -f
...
Titan gateway start
Titan gateway stop
tail -f ~/.Titan/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `Titan gateway install` to update the plist.
:::
...
```bash
Titan gateway stop && Titan gateway start
```
...
Titan pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.Titan/.env`:
...
Control how much detail the bot shows when using tools. In `~/.Titan/config.yaml`:
...
Customize how the bot communicates by editing `~/.Titan/SOUL.md`:

For a full guide, see [Use SOUL.md with Titan](/docs/guides/use-soul-with-Titan).
...
```bash
# From the CLI
Titan cron list          # View all scheduled jobs
Titan cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u Titan-gateway` (Linux) or `~/.Titan/logs/gateway.log` (macOS) |
| Cron job output | `~/.Titan/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.Titan/cron/jobs.json` |
| Pairing data | `~/.Titan/pairing/` |
| Session history | `~/.Titan/sessions/` |
