---
sidebar_position: 6
title: "Use MCP with Titan"
description: "A practical guide to connecting MCP servers to Titan Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Titan

This guide shows how to actually use MCP with Titan Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Titan tool
- you want Titan to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Titan to internal APIs, databases, or company systems without modifying Titan core

Do not use MCP when:
- a built-in Titan tool already solves the job well
...
- Titan remains the agent
- MCP servers contribute tools
- Titan discovers those tools at startup or reload time
...
If you installed Titan with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Titan banner/status should show MCP integration when configured
- ask Titan what tools it has available
...
## WSL2: bridge Titan in WSL to Windows Chrome

This is the practical setup when:

- Titan runs inside WSL2
...
In this setup, Titan does **not** connect to Chrome directly. Instead:

- Titan runs in WSL
- Titan starts a local stdio MCP server
...
```text
Titan (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Titan stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Titan core browser transport
...
```bash
Titan mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
Titan mcp test chrome-devtools-win
```

Then start a fresh Titan session or run:
...
Once loaded, Titan can use the MCP-prefixed browser tools directly. For example:
...
If Titan runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Titan from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Titan from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Titan:
...
2. Titan-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Titan will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Titan to reason over a bounded workspace.
...
Now Titan can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Titan core.
...
Because Titan now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
