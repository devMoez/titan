...
description: "Sign in with your SuperGrok subscription to use Grok models in Titan Agent — no API key required"
---

# xAI Grok OAuth (SuperGrok Subscription)

Titan Agent supports xAI Grok through a browser-based OAuth login flow against [accounts.x.ai](https://accounts.x.ai), using your existing **SuperGrok subscription**. No `XAI_API_KEY` is required — log in once and Titan automatically refreshes your session in the background.
...
The same OAuth bearer token is also reused by every direct-to-xAI surface in Titan — TTS, image generation, video generation, and transcription — so a single login covers all four.
...
# → Titan opens your browser to accounts.x.ai
...
After the first login, credentials are stored under `~/.Titan/auth.json` and refreshed automatically before they expire.
...
```bash
Titan auth add xai-oauth
```

### Remote / headless sessions

On servers, containers, or SSH sessions where no browser is available, Titan detects the remote environment and prints the authorization URL instead of opening a browser.
...
1. Titan opens your browser to `accounts.x.ai`.
2. You sign in (or confirm your existing session) and approve access.
3. xAI redirects back to Titan and the tokens are saved to `~/.Titan/auth.json`.
4. From then on, Titan refreshes the access token in the background — you stay signed in until you `Titan auth remove xai-oauth` or revoke access from your xAI account settings.
...
```bash
Titan config set model.default grok-4.3
Titan config set model.provider xai-oauth
```

## Configuration Reference

After login, `~/.Titan/config.yaml` will contain:
...
```bash
Titan --provider xai-oauth        # canonical
Titan --provider grok-oauth       # alias
Titan --provider x-ai-oauth       # alias
Titan --provider xai-grok-oauth   # alias
```
...
The `video_gen` toolset is disabled by default. Enable it in `Titan tools` → `🎬 Video Generation` (press space) before the agent can call `video_generate`. Otherwise the agent may fall back to the bundled ComfyUI skill, which is also tagged for video generation.
:::

:::note X search is off by default
The `x_search` toolset is disabled by default. Enable it in `Titan tools` → `🐦 X (Twitter) Search` (press space) before the agent can call `x_search`. The tool routes through xAI's built-in `x_search` Responses API — it works with **either** your SuperGrok OAuth login or a paid `XAI_API_KEY`, and prefers OAuth when both are configured (uses your subscription quota instead of API spend). The tool schema is hidden from the model when no xAI credentials are configured, regardless of whether the toolset is enabled.
:::
...
| `Titan_INFERENCE_PROVIDER` | Force the active provider at runtime, e.g. `Titan_INFERENCE_PROVIDER=xai-oauth Titan`. |

## Troubleshooting

### Token expired — not re-logging in automatically

Titan refreshes the token before each session and again reactively on a 401. If refresh fails with `invalid_grant` (the refresh token was revoked, or the account was rotated), Titan surfaces a typed re-auth message instead of crashing.

**Fix:** run `Titan auth add xai-oauth` again to start a fresh login.

### Authorization timed out

The loopback listener has a finite expiry window (default 180 s). If you don't approve the login in time, Titan raises a timeout error.

**Fix:** re-run `Titan auth add xai-oauth` (or `Titan model`). The flow starts fresh.

### State mismatch (possible CSRF)

Titan detected that the `state` value returned by the authorization server doesn't match what it sent.
...
On SSH or container sessions Titan prints the authorization URL instead of opening a browser. The loopback callback listener still binds `127.0.0.1:56121` on the remote host — your laptop's browser can't reach it without an SSH local-forward:
...
**Fix:** run `Titan model` and pick the xAI Grok OAuth provider, or run `Titan auth add xai-oauth`.
...
```bash
Titan auth logout xai-oauth
```

This clears both the singleton OAuth entry in `auth.json` and any credential-pool rows for `xai-oauth`. Use `Titan auth remove xai-oauth <index|id|label>` if you only want to drop a single pool entry (run `Titan auth list xai-oauth` to see them).

## See Also

- [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md) — required reading if Titan is on a different machine than your browser
