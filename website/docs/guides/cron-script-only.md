...
Titan calls this **no-agent mode**. It's the cron system minus the LLM.
...
- **Bash or Python.** `.sh` / `.bash` files run under `/bin/bash`; any other extension runs under the current Python interpreter. Anything in `~/.Titan/scripts/` is accepted.
...
The real win of no-agent mode is that the agent itself can set up the watchdog for you — no editor, no shell, no remembering CLI flags. You describe what you want, Titan writes the script, schedules it, and tells you when it'll fire.

### Example transcript

> **You:** ping me on telegram if RAM is over 85% every 5 minutes
>
> **Titan:** *(writes `~/.Titan/scripts/memory-watchdog.sh`, then calls `cronjob(...)` with `no_agent=true`)*
>
...
write_file(
    path="~/.Titan/scripts/memory-watchdog.sh",
...
>
> **Titan:** *(calls `cronjob(action='pause', job_id='abc123')`)*
>
> Paused. Resume with "turn it back on" or via `Titan cron resume abc123`.

> **You:** change it to every 15 minutes
>
> **Titan:** *(calls `cronjob(action='update', job_id='abc123', schedule='every 15m')`)*
...
```bash
# 1. Write your script
cat > ~/.Titan/scripts/memory-watchdog.sh <<'EOF'
#!/usr/bin/env bash
...
EOF
chmod +x ~/.Titan/scripts/memory-watchdog.sh
...
# 3. Verify
Titan cron list
Titan cron run <job_id>    # fire it once to test
```
...
Scripts must live in `~/.Titan/scripts/`. This is enforced at both job-creation time and run time — absolute paths, `~/` expansion, and path-traversal patterns (`../`) are rejected. The same directory is shared with the pre-check script gate used by LLM jobs.
...
```bash
Titan cron create "every 5m"        # interval
Titan cron create "every 2h"
Titan cron create "0 9 * * *"       # standard cron: 9am daily
Titan cron create "30m"             # one-shot: run once in 30 minutes
```
...
--deliver local                          # just save to ~/.Titan/cron/output/
```

No running gateway is required at script-run time for bot-token platforms (Telegram, Discord, Slack, Signal, SMS, WhatsApp) — the tool calls each platform's REST endpoint directly using the credentials already in `~/.Titan/.env` / `~/.Titan/config.yaml`.

## Editing and Lifecycle

```bash
Titan cron list                                    # see all jobs
Titan cron pause <job_id>                          # stop firing, keep definition
Titan cron resume <job_id>
Titan cron edit <job_id> --schedule "every 10m"    # adjust cadence
Titan cron edit <job_id> --agent                   # flip to LLM mode
Titan cron edit <job_id> --no-agent --script …     # flip back
Titan cron remove <job_id>                         # delete it
```
...
```bash
cat > ~/.Titan/scripts/disk-alert.sh <<'EOF'
#!/usr/bin/env bash
...
'
EOF
chmod +x ~/.Titan/scripts/disk-alert.sh

Titan cron create "*/15 * * * *" \
  --no-agent \
...
|----------|-----------|-------------|
| `cronjob --no-agent` (this page) | Your script on Titan' schedule | Recurring watchdogs / alerts / metrics that don't need reasoning |
| `cronjob` (default, LLM) | Agent with optional pre-check script | When the message content requires reasoning over data |
| OS cron + `curl` to a [webhook subscription](/docs/user-guide/messaging/webhooks) | Your script on the OS schedule | When Titan might be unhealthy (the thing you're monitoring) |

For critical system-health watchdogs that must fire *even when the gateway is down*, use OS-level cron with a plain `curl` to a Titan webhook subscription (or any external alerting endpoint) — those run as independent OS processes and don't depend on Titan being up. The in-gateway scheduler is the right choice when the thing being monitored is external.
