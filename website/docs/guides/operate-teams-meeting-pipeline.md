...
```bash
Titan teams-pipeline token-health
Titan teams-pipeline token-health --force-refresh
```
...
```bash
Titan teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
Titan teams-pipeline maintain-subscriptions
Titan teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Titan cron (recommended if you already run the Titan gateway)

Titan ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.Titan/scripts/`. First create the script:

```bash
mkdir -p ~/.Titan/scripts
cat > ~/.Titan/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec Titan teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.Titan/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
Titan cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/Titan-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Titan Teams pipeline subscription maintenance
...
ExecStart=/usr/local/bin/Titan teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/Titan-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Titan Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now Titan-teams-pipeline-maintain.timer
systemctl list-timers Titan-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/Titan teams-pipeline maintain-subscriptions >> /var/log/Titan/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.Titan/.env` at the top of a wrapper script that crontab calls.
...
```bash
Titan teams-pipeline subscriptions   # should show expirationDateTime advanced
Titan teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
Titan teams-pipeline list
Titan teams-pipeline list --status failed
Titan teams-pipeline show <job-id>
```

### Replay a stored job

```bash
Titan teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
Titan teams-pipeline fetch --meeting-id <meeting-id>
Titan teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `Titan teams-pipeline maintain-subscriptions --dry-run`
- inspect `Titan teams-pipeline list --status failed`
...
- run `Titan teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `Titan teams-pipeline run`
...
- [ ] `Titan teams-pipeline validate` returns an OK snapshot
- [ ] `Titan teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Titan cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
