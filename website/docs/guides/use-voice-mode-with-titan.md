---
sidebar_position: 8
title: "Use Voice Mode with Titan"
description: "A practical guide to setting up and using Titan voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Titan
...
- you want Titan sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Titan.
...
## Step 1: make sure normal Titan works first
...
Titan supports both local and cloud speech stacks.
...
### If you use `Titan setup`

If you choose NeuTTS in the setup wizard, Titan checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Titan transcribes and responds
...
- asking Titan to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Titan loop.

## Tuning CLI behavior

### Silence threshold

If Titan starts/stops too aggressively, tune:
...
Titan stays a normal chat bot, but can speak replies.
...
- you want Titan to function like a portable research or ops assistant
...
Titan joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Titan detects speech boundaries
- transcripts are posted in the associated text channel
- Titan responds in text and audio
...
1. get text Titan working
2. install `titan-agent[voice]`
