---
sidebar_position: 7
title: "Use SOUL.md with Titan"
description: "How to use SOUL.md to shape Titan Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Titan

`SOUL.md` is the **primary identity** for your Titan instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Titan to feel like the same assistant every time you talk to it — or if you want to replace the Titan persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Titan should be
- what Titan should avoid stylistically
- how Titan should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Titan is and how Titan speaks
...
Titan now uses only the global SOUL file for the current instance:

```text
~/.Titan/SOUL.md
```

If you run Titan with a custom home directory, it becomes:
...
Titan automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Titan does not overwrite it
- if the file exists but is empty, Titan adds nothing from it to the prompt

## How Titan uses it

When Titan starts a session, it reads `SOUL.md` from `Titan_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Titan feels.
...
Titan already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Titan should sound.

# Avoid
What Titan should not do.

# Defaults
How Titan should behave when ambiguity appears.
```
...
Then restart Titan or start a new session.
...
### I edited SOUL.md but Titan still sounds the same

Check:
- you edited `~/.Titan/SOUL.md` or `$Titan_HOME/SOUL.md`
...
### Titan is ignoring parts of my SOUL.md
