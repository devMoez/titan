...
This page covers all commands related to [Titan profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `Titan profile` without a subcommand shows help.
...
```bash
Titan profile use <name>
```

Sets `<name>` as the active profile. All subsequent `Titan` commands (without `-p`) will use this profile.
...
Titan profile use default
```

## `Titan profile create`

```bash
Titan profile create <name> [options]
```
...
Titan profile create mybot

# Clone config only from current profile
Titan profile create work --clone

# Clone everything from current profile
Titan profile create backup --clone-all

# Clone config from a specific profile
Titan profile create work2 --clone --clone-from work
```

## `Titan profile delete`

```bash
Titan profile delete <name> [options]
```
...
```bash
Titan profile delete mybot
Titan profile delete mybot --yes
```
...
```bash
Titan profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Titan home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ Titan profile show work
Profile: work
Path:    ~/.Titan/profiles/work
...
```bash
Titan profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Titan installation.
...
```bash
Titan profile alias work
# Creates/updates ~/.local/bin/work

Titan profile alias work --name mywork
# Creates ~/.local/bin/mywork

Titan profile alias work --remove
# Removes the wrapper script
```

## `Titan profile rename`

```bash
Titan profile rename <old-name> <new-name>
```
...
```bash
Titan profile rename mybot assistant
# ~/.Titan/profiles/mybot → ~/.Titan/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `Titan profile export`

```bash
Titan profile export <name> [options]
```
...
```bash
Titan profile export work
# Creates work.tar.gz in the current directory

Titan profile export work -o ./work-2026-03-29.tar.gz
```

## `Titan profile import`

```bash
Titan profile import <archive> [options]
```
...
```bash
Titan profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

Titan profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`Titan profile export` / `import` are still the right commands for
...
### `Titan profile install`

```bash
Titan profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `Titan -p telemetry`). |
...
Titan profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
Titan profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
Titan profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
Titan profile install ./telemetry/
```

### `Titan profile update`

```bash
Titan profile update <name> [--force-config] [--yes]
```
...
```bash
Titan profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Titan version, author, env var requirements, the source URL/path, and
...
`Titan profile list` also shows the distribution name and version in a
`Distribution` column, and `Titan profile show <name>` / `delete <name>`
...
Titan profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
Titan profile install https://github.com/your-org/internal-assistant.git
```
...
`Titan_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Titan version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Titan can clone from.
3. Tell recipients to run `Titan profile install <your-repo-url>`.
...
## `Titan -p` / `Titan --profile`

```bash
Titan -p <name> <command> [options]
Titan --profile <name> <command> [options]
```

Global flag to run any Titan command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
Titan -p work chat -q "Check the server status"
Titan --profile dev gateway start
Titan -p personal skills list
Titan -p work config edit
```

## `Titan completion`

```bash
Titan completion <shell>
```
...
- `Titan profile <TAB>` — subcommands (list, use, create, etc.)
- `Titan profile use <TAB>` — profile names
- `Titan -p <TAB>` — profile names
