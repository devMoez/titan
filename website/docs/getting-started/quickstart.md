...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Titan Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Titan setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Titan Agent Tutorials & Use Cases](https://www.youtube.com/channel/UCqB1bhMwGsW-yefBxYwFCCg) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Titan Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Titan for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Titan working on my machine | `Titan setup` | Run a real chat and verify it responds |
| I already know my provider | `Titan model` | Save the config, then start chatting |
| I want a bot or always-on setup | `Titan gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `Titan model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `Titan model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Titan cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Titan Agent

**Option A — pip (simplest):**

```bash
pip install titan-agent
Titan postinstall     # optional: installs Node.js, browser, ripgrep, ffmpeg + runs setup
```
...
The single most important setup step. Use `Titan model` to walk through the choice interactively:
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `Titan model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `Titan model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `Titan model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax-M2.7 via browser OAuth — no API key needed | `Titan model` → MiniMax (OAuth) |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `Titan model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `Titan model` (requires `copilot` CLI + `copilot login`) |
...
:::tip
You can switch providers at any time with `Titan model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Titan separates secrets from normal config:

- **Secrets and tokens** → `~/.Titan/.env`
- **Non-secret settings** → `~/.Titan/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
Titan config set model anthropic/claude-opus-4.6
Titan config set terminal.backend docker
Titan config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
Titan            # classic CLI
Titan --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Titan ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `Titan` vs `Titan --tui`.
:::
...
```bash
Titan --continue    # Resume the most recent session
Titan -c            # Short form
```
...
```bash
Titan gateway setup    # Interactive platform configuration
```
...
- `Titan tools` — tune tool access per platform
- `Titan skills` — browse and install reusable workflows
...
```bash
Titan config set terminal.backend docker    # Docker isolation
Titan config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
# From the Titan install directory (the curl installer placed it at
# ~/.Titan/titan-agent on Linux/macOS or %LOCALAPPDATA%\Titan\titan-agent on Windows):
...
```bash
Titan skills search kubernetes
...
(If you installed without `[all]`, run `cd ~/.Titan/titan-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Titan opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `Titan model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `Titan gateway setup` and check `Titan gateway status` |
| `Titan --continue` can't find old session | Switched profiles or session never saved | Check `Titan sessions list` and confirm you're in the right profile |
...
| `Titan doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `Titan model`
3. `Titan setup`
4. `Titan sessions list`
5. `Titan --continue`
6. `Titan gateway status`
...
| `Titan tools` | Configure which tools are enabled per platform |
| `Titan setup` | Full setup wizard (configures everything at once) |
| `Titan doctor` | Diagnose issues |
| `Titan update` | Update to latest version |
| `Titan gateway` | Start the messaging gateway |
| `Titan --continue` | Resume last session |
