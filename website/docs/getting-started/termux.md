...
description: "Run Titan Agent directly on an Android phone with Termux"
---

# Titan on Android with Termux

This is the tested path for running Titan Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Titan from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Titan now ships a Termux-aware installer path:
...
- links `Titan` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `Titan` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/Titan" "$PREFIX/bin/Titan"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `Titan` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.Titan/.env`.
...
### `Titan doctor` says ripgrep or Node is missing
