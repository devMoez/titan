...
def _load_supermemory_config(Titan_home: str) -> dict:
    config = _default_config()
    config_path = Path(Titan_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, Titan_home: str) -> None:
    config_path = Path(Titan_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `Titan memory setup`.
        # All other options are documented for $Titan_HOME/supermemory.json
    ...
    def save_config(self, values, Titan_home):
        ...
        _save_supermemory_config(sanitized, Titan_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from Titan_constants import get_Titan_home
        self._Titan_home = kwargs.get("Titan_home") or str(get_Titan_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._Titan_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "Titan", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "Titan_memory", "target": target, "type": "explicit_memory"},
