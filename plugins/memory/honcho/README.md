...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/Titan>
...
```bash
Titan honcho setup    # full interactive wizard (cloud or local)
Titan memory setup    # generic picker, also works
```

Or manually:
```bash
Titan config set memory.provider honcho
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$Titan_HOME/honcho.json` | Profile-local (isolated Titan instances) |
| 2 | `~/.Titan/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Titan profile: `Titan` (default) or `Titan.<profile>`.
...
| 4 | `per-session` strategy | Titan session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`titan-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`Titan`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-titan-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening Titan in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Titan session ID (timestamp + hex). Every `Titan` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Titan profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`Titan`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Titan profile: `Titan` (default) or `Titan.<profile>` (e.g. `Titan -p coder` → host key `Titan.coder`).
...
| `Titan_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `Titan honcho setup` | Full interactive setup wizard |
| `Titan honcho status` | Show resolved config for active profile |
| `Titan honcho enable` / `disable` | Toggle Honcho for active profile |
| `Titan honcho mode <mode>` | Change recall or observation mode |
| `Titan honcho peer --user <name>` | Update user peer name |
| `Titan honcho peer --ai <name>` | Update AI peer name |
| `Titan honcho tokens --context <N>` | Set context token budget |
| `Titan honcho tokens --dialectic <N>` | Set dialectic max chars |
| `Titan honcho map <name>` | Map current directory to a session name |
| `Titan honcho sync` | Create host blocks for all Titan profiles |
