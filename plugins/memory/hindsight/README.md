...
Titan spins up a local Hindsight daemon with built-in PostgreSQL. Requires an LLM API key for memory extraction and synthesis. The daemon starts automatically in the background on first use and stops after 5 minutes of inactivity.
...
Daemon startup logs: `~/.Titan/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p Titan ui start
```
...
Config file: `~/.Titan/hindsight/config.json`
...
| Key | Default | Description |
|-----|---------|-------------|
| `bank_id` | `Titan` | Memory bank name (static fallback used when `bank_id_template` is unset or resolves empty) |
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `Titan-{profile}` isolates memory per active Titan profile. Empty placeholders collapse cleanly (e.g. `Titan-{user}` with no user becomes `Titan`). |
...
The LLM API key is stored in `~/.Titan/.env` as `HINDSIGHT_LLM_API_KEY`.
