"""SimpleX Chat platform adapter (Titan plugin).
...
This adapter ships as a Titan platform plugin under
``plugins/platforms/simplex/``. The Titan plugin loader scans the
...
discoverable and `Titan setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Titan — no
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"Titan-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``Titan setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / home channel.
    Writes to ``~/.Titan/.env`` via ``Titan_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("Titan_cli.config not available; set SIMPLEX_* vars manually in ~/.Titan/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `Titan gateway status` without
