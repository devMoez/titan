"""
Microsoft Teams platform adapter for Titan Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Titan"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        Titan_action = data.get("Titan_action", "")
        session_key = data.get("session_key", "")

        if not Titan_action or not session_key:
        ...
        choice = choice_map.get(Titan_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="Titan_approve",
                    data={**btn_data_base, "Titan_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="Titan_approve",
                    data={**btn_data_base, "Titan_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="Titan_approve",
                    data={**btn_data_base, "Titan_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="Titan_approve",
                    data={**btn_data_base, "Titan_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Titan\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.Titan/.env")
