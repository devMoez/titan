"""
LINE Messaging API platform adapter for Titan Agent.
...
to Titan Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``Titan_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            Titan_home = Path.home().joinpath(".Titan").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``Titan status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``Titan status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``Titan setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.Titan/.env`` via ``Titan_cli.config``.
    """
    print()
    ...
    try:
        from Titan_cli.config import get_env_var, set_env_var
    except ImportError:
        print("Titan_cli.config not available; set LINE_* vars manually in ~/.Titan/.env")
        return
