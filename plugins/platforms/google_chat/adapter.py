...
# ``model_tools`` import time, so ``Titan status``, ``Titan chat``, etc.
...
# ``Titan_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Titan is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a Titan MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${Titan_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _Titan_home = _get_Titan_home()
        except (ModuleNotFoundError, ImportError):
            _Titan_home = _Path.home() / ".Titan"
        self._thread_count_store = _ThreadCountStore(
            _Titan_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Titan is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("Titan_HOME", str(_Path.home() / ".Titan"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${Titan_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a Titan MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Titan is thinking…' marker message.
        ...
        "Titan is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Titan is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Titan is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Titan is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Titan is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``Titan setup``.

    The setup wizard at ``Titan_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.Titan/.env`` so the next
    gateway restart picks them up.
    """
    from Titan_cli.cli_output import (
        print_info,
        print_success,
        print_warning,
        prompt,
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. Titan-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.Titan/.env")
    print_info("Restart the gateway: Titan gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``Titan cron`` running as a
    separate process from ``Titan gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Titan plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'titan-agent[google_chat]'",
        ...
        # the "Titan is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Titan is thinking…' marker message "
