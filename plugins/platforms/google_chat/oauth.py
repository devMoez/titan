...
- Per-user tokens (keyed by sender email):
    ``${Titan_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${Titan_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${Titan_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${Titan_HOME}/google_chat_user_oauth_pending.json``
- Shared OAuth client (one per host):
    ``${Titan_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's Titan_HOME helper so the token follows the user's
# profile (e.g. tests can override via Titan_HOME=/tmp/...).
try:
    from Titan_constants import display_Titan_home, get_Titan_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where Titan_constants isn't importable
...
def _Titan_home() -> Path:
    """Resolve Titan_HOME at call time (NOT module import).

    Tests and ``Titan_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.Titan/google_chat_user_tokens/`` trivial.
...
def _legacy_token_path() -> Path:
    return _Titan_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _Titan_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _Titan_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _Titan_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _Titan_home() / "google_chat_user_client_secret.json"
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Install titan-agent[google_chat]."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("  pip install 'titan-agent[google_chat]'")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into Titan_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_Titan_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_Titan_home()}/google_chat_user_token.json"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for Titan (native attachment delivery)"
    )
