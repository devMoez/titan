/**
 * Titan Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.Titan/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__Titan_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__Titan_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "Titan-kanban-dot-triage",
    todo: "Titan-kanban-dot-todo",
    ready: "Titan-kanban-dot-ready",
    running: "Titan-kanban-dot-running",
    blocked: "Titan-kanban-dot-blocked",
    done: "Titan-kanban-dot-done",
    archived: "Titan-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-Titan-task";
  ...
  const DOCS_URL = "https://titan-agent.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://titan-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "Titan.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="Titan-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "Titan-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "Titan-kanban-md",
  ...
  // standard `drop` event and our `Titan-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("Titan-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (col !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("Titan-kanban-column--drop");
          if (col) col.classList.add("Titan-kanban-column--drop");
          lastTarget = col;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("Titan-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          lastTarget.dispatchEvent(new CustomEvent("Titan-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        const token = window.__Titan_SESSION_TOKEN__ || "";
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "Titan-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "Titan-kanban-attention",
        "Titan-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "Titan-kanban-attention-bar" },
        h("span", { className: "Titan-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "Titan-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "Titan-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "Titan-kanban-attention-row",
                  "Titan-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "Titan-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "Titan-kanban-attention-row-id" }, task.id),
                h("span", { className: "Titan-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "Titan-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "Titan-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "Titan-kanban-diag-action-btn",
      action.suggested ? "Titan-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " Titan-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".Titan-kanban-drawer-comment-row input, .Titan-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "Titan-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("Titan-kanban-diag", sevClass) },
      h("div", { className: "Titan-kanban-diag-header" },
        h("span", { className: "Titan-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "Titan-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "Titan-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "Titan-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "Titan-kanban-diag-data-row" },
                  h("span", { className: "Titan-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "Titan-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "Titan-kanban-diag-data-row" },
                h("span", { className: "Titan-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "Titan-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "Titan-kanban-diag-reassign-row" },
            h("span", { className: "Titan-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "Titan-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "Titan-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "Titan-kanban-diag-msg",
              msg.ok ? "Titan-kanban-diag-msg--ok" : "Titan-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "Titan-kanban-section" },
      h("div", { className: "Titan-kanban-section-head-row" },
        h("span", { className: "Titan-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "Titan-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "Titan-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "Titan-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "Titan-kanban-docs-link",
      title: "Open Titan Kanban docs in a new tab",
      "aria-label": "Titan Kanban documentation",
    }, "?");
  }
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "Titan-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "Titan-kanban-boardswitcher" },
      h("div", { className: "Titan-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "Titan-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "Titan-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "Titan-kanban-dialog-title" },
        ...
        h("div", { className: "Titan-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Titan profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "Titan-kanban-bulk" },
      h("span", { className: "Titan-kanban-bulk-count" },
      ...
      h("div", { className: "Titan-kanban-bulk-priority",
      ...
      h("div", { className: "Titan-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Titan profile. Pick a profile (or unassign) and click Apply." },
        h(Select, {
          value: assignee,
      ...
      h("label", { className: "Titan-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h("input", {
          type: "checkbox",
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".Titan-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "Titan-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("Titan-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("Titan-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "Titan-kanban-column",
        dragOver ? "Titan-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "Titan-kanban-column-header",
                 title: colHelp || "" },
        h("input", {
          type: "checkbox",
          className: "Titan-kanban-col-check",
        ...
        h("span", { className: cn("Titan-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "Titan-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "Titan-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "Titan-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "Titan-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "Titan-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "Titan-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "Titan-kanban-lane" },
                  h("div", { className: "Titan-kanban-lane-head" },
                    h("span", { className: "Titan-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "Titan-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "Titan-kanban-card--stale-red";
    if (age >= tier.amber) return "Titan-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".Titan-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "Titan-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "Titan-kanban-card",
        props.selected ? "Titan-kanban-card--selected" : "",
        props.failed ? "Titan-kanban-card--failed" : "",
        props.draggingSource ? "Titan-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "Titan-kanban-card-content" },
          h("div", { className: "Titan-kanban-card-row" },
            h("label", {
              className: "Titan-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h("input", {
                type: "checkbox",
                className: "Titan-kanban-card-check",
            ...
            h("span", { className: "Titan-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or Titan kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "Titan-kanban-warning-badge",
                    "Titan-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "Titan-kanban-needs-assignee",
          ...
          h("div", { className: "Titan-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "Titan-kanban-card-row Titan-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "Titan-kanban-assignee",
                            title: `Assigned to Titan profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "Titan-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "Titan-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "Titan-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "Titan-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Titan profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Titan profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "Titan-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "Titan-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "Titan-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "Titan-kanban-drawer-close",
        ...
        data ? h("div", { className: "Titan-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "Titan-kanban-drawer-body" },
      h("div", { className: "Titan-kanban-drawer-title" },
        h("span", { className: cn("Titan-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "Titan-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "Titan-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "Titan-kanban-section" },
        h("div", { className: "Titan-kanban-section-head" }, tx(i18n, "result", "Result")),
        h(MarkdownBlock, { source: t.result, enabled: props.renderMarkdown }),
      ) : null,
      h("div", { className: "Titan-kanban-section" },
        h("div", { className: "Titan-kanban-section-head" },
          `${tx(i18n, "comments", "Comments")} (${comments.length})`),
        comments.length === 0
          ? h("div", { className: "text-xs text-muted-foreground" },
              tx(i18n, "noComments", "— no comments —"))
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "Titan-kanban-comment" },
                h("div", { className: "Titan-kanban-comment-head" },
                  h("span", { className: "Titan-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "Titan-kanban-comment-ago" },
      ...
      h("div", { className: "Titan-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "Titan-kanban-event",
              isDiag ? "Titan-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "Titan-kanban-event-header" },
                  h("span", { className: "Titan-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "Titan-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "Titan-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "Titan-kanban-event-header-plain" },
                  h("span", { className: "Titan-kanban-event-kind" }, e.kind),
                  h("span", { className: "Titan-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "Titan-kanban-event-phantom-row" },
                  h("span", { className: "Titan-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "Titan-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "Titan-kanban-event-payload" },
    ...
    return h("div", { className: "Titan-kanban-section" },
      h("div", { className: "Titan-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              ...
              className: "Titan-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `Titan-kanban-run--${r.outcome || r.status || "ended"}`
          : "Titan-kanban-run--active";
        return h("div", { key: r.id, className: cn("Titan-kanban-run", outcomeClass) },
          h("div", { className: "Titan-kanban-run-head" },
            h("span", { className: "Titan-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "Titan-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "Titan-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "Titan-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "Titan-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "Titan-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "Titan-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "Titan-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "Titan-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "Titan-kanban-pre Titan-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "Titan-kanban-meta-row" },
      h("span", { className: "Titan-kanban-meta-label" }, props.label),
      h("span", { className: "Titan-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "Titan-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "Titan-kanban-meta-row" },
        h("span", { className: "Titan-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "Titan-kanban-meta-value Titan-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "Titan-kanban-meta-row" },
        h("span", { className: "Titan-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "Titan-kanban-meta-value Titan-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "Titan-kanban-section" },
      h("div", { className: "Titan-kanban-section-head-row" },
        h("span", { className: "Titan-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "Titan-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "Titan-kanban-section" },
      h("div", { className: "Titan-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "Titan-kanban-deps-row" },
        h("span", { className: "Titan-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "Titan-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "Titan-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "Titan-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "Titan-kanban-dep-chip-x",
      ...
      h("div", { className: "Titan-kanban-deps-row" },
        h("span", { className: "Titan-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "Titan-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "Titan-kanban-actions" },
        specifyButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "Titan-kanban-section" },
      ...
      h("div", { className: "Titan-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "Titan-kanban-home-sub Titan-kanban-home-sub--on"
              : "Titan-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__Titan_PLUGINS__ && typeof window.__Titan_PLUGINS__.register === "function") {
    window.__Titan_PLUGINS__.register("kanban", KanbanPage);
  }
})();
