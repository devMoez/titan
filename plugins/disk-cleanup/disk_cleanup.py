"""disk_cleanup — ephemeral file cleanup for Titan Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under Titan_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly Titan_HOME and /tmp/Titan-*
Never touches: ~/.Titan/logs/ or any system directory.
"""
...
try:
    ...
    def get_Titan_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("Titan_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".Titan").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$Titan_HOME/logs/``."""
    return get_Titan_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$Titan_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under Titan_HOME or ``/tmp/Titan-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    Titan_home = get_Titan_home()
    try:
        path.resolve().relative_to(Titan_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/Titan-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("Titan-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside Titan_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under Titan_HOME (but leave Titan_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "titan-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(Titan_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == Titan_home:
                continue
            try:
                rel_parts = dirpath.relative_to(Titan_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    Titan_home = get_Titan_home()
    try:
        rel = path.resolve().relative_to(Titan_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "titan-agent",
        }:
            return None
        ...
        # Path isn't under Titan_HOME (e.g. /tmp/Titan-*) — fall through.
        pass
