...
Auto-tracks and cleans up ephemeral files created during Titan Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$Titan_HOME` and `/tmp/Titan-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `Titan_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `Titan_HOME` or `/tmp/Titan-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$Titan_HOME/disk-cleanup/` is itself excluded
- `$Titan_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
