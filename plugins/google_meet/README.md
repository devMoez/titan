...
Let the Titan agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where Titan runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `Titan meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `Titan meet` CLI |
...
| `cli.py` | `Titan meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$Titan_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `Titan meet node {run,list,approve,remove,status,ping}` |
...
Titan meet install                                      # pip + Chromium
Titan meet setup                                        # preflight
Titan meet auth                                         # optional
Titan meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
Titan meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.Titan/.env
Titan meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
Titan meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
Titan meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.Titan/.env
Titan meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, Titan will **not** switch your system audio input automatically — the
...
Titan plugins enable google_meet
Titan meet node run --display-name my-mac --host 0.0.0.0 --port 18789
