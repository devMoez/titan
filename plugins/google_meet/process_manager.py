...
session-scoped state file under ``$Titan_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $Titan_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_Titan_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this Titan install, leave it first —
    ...
    env = os.environ.copy()
    env["Titan_MEET_URL"] = url
    env["Titan_MEET_OUT_DIR"] = str(out)
    env["Titan_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["Titan_MEET_HEADED"] = "1"
    if auth_state:
        env["Titan_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["Titan_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if Titan_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["Titan_MEET_MODE"] = mode
    if realtime_model:
        env["Titan_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["Titan_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["Titan_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["Titan_MEET_REALTIME_KEY"] = realtime_api_key
