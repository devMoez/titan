# Titan Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: Titan-achievements plugin
...
### `GET /api/plugins/Titan-achievements/achievements`
...
### `POST /api/plugins/Titan-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/Titan-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/Titan-achievements/overview`
...
- Persist plugin data under:
  - `~/.Titan/plugins/Titan-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.Titan/plugins/Titan-achievements/scan_checkpoint.json`
...
- `~/.Titan/plugins/Titan-achievements/`
