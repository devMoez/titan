...
class TestToolsetConsistency:
    ...
    def test_Titan_platforms_share_core_tools(self):
        """All Titan-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on Titan-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["Titan-cli", "Titan-telegram", "Titan-discord", "Titan-whatsapp", "Titan-slack", "Titan-signal", "Titan-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_Titan_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("Titan-whatsapp")

    def test_Titan_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("Titan-api-server")
