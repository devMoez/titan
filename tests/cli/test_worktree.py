...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("Titan/Titan-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned Titan/* and pr-* branches."""

    def test_prunes_orphaned_Titan_branch(self, git_repo):
        """Titan/Titan-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "Titan/Titan-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "Titan/Titan-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "Titan/Titan-deadbeef" in result.stdout
        ...
        assert "Titan/Titan-deadbeef" in orphaned
        ...
        assert "Titan/Titan-deadbeef" not in result.stdout
