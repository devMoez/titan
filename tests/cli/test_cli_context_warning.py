...
def _isolate(tmp_path, monkeypatch):
    """Isolate Titan_HOME so tests don't touch real config."""
    home = tmp_path / ".Titan"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal TitanCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = TitanCLI.__new__(TitanCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["Titan-core"]
        obj.compact = False
