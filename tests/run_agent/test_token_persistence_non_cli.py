...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    Titan_state = ModuleType("Titan_state")
    Titan_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "Titan_state", Titan_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Titan"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Titan"
