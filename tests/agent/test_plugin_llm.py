...
""",
        ...
        from Titan_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        Titan_home = tmp_path / ".Titan"
        Titan_home.mkdir()
        (Titan_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("Titan_HOME", str(Titan_home))
        from Titan_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from Titan_cli.plugins import PluginContext, PluginManifest, PluginManager
