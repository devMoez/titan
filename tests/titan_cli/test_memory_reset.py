"""Tests for the `Titan memory reset` CLI command.
...
- Profile-scoped reset (uses Titan_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake Titan_HOME with memory files."""
    Titan_home = tmp_path / ".Titan"
    memories = Titan_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("Titan_HOME", str(Titan_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nTitan repo is at ~/.Titan/titan-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `Titan memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        Titan_home = tmp_path / ".Titan"
        (Titan_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("Titan_HOME", str(Titan_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's Titan_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        Titan_home = tmp_path / ".Titan"
        Titan_home.mkdir(parents=True)
        # No memories dir
        monkeypatch.setenv("Titan_HOME", str(Titan_home))

        # The memories dir won't exist; get_Titan_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
