"""Tests for Titan backup and import commands."""
...
def _make_Titan_tree(root: Path) -> None:
    """Create a realistic ~/.Titan directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    (root / "memory_store.db").write_bytes(b"fake-sqlite")
    (root / "Titan_state.db").write_bytes(b"fake-state")
    ...
    # titan-agent repo (should be EXCLUDED)
    (root / "titan-agent").mkdir(exist_ok=True)
    (root / "titan-agent" / "run_agent.py").write_text("# big file\n")
    (root / "titan-agent" / ".git").mkdir()
    (root / "titan-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_titan_agent(self):
        from Titan_cli.backup import _should_exclude
        assert _should_exclude(Path("titan-agent/run_agent.py"))
        assert _should_exclude(Path("titan-agent/.git/HEAD"))
...
class TestBackup:
    def test_creates_zip(self, tmp_path, monkeypatch):
        ...
        Titan_home.mkdir()
        _make_Titan_tree(Titan_home)

        monkeypatch.setenv("Titan_HOME", str(Titan_home))
        # get_default_Titan_root needs this
    ...
    def test_excludes_titan_agent(self, tmp_path, monkeypatch):
        """Backup does NOT include titan-agent/ directory."""
        Titan_home = tmp_path / ".Titan"
        Titan_home.mkdir()
        _make_Titan_tree(Titan_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "titan-agent" in n]
            assert agent_files == [], f"titan-agent files leaked into backup: {agent_files}"
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/Titan-backup-*.zip."""
        Titan_home = tmp_path / ".Titan"
        Titan_home.mkdir()
        (Titan_home / "config.yaml").write_text("model: test\n")
        ...
        zips = list(tmp_path.glob("Titan-backup-*.zip"))
        assert len(zips) == 1
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Titan backup."""
        from Titan_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only Titan_state.db (old wrong name) is rejected."""
        from Titan_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["Titan_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into Titan home."""
        Titan_home = tmp_path / ".Titan"
        Titan_home.mkdir()
        monkeypatch.setenv("Titan_HOME", str(Titan_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (Titan_home / "config.yaml").read_text() == "model:\n  provider: openrouter\n"
        assert (Titan_home / ".env").read_text() == "OPENROUTER_API_KEY=sk-test\n"
        assert (Titan_home / "skills" / "my-skill" / "SKILL.md").read_text() == "# My Skill\n"
        assert (Titan_home / "profiles" / "coder" / "config.yaml").exists()

    def test_strips_Titan_prefix(self, tmp_path, monkeypatch):
        """Import strips .Titan/ prefix if all entries share it."""
        Titan_home = tmp_path / ".Titan"
        Titan_home.mkdir()
        monkeypatch.setenv("Titan_HOME", str(Titan_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".Titan/config.yaml": "model: test\n",
            ".Titan/skills/a/SKILL.md": "# A\n",
        })
        ...
        assert (Titan_home / "config.yaml").read_text() == "model: test\n"
        assert (Titan_home / "skills" / "a" / "SKILL.md").read_text() == "# A\n"
    ...
    def test_rejects_non_Titan_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a Titan backup."""
        Titan_home = tmp_path / ".Titan"
        Titan_home.mkdir()
        monkeypatch.setenv("Titan_HOME", str(Titan_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        assert (Titan_home / "config.yaml").exists()
        # traversal file should NOT exist outside Titan home
    ...
    def test_confirmation_prompt_abort(self, tmp_path, monkeypatch):
        ...
        (Titan_home / "config.yaml").write_text("existing: true\n")
        monkeypatch.setenv("Titan_HOME", str(Titan_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (Titan_home / "config.yaml").read_text() == "existing: true\n"
    ...
    def test_force_skips_confirmation(self, tmp_path, monkeypatch):
        ...
        assert (Titan_home / "config.yaml").read_text() == "model: restored\n"
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (Titan_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".Titan"
        src_home.mkdir(parents=True)
        _make_Titan_tree(src_home)

        monkeypatch.setenv("Titan_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from Titan_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".Titan"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("Titan_HOME", str(dst_home))
        ...
        # titan-agent should NOT be present
        assert not (dst_home / "titan-agent").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from Titan_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without Titan markers fails validation."""
        import io
        from Titan_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_Titan(self):
        """Detects .Titan/ prefix wrapping all entries."""
        import io
        from Titan_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".Titan/config.yaml", "test")
            zf.writestr(".Titan/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".Titan/"
...
class TestBackupEdgeCases:
    def test_nonexistent_Titan_home(self, tmp_path, monkeypatch):
        """Backup exits when Titan home doesn't exist."""
        fake_home = tmp_path / "nonexistent" / ".Titan"
        monkeypatch.setenv("Titan_HOME", str(fake_home))
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("Titan-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_empty_Titan_home(self, tmp_path, monkeypatch):
        """Backup handles empty Titan home (no files to back up)."""
        Titan_home = tmp_path / ".Titan"
        Titan_home.mkdir()
        # Only excluded dirs, no actual files
        (Titan_home / "__pycache__").mkdir()
        (Titan_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        old_file = Titan_home / "ancient.txt"
    ...
    def test_skips_output_zip_inside_Titan(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside Titan root."""
        Titan_home = tmp_path / ".Titan"
        Titan_home.mkdir()
        (Titan_home / "config.yaml").write_text("model: test\n")
        ...
        # Output inside Titan home
        out_zip = Titan_home / "backup.zip"
        args = Namespace(output=str(out_zip))
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        Titan_home = tmp_path / ".Titan"
        Titan_home.mkdir()
        (Titan_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("Titan_HOME", str(Titan_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_keyboard_interrupt_during_confirmation(self, tmp_path, monkeypatch):
        ...
        Titan_home = tmp_path / ".Titan"
        Titan_home.mkdir()
        (Titan_home / ".env").write_text("KEY=val\n")
        monkeypatch.setenv("Titan_HOME", str(Titan_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (Titan_home / "config.yaml").exists()
        assert (Titan_home / "sessions" / "s0599.json").exists()
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert (Titan_home / "profiles" / "coder" / "config.yaml").exists()
        assert (Titan_home / "profiles" / "researcher" / "config.yaml").exists()
        ...
        assert "Titan -p coder" in coder_wrapper
    ...
    def test_import_without_profiles_module(self, tmp_path, monkeypatch):
        ...
        import Titan_cli.backup as backup_mod
        original_import = __builtins__.__import__ if hasattr(__builtins__, '__import__') else __import__

        def fake_import(name, *a, **kw):
            if name == "Titan_cli.profiles":
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from Titan_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    @pytest.fixture
    def Titan_home(self, tmp_path):
        """Create a fake Titan_HOME with critical state files."""
        home = tmp_path / ".Titan"
        home.mkdir()
    ...
    def test_creates_snapshot(self, Titan_home):
        from Titan_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(Titan_home=Titan_home)
        assert snap_id is not None
        snap_dir = Titan_home / "state-snapshots" / snap_id
        assert snap_dir.is_dir()
        assert (snap_dir / "manifest.json").exists()

    def test_label_in_id(self, Titan_home):
        from Titan_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(label="before-upgrade", Titan_home=Titan_home)
        assert "before-upgrade" in snap_id

    def test_state_db_safely_copied(self, Titan_home):
        from Titan_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(Titan_home=Titan_home)
        db_copy = Titan_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_copies_nested_files(self, Titan_home):
        from Titan_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(Titan_home=Titan_home)
        assert (Titan_home / "state-snapshots" / snap_id / "cron" / "jobs.json").exists()

    def test_missing_files_skipped(self, Titan_home):
        from Titan_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(Titan_home=Titan_home)
        with open(Titan_home / "state-snapshots" / snap_id / "manifest.json") as f:
            meta = json.load(f)
    ...
    def test_empty_home_returns_none(self, tmp_path):
        ...
        empty.mkdir()
        assert create_quick_snapshot(Titan_home=empty) is None

    def test_list_snapshots(self, Titan_home):
        from Titan_cli.backup import create_quick_snapshot, list_quick_snapshots
        id1 = create_quick_snapshot(label="first", Titan_home=Titan_home)
        id2 = create_quick_snapshot(label="second", Titan_home=Titan_home)

        snaps = list_quick_snapshots(Titan_home=Titan_home)
        assert len(snaps) == 2
        assert snaps[0]["id"] == id2  # most recent first
        assert snaps[1]["id"] == id1

    def test_list_limit(self, Titan_home):
        from Titan_cli.backup import create_quick_snapshot, list_quick_snapshots
        for i in range(5):
            create_quick_snapshot(label=f"s{i}", Titan_home=Titan_home)
        snaps = list_quick_snapshots(limit=3, Titan_home=Titan_home)
        assert len(snaps) == 3

    def test_restore_config(self, Titan_home):
        from Titan_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(Titan_home=Titan_home)

        (Titan_home / "config.yaml").write_text("model:\n  provider: anthropic\n")
        assert "anthropic" in (Titan_home / "config.yaml").read_text()

        result = restore_quick_snapshot(snap_id, Titan_home=Titan_home)
        assert result is True
        assert "openrouter" in (Titan_home / "config.yaml").read_text()

    def test_restore_state_db(self, Titan_home):
        from Titan_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(Titan_home=Titan_home)

        conn = sqlite3.connect(str(Titan_home / "state.db"))
        conn.execute("INSERT INTO sessions VALUES ('s2', 'new')")
        conn.commit()
        conn.close()

        restore_quick_snapshot(snap_id, Titan_home=Titan_home)
    ...
    def test_restore_nonexistent(self, Titan_home):
        from Titan_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", Titan_home=Titan_home) is False

    def test_auto_prune(self, Titan_home):
        from Titan_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
        for i in range(_QUICK_DEFAULT_KEEP + 5):
            create_quick_snapshot(label=f"snap-{i:03d}", Titan_home=Titan_home)
        snaps = list_quick_snapshots(limit=100, Titan_home=Titan_home)
        assert len(snaps) <= _QUICK_DEFAULT_KEEP

    def test_manual_prune(self, Titan_home):
        from Titan_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", Titan_home=Titan_home)
        deleted = prune_quick_snapshots(keep=3, Titan_home=Titan_home)
        assert deleted == 7
        assert len(list_quick_snapshots(Titan_home=Titan_home)) == 3

    def test_snapshot_includes_pairing_directories(self, Titan_home):
        ...
        (Titan_home / "platforms" / "pairing").mkdir(parents=True)
        (Titan_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (Titan_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (Titan_home / "pairing").mkdir()
        (Titan_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (Titan_home / "feishu_comment_pairing.json").write_text(
    ...
    def test_restore_recovers_pairing_data(self, Titan_home):
        ...
        assert restore_quick_snapshot(snap_id, Titan_home=Titan_home) is True
        assert approved.exists()
        assert '"alice"' in approved.read_text()
        assert feishu.exists()
        assert '"user_xyz"' in feishu.read_text()

    def test_empty_pairing_dir_does_not_fail(self, Titan_home):
...
# ---------------------------------------------------------------------------
# Pre-update backup (Titan update safety net)
# ---------------------------------------------------------------------------

class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``Titan update``
    ...
    def Titan_home(self, tmp_path):
        root = tmp_path / ".Titan"
        root.mkdir()
        _make_Titan_tree(root)
        return root

    def test_creates_backup_under_backups_dir(self, Titan_home):
        from Titan_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(Titan_home=Titan_home)
        assert out is not None
        assert out.exists()
        assert out.parent == Titan_home / "backups"
        assert out.name.startswith("pre-update-")
        assert out.suffix == ".zip"

    def test_backup_contents_match_full_backup(self, Titan_home):
        """Pre-update backup should include the same user data that
        ``Titan backup`` would, and should exclude the same directories."""
        from Titan_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(Titan_home=Titan_home)
        assert out is not None
        ...
        assert "skills/my-skill/SKILL.md" in names
        assert "profiles/coder/config.yaml" in names
        # titan-agent repo excluded
        assert not any(n.startswith("titan-agent/") for n in names)
        # __pycache__ excluded
        assert not any("__pycache__" in n for n in names)
        # pid files excluded
        assert "gateway.pid" not in names

    def test_does_not_recurse_into_prior_backups(self, Titan_home):
        """The ``backups/`` directory must be excluded so that each backup
        doesn't grow exponentially by including all prior backups."""
        from Titan_cli.backup import create_pre_update_backup
        # First backup
        out1 = create_pre_update_backup(Titan_home=Titan_home)
        assert out1 is not None
        # Second backup — must not include the first
        out2 = create_pre_update_backup(Titan_home=Titan_home)
    ...
    def test_rotation_keeps_only_n(self, Titan_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(Titan_home=Titan_home, keep=3)
            created.append(out)
            _t.sleep(1.05)  # ensure distinct seconds in timestamp

        remaining = sorted(
            p.name for p in (Titan_home / "backups").iterdir()
    ...
    def test_rotation_preserves_manual_files(self, Titan_home):
        """Hand-dropped zips in ``backups/`` must not be touched by
        rotation — it only prunes files matching ``pre-update-*.zip``."""
        import time as _t
        from Titan_cli.backup import create_pre_update_backup

        (Titan_home / "backups").mkdir(exist_ok=True)
        manual = Titan_home / "backups" / "my-manual.zip"
        manual.write_bytes(b"manual backup")

        for _ in range(5):
            create_pre_update_backup(Titan_home=Titan_home, keep=2)
            _t.sleep(1.05)
    ...
    def test_returns_none_if_root_missing(self, tmp_path):
        from Titan_cli.backup import create_pre_update_backup
        assert create_pre_update_backup(Titan_home=tmp_path / "does-not-exist") is None

    def test_keep_zero_does_not_delete_freshly_created_backup(self, Titan_home):
        ...
        """
        from Titan_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(Titan_home=Titan_home, keep=0)
        assert out is not None
        assert out.exists(), (
    ...
    def test_keep_negative_does_not_delete_freshly_created_backup(self, Titan_home):
    ...
    def test_keep_zero_still_prunes_older_backups(self, Titan_home):
        ...
        first = create_pre_update_backup(Titan_home=Titan_home, keep=5)
        _t.sleep(1.05)
        second = create_pre_update_backup(Titan_home=Titan_home, keep=5)
        _t.sleep(1.05)
        third = create_pre_update_backup(Titan_home=Titan_home, keep=0)
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def Titan_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".Titan"
        root.mkdir()
        _make_Titan_tree(root)
        # Point Titan_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("Titan_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for Titan_cli.config + Titan_constants so they pick up Titan_HOME
        for mod in list(__import__("sys").modules.keys()):
            if mod.startswith("Titan_cli.config") or mod == "Titan_constants":
                del __import__("sys").modules[mod]
        return root

    def test_backup_flag_creates_backup(self, Titan_home, capsys):
        """--backup forces the pre-update backup for one run even when config is off."""
        from Titan_cli.main import _run_pre_update_backup
        _run_pre_update_backup(Namespace(no_backup=False, backup=True))
        out = capsys.readouterr().out
        assert "Creating pre-update backup" in out
        assert "Saved:" in out
        assert "Restore:" in out
        assert "Titan import" in out
        assert "Disable:" in out
        # Actual backup was created
        backups = list((Titan_home / "backups").glob("pre-update-*.zip"))
        assert len(backups) == 1

    def test_default_disabled_is_silent(self, Titan_home, capsys):
        ...
        out = capsys.readouterr().out
        assert out == ""
        assert not (Titan_home / "backups").exists() or not list(
            (Titan_home / "backups").glob("pre-update-*.zip")
        )

    def test_no_backup_flag_skips(self, Titan_home, capsys):
        from Titan_cli.main import _run_pre_update_backup
    ...
    def test_config_enabled_creates_backup(self, Titan_home, capsys):
        ...
        import yaml
        (Titan_home / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
            "updates": {"pre_update_backup": True},
        }))
        import sys as _sys
        for mod in list(_sys.modules.keys()):
            if mod.startswith("Titan_cli.config"):
    ...
    def test_config_disabled_is_silent(self, Titan_home, capsys):
        ...
        out = capsys.readouterr().out
        assert out == ""
        assert not list((Titan_home / "backups").glob("pre-update-*.zip")) \
            if (Titan_home / "backups").exists() else True

    def test_cli_flag_overrides_enabled_config(self, Titan_home, capsys):
...
# ---------------------------------------------------------------------------
# Pre-migration backup (Titan claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``Titan claw migrate`` runs before mutating ~/.Titan/."""
    ...
    def test_creates_backup_under_backups_dir(self, Titan_home):
        from Titan_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(Titan_home=Titan_home)
        assert out is not None
        assert out.exists()
        # Shares the backups/ directory with pre-update backups so `Titan
        # import` and the update-backup listing both pick them up.
        assert out.parent == Titan_home / "backups"
        assert out.name.startswith("pre-migration-")
        assert out.suffix == ".zip"

    def test_backup_uses_shared_exclusion_rules(self, Titan_home):
        """Pre-migration backup reuses the same exclusion rules as
        ``Titan backup`` / ``create_pre_update_backup`` — no drift."""
        from Titan_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(Titan_home=Titan_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_restorable_with_Titan_import(self, Titan_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Titan
        backup — `Titan import` should accept it."""
        from Titan_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(Titan_home=Titan_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_recurse_into_prior_backups(self, Titan_home):
        from Titan_cli.backup import create_pre_migration_backup
        out1 = create_pre_migration_backup(Titan_home=Titan_home)
        assert out1 is not None
        out2 = create_pre_migration_backup(Titan_home=Titan_home)
        assert out2 is not None
        with zipfile.ZipFile(out2) as zf:
            names = zf.namelist()
        assert not any(n.startswith("backups/") for n in names)
    ...
    def test_rotation_keeps_only_n(self, Titan_home):
        ...
        created = []
        for _ in range(7):
            out = create_pre_migration_backup(Titan_home=Titan_home, keep=3)
            if out is not None:
                created.append(out)
            _t.sleep(1.05)  # timestamp resolution

        remaining = sorted((Titan_home / "backups").glob("pre-migration-*.zip"))
        assert len(remaining) <= 3, f"expected <=3 backups retained, got {len(remaining)}"

    def test_missing_Titan_home_returns_none(self, tmp_path):
        """Fresh install with no ~/.Titan yet — nothing to back up."""
        from Titan_cli.backup import create_pre_migration_backup
        missing = tmp_path / "does-not-exist"
        out = create_pre_migration_backup(Titan_home=missing)
        assert out is None

    def test_does_not_touch_pre_update_backups(self, Titan_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from Titan_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(Titan_home=Titan_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        import time as _t
        for _ in range(3):
            out = create_pre_migration_backup(Titan_home=Titan_home, keep=1)
            assert out is not None
            _t.sleep(1.05)
