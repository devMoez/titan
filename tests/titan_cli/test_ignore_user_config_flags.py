"""Tests for --ignore-user-config and --ignore-rules flags on `Titan chat`.
...
files. In Titan the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.Titan/config.yaml`` in ``load_cli_config()``
...
argparse → cmd_chat → cli.main() → TitanCLI → AIAgent call chain.
"""
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("Titan_IGNORE_USER_CONFIG", "Titan_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``Titan_IGNORE_USER_CONFIG=1``.

    When the env var is set, user config at ``<Titan_home>/config.yaml`` is
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._Titan_home at tmp_path and return a fresh load_cli_config."""
        import cli
        monkeypatch.setattr(cli, "_Titan_home", tmp_path)
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "anthropic/claude-sonnet-4.6")
        monkeypatch.setenv("Titan_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``TitanCLI.ignore_rules``
    so ``AIAgent`` is built with ``skip_context_files=True`` and
    ``skip_memory=True``.
    """

    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting Titan_IGNORE_RULES=1 flips TitanCLI.ignore_rules True."""
        monkeypatch.setenv("Titan_IGNORE_RULES", "1")

        # Import TitanCLI lazily — cli.py has heavy module-init side effects
        # that we don't want to run at test collection time.
        import cli
        importlib.reload(cli)

        # Build only enough of TitanCLI to reach the ignore_rules assignment.
        ...
        obj = object.__new__(cli.TitanCLI)
        # Replicate the exact logic from cli.py TitanCLI.__init__:
        ignore_rules = False  # constructor default
        obj.ignore_rules = ignore_rules or os.environ.get("Titan_IGNORE_RULES") == "1"

        assert obj.ignore_rules is True

    def test_constructor_flag_alone_enables_ignore_rules(self, monkeypatch):
        monkeypatch.delenv("Titan_IGNORE_RULES", raising=False)
        import cli
        obj = object.__new__(cli.TitanCLI)
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``Titan_cli/main.py`` must set
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("Titan_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("Titan_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("Titan_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("Titan_IGNORE_RULES") == "1"
    ...
    def test_only_ignore_user_config(self, monkeypatch):
        ...
        assert os.environ.get("Titan_IGNORE_USER_CONFIG") == "1"
        assert "Titan_IGNORE_RULES" not in os.environ
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "Titan_IGNORE_USER_CONFIG" not in os.environ
        assert "Titan_IGNORE_RULES" not in os.environ
...
class TestArgparseFlagsRegistered:
    ...
    def test_main_py_registers_both_flags(self):
        """E2E: the real Titan parser accepts both flags."""
        ...
        import inspect
        import Titan_cli.main as hm
        src = inspect.getsource(hm)
        assert "Titan_IGNORE_USER_CONFIG" in src
        assert "Titan_IGNORE_RULES" in src
