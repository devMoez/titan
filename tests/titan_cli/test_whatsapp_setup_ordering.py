...
Before the fix, ``Titan whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``Titan gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("Titan_HOME", str(Titan))
...
def _env_value(Titan_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    monkeypatch.setattr("Titan_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``Titan whatsapp`` with an existing paired session and
