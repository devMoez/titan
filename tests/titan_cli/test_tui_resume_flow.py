...
def test_main_top_level_tui_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(sys, "argv", ["Titan", "--tui", "--toolsets", "web,terminal"])
    monkeypatch.setitem(
        sys.modules,
        "Titan_cli.plugins",
...
def test_main_top_level_oneshot_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys, "argv", ["Titan", "-z", "hello", "--toolsets", "web,terminal"]
    )
    monkeypatch.setitem(
        sys.modules,
        "Titan_cli.plugins",
...
def test_oneshot_filters_invalid_toolsets_before_redirect(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    from Titan_cli.oneshot import _validate_explicit_toolsets
...
def test_oneshot_rejects_disabled_mcp_toolset(monkeypatch, capsys):
    ...
    assert valid is None
    assert error == "Titan -z: --toolsets did not contain any valid toolsets.\n"
    err = capsys.readouterr().err
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """Titan -z bypasses TitanCLI, but recall still needs SessionDB."""
    from Titan_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "Titan_state", mod("Titan_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "Titan_cli.config",
        mod("Titan_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "Titan_cli.models",
        mod("Titan_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
        "Titan_cli.runtime_provider",
        mod(
            "Titan_cli.runtime_provider",
    ...
    monkeypatch.setitem(
        sys.modules,
        "Titan_cli.tools_config",
        mod("Titan_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["Titan_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="nous/Titan-test", provider="nous", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["Titan_MODEL"] == "nous/Titan-test"
    assert env["Titan_INFERENCE_MODEL"] == "nous/Titan-test"
    assert env["Titan_TUI_PROVIDER"] == "nous"
    assert env["Titan_INFERENCE_PROVIDER"] == "nous"
    assert env["Titan_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["Titan_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("Titan-tui-active-session-")
...
def test_print_tui_exit_summary_includes_resume_and_token_totals(monkeypatch, capsys):
    import Titan_cli.main as main_mod
    ...
    monkeypatch.setitem(
        sys.modules, "Titan_state", types.SimpleNamespace(SessionDB=lambda: _FakeDB())
    )
    ...
    assert "Titan --tui --resume 20260409_000001_abc123" in out
    assert 'Titan --tui -c "demo title"' in out
...
def test_print_tui_exit_summary_prefers_actual_active_session_file(
    ...
    assert "Titan --tui --resume actual_session" in out
