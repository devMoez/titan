...
def container_env(tmp_path, monkeypatch):
    """Set up a fake Titan_HOME with .container-mode file."""
    Titan_home = tmp_path / ".Titan"
    Titan_home.mkdir()
    monkeypatch.setenv("Titan_HOME", str(Titan_home))
    monkeypatch.delenv("Titan_DEV", raising=False)

    container_mode = Titan_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=titan-agent\n"
        "exec_user=Titan\n"
        "Titan_bin=/data/current-package/bin/Titan\n"
    )
    return Titan_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("Titan_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "titan-agent"
    assert info["exec_user"] == "Titan"
    assert info["Titan_bin"] == "/data/current-package/bin/Titan"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("Titan_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_Titan_dev(container_env, monkeypatch):
    """Returns None when Titan_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("Titan_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_Titan_dev_zero(container_env, monkeypatch):
    """Titan_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("Titan_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        Titan_home = Path(tmpdir) / ".Titan"
        Titan_home.mkdir()
        (Titan_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("Titan_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_Titan_home": lambda: Titan_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("Titan_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=Titan-custom\n"
        "exec_user=myuser\n"
        "Titan_bin=/opt/Titan/bin/Titan\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "Titan-custom"
    assert info["exec_user"] == "myuser"
    assert info["Titan_bin"] == "/opt/Titan/bin/Titan"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from Titan_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "Titan"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "titan-agent" in cmd
    assert "/data/current-package/bin/Titan" in cmd
    assert "chat" in cmd
