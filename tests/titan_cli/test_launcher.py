"""Tests for the top-level `./Titan` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./Titan` should use `Titan_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "Titan"
    called = []

    fake_main_module = types.ModuleType("Titan_cli.main")

    def fake_main():
        called.append("Titan_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "Titan_cli.main", fake_main_module)
