"""Tests for Titan_cli.security_advisories.
...
import Titan_cli.security_advisories as adv
...
def isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect Titan_HOME so banner cache and config writes are sandboxed."""
    home = tmp_path / ".Titan"
    home.mkdir()
    (home / "cache").mkdir()
...
class TestAck:
    ...
    def test_ack_advisory_persists_id(self, isolated_home, monkeypatch):
        ...
        # don't depend on the full Titan_cli.config bootstrap.
        store: dict = {"security": {}}
        monkeypatch.setattr(
            "Titan_cli.config.load_config", lambda: store
        )
        monkeypatch.setattr(
            "Titan_cli.config.save_config",
