...
When ``Titan_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.Titan/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.Titan            (has its own auth.json fixture)
    * Profile     -> tmp_path/.Titan/profiles/coder   (active, Titan_HOME points here)
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (Titan_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.Titan as the "real home" — which is NOT equal
    # to the Titan_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    Titan_home = tmp_path / "classic"
    Titan_home.mkdir()
    monkeypatch.setenv("Titan_HOME", str(Titan_home))

    _write(Titan_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from Titan_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: Titan_HOME is set to a custom path that is NOT under
    # ~/.Titan/profiles/ — get_default_Titan_root() returns Titan_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from Titan_cli.auth import read_credential_pool, write_credential_pool
