...
Ensures ``Titan update`` correctly detects running gateways managed by
...
def _no_restart_verify_sleep(monkeypatch):
    """Titan_cli/main.py uses time.sleep(3) after systemctl restart to
...
def _make_run_side_effect(
    ...
    def side_effect(cmd, **kwargs):
        ...
        # systemctl list-units Titan-gateway* — discover all gateway services
        if "systemctl" in joined and "list-units" in joined:
            if "--user" in joined and systemd_active:
                return subprocess.CompletedProcess(
                    cmd, 0,
                    stdout="Titan-gateway.service loaded active running Titan Gateway\n",
                    stderr="",
                )
...
class TestLaunchdPlistPath:
    def test_plist_contains_environment_variables(self):
        ...
        assert "<key>Titan_HOME</key>" in plist
...
class TestCmdUpdateLaunchdRestart:
    ...
    def test_update_detects_launchd_and_skips_manual_restart_message(
        ...
        'auto-restart via launchd' instead of 'Restart it with: Titan gateway run'."""
        ...
        assert "Restart manually: Titan gateway run" not in captured
    ...
    def test_update_without_launchd_shows_manual_restart(
        ...
        captured = capsys.readouterr().out
        assert "Restart manually: Titan gateway run" in captured
    ...
    def test_update_restarts_profile_manual_gateways(
        ...
        monkeypatch.setattr(
            ...
            lambda: tmp_path / "ai.Titan.gateway.plist",
        )
        ...
        process = gateway_cli.ProfileGatewayProcess(
            profile="coder",
            path=tmp_path / ".Titan" / "profiles" / "coder",
            pid=12345,
        )
    ...
    def test_update_with_systemd_still_restarts_via_systemd(
        ...
        captured = capsys.readouterr().out
        assert "Restarted Titan-gateway" in captured
        ...
        def side_effect(cmd, **kwargs):
            ...
            if "systemctl" in joined and "list-units" in joined:
                if "--user" in joined:
                    return subprocess.CompletedProcess(
                        cmd, 0,
                        stdout="Titan-gateway.service loaded active running\n",
                        stderr="",
                    )
                return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
    ...
    def test_update_falls_back_to_systemctl_restart_when_sigusr1_times_out(
        ...
        monkeypatch.setattr(
            "Titan_cli.gateway._graceful_restart_via_sigusr1",
    ...
    def test_update_bypasses_restartsec_after_graceful_drain(
        ...
        # Must have called ``reset-failed Titan-gateway`` AND ``start
        # Titan-gateway`` explicitly so systemd bypasses RestartSec.
        reset_calls = [c for c in calls if "reset-failed" in c and "Titan-gateway" in c]
        start_calls = [
            c for c in calls
            if "start" in c and "Titan-gateway" in c and "restart" not in c
        ]
        assert reset_calls, (
            f"Expected explicit `reset-failed Titan-gateway` after graceful drain; "
            f"systemctl calls were: {calls}"
        )
        assert start_calls, (
            f"Expected explicit `start Titan-gateway` after graceful drain to "
...
class TestServicePidExclusion:
    ...
    where ``Titan update`` would restart the gateway and immediately kill it.
    """
...
class TestGetServicePids:
    ...
    def test_returns_launchd_pid(self, monkeypatch):
        monkeypatch.setattr(gateway_cli, "is_linux", lambda: False)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: True)
        monkeypatch.setattr(gateway_cli, "get_launchd_label", lambda: "ai.Titan.gateway")
    ...
    def test_excludes_zero_pid(self, monkeypatch):
        ...
        def fake_run(cmd, **kwargs):
            ...
            if "list-units" in joined:
                return subprocess.CompletedProcess(
                    cmd, 0,
                    stdout="Titan-gateway.service loaded inactive dead Titan Gateway\n",
                    stderr="",
                )
            if "show" in joined and "MainPID" in joined:
    ...
    def test_filters_to_current_profile(self, monkeypatch, tmp_path):
        ...
        def fake_run(cmd, **kwargs):
            return subprocess.CompletedProcess(
                cmd, 0,
                stdout=(
                    "100 /Users/dgrieco/.Titan/titan-agent/venv/bin/python -m Titan_cli.main --profile orcha gateway run --replace\n"
                    "200 /Users/dgrieco/.Titan/titan-agent/venv/bin/python -m Titan_cli.main --profile other gateway run --replace\n"
                ),
                stderr="",
            )
        ...
        monkeypatch.setattr("os.getpid", lambda: 999)
        monkeypatch.setattr(gateway_cli, "_get_service_pids", lambda: set())
        monkeypatch.setattr(gateway_cli, "_profile_arg", lambda Titan_home=None: "--profile orcha")
...
class TestGatewayModeWritesExitCodeEarly:
    """When running as ``Titan update --gateway``, the exit code marker must be
    ...
    def test_exit_code_written_in_gateway_mode(
        ...
        # Point Titan_HOME at a temp dir so the marker file lands there
        Titan_home = tmp_path / ".Titan"
        Titan_home.mkdir()
        monkeypatch.setenv("Titan_HOME", str(Titan_home))
        import Titan_cli.config as _cfg
        monkeypatch.setattr(_cfg, "get_Titan_home", lambda: Titan_home)
        # Also patch the module-level ref used by cmd_update
        import Titan_cli.main as _main_mod
        monkeypatch.setattr(_main_mod, "get_Titan_home", lambda: Titan_home)
        ...
        exit_code_path = Titan_home / ".update_exit_code"
...
class TestCmdUpdateLegacyGatewayWarning:
    """Tests for the legacy Titan.service warning printed by `Titan update`.

    Users who installed Titan before the service rename often have a
    dormant ``Titan.service`` that starts flap-fighting the current
    ``Titan-gateway.service`` after PR #5646. Every ``Titan update``
    should remind them to run ``Titan gateway migrate-legacy`` until
    they do.
    """
    ...
    def test_update_prints_legacy_warning_when_detected(
        ...
        user_dir.mkdir()
        system_dir.mkdir()
        legacy_path = user_dir / "Titan.service"
        ...
        captured = capsys.readouterr().out
        assert "Legacy Titan gateway unit(s) detected" in captured
        assert "Titan.service" in captured
        assert "Titan gateway migrate-legacy" in captured
    ...
    def test_update_silent_when_no_legacy_units(
        ...
        captured = capsys.readouterr().out
        assert "Legacy Titan gateway" not in captured
    ...
    def test_update_does_not_flag_profile_units(
        ...
        """Profile units (Titan-gateway-coder.service) must not trigger the warning.

        This is the core safety invariant: the legacy allowlist is
        ``Titan.service`` only, no globs.
        """
        user_dir = tmp_path / "user"
        system_dir = tmp_path / "system"
        user_dir.mkdir()
        ...
        (user_dir / "Titan-gateway-coder.service").write_text(
            self._OUR_UNIT_TEXT, encoding="utf-8"
        )
        (user_dir / "Titan-gateway.service").write_text(
            self._OUR_UNIT_TEXT, encoding="utf-8"
        )
        ...
        captured = capsys.readouterr().out
        assert "Legacy Titan gateway" not in captured
        assert "Titan-gateway-coder.service" not in captured  # not flagged
    ...
    def test_update_lists_system_scope_unit_with_sudo_hint(
        ...
        captured = capsys.readouterr().out
        assert "Legacy Titan gateway" in captured
...
class TestCmdUpdateResetFailedBeforeRestart:
    """`Titan update` must call `systemctl reset-failed` before every
    ...
    Mirrors the recovery pattern `Titan gateway restart` (systemd_restart)
    ...
    def test_reset_failed_runs_before_fallback_restart(
        ...
        assert any(
            "Titan-gateway" in " ".join(str(c) for c in call)
            for call in reset_calls
        ), (
            "Expected `systemctl reset-failed Titan-gateway` before the "
        ...
        for idx, call in enumerate(mock_run.call_args_list):
            ...
            if "systemctl" in joined and "reset-failed" in joined and first_reset_idx is None:
                first_reset_idx = idx
            if "systemctl" in joined and "restart" in joined and "Titan-gateway" in joined:
    ...
    def test_reset_failed_also_runs_before_retry_restart(
        ...
        gateway_restarts = [
            ...
            if "Titan-gateway" in " ".join(str(a) for a in c)
        ]
        gateway_resets = [
    ...
    def test_final_failure_message_tells_user_to_reset_failed(
        ...
        assert "Titan-gateway" in captured
