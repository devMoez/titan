"""Unit tests for Titan_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf Titan-ok"])
        try:
            output = _read_until(bridge, b"Titan-ok")
            assert b"Titan-ok" in output
        finally:
            bridge.close()
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$Titan_PTY_TEST\""],
            env={**os.environ, "Titan_PTY_TEST": "pty-env-works"},
        )
        try:
