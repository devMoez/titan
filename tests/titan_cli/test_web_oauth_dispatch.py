"""Regression tests for the OAuth dispatcher in Titan_cli.web_server.
...
from Titan_cli.web_server import _SESSION_TOKEN, app

client = TestClient(app)
HEADERS = {"X-Titan-Session-Token": _SESSION_TOKEN}
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        "Titan_cli.auth._minimax_request_user_code",
        return_value=fake_user_code_resp,
    ), patch(
        "Titan_cli.auth._minimax_pkce_pair",
...
def test_minimax_dashboard_poller_accepts_absolute_ms_expired_in():
    ...
    from Titan_cli import web_server as ws
    ...
    try:
        with patch(
            "Titan_cli.auth._minimax_poll_token",
            return_value={
                "status": "success",
                "access_token": "access",
            ...
            "Titan_cli.auth._minimax_save_auth_state",
...
def test_anthropic_pkce_branch_still_works():
    ...
    with patch(
        "Titan_cli.web_server._start_anthropic_pkce",
...
def test_unknown_pkce_provider_rejected_cleanly():
    ...
    fake_entry = {
        ...
        "flow": "pkce",
        "cli_command": "Titan auth add hypothetical-pkce-provider",
