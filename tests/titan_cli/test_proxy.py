"""Tests for the `Titan proxy` subcommand and its upstream adapters."""
...
from Titan_cli.proxy.adapters import ADAPTERS, get_adapter
from Titan_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
from Titan_cli.proxy.adapters.nous_portal import NousPortalAdapter
...
def _write_auth_store(Titan_home: Path, nous_state: Dict[str, Any]) -> Path:
    """Write an auth.json with the given nous state into a hermetic Titan_HOME."""
...
def test_nous_adapter_not_authenticated_when_no_auth_file(tmp_path, monkeypatch):
    # Titan_HOME is already set by conftest, but make doubly sure
...
def test_nous_adapter_get_credential_refreshes_and_persists(tmp_path, monkeypatch):
    ...
    with patch(
        "Titan_cli.proxy.adapters.nous_portal.refresh_nous_oauth_from_state",
...
def test_nous_adapter_get_credential_raises_when_not_logged_in(tmp_path, monkeypatch):
    monkeypatch.setenv("Titan_HOME", str(tmp_path))
    adapter = NousPortalAdapter()
    with pytest.raises(RuntimeError, match="Titan login nous"):
...
from Titan_cli.proxy.server import create_app  # noqa: E402
...
def test_server_forwards_chat_completions():
    async def run():
        ...
        try:
            ...
            assert "Titan-4-70B" in req["body"]
        finally:
...
def test_cmd_proxy_status_runs(capsys, tmp_path, monkeypatch):
    monkeypatch.setenv("Titan_HOME", str(tmp_path))
    from Titan_cli.proxy.cli import cmd_proxy_status
...
def test_cmd_proxy_providers_runs(capsys):
    from Titan_cli.proxy.cli import cmd_proxy_list_providers
...
def test_cmd_proxy_start_refuses_unknown_provider(capsys):
    from Titan_cli.proxy.cli import cmd_proxy_start
...
def test_cmd_proxy_start_refuses_when_unauthenticated(capsys, tmp_path, monkeypatch):
    ...
    assert rc == 2
    err = capsys.readouterr().err
    assert "Titan login nous" in err
