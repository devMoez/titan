...
``Titan_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``Titan --help``, ``Titan version``,
``Titan logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``Titan --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from Titan_cli import main as _main
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["Titan", "--tui", "chat"], "chat"),
        (["Titan", "-w", "logs"], "logs"),
        (["Titan", "chat", "hello world"], "chat"),
        (["Titan", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["Titan", "-m", "gpt5", "chat"], "chat"),
        (["Titan", "--model", "gpt5", "chat", "hi"], "chat"),
        (["Titan", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["Titan", "-z", "hello world"], None),
        (["Titan", "-z", "hello", "chat"], "chat"),
        (["Titan", "--model=gpt5", "chat"], "chat"),     # inline form
        (["Titan", "--", "chat"], "chat"),               # -- terminator
        (["Titan", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["Titan", "some-plugin-cmd"], "some-plugin-cmd"),
        (["Titan", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["Titan"],                          # bare → chat
        ["Titan", "--help"],                # top-level help
        ["Titan", "-h"],
        ["Titan", "version"],               # known built-in
        ["Titan", "logs"],
        ["Titan", "gateway", "run"],
        ["Titan", "--tui"],
        ["Titan", "-w", "--tui"],
        ["Titan", "chat", "hi"],
        ["Titan", "help"],                  # accepted built-in-ish
        ["Titan", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["Titan", "meet", "join"],          # potential google_meet plugin
        ["Titan", "honcho", "status"],      # potential memory plugin
        ["Titan", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``Titan help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"Titan_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
