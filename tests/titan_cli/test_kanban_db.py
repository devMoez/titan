"""Tests for the Kanban DB layer (Titan_cli.kanban_db)."""
...
from Titan_cli import kanban_db as kb


@pytest.fixture
def kanban_home(tmp_path, monkeypatch):
    """Isolated Titan_HOME with an empty kanban DB."""
    home = tmp_path / ".Titan"
    home.mkdir()
...
def test_stale_claim_reclaimed(kanban_home, monkeypatch):
    import signal
    import Titan_cli.kanban_db as _kb
...
def test_dispatch_skips_nonspawnable_into_separate_bucket(kanban_home, monkeypatch):
    ...
    from Titan_cli import profiles
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real Titan profile — preserves the
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="Titan-task", assignee="daily")
...
def test_scratch_workspace_created_under_Titan_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
...
# spawned with `Titan -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's Titan_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's Titan_HOME."""

    def _set_home(self, monkeypatch, tmp_path, Titan_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("Titan_HOME", str(Titan_home))
        monkeypatch.delenv("Titan_KANBAN_HOME", raising=False)

    def test_default_install_anchors_at_home_dot_Titan(
        self, tmp_path, monkeypatch
    ):
        # Standard install: Titan_HOME == ~/.Titan, no profile active.
        default_home = tmp_path / ".Titan"
        default_home.mkdir()
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.Titan/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.Titan/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.Titan/kanban.db.
        default_home = tmp_path / ".Titan"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # Titan_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `Titan -p coder`).
        monkeypatch.setenv("Titan_HOME", str(profile_home))
    ...
    def test_docker_custom_Titan_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: Titan_HOME points outside ~/.Titan.
        # `get_default_Titan_root()` returns env_home directly when it
        # is not a `<root>/profiles/<name>` shape and not under
        # `Path.home() / ".Titan"`.
        custom_root = tmp_path / "opt" / "Titan"
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: Titan_HOME=/opt/Titan/profiles/coder;
        # `get_default_Titan_root()` walks up to /opt/Titan because
    ...
    def test_explicit_override_via_Titan_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # Explicit override: Titan_KANBAN_HOME beats every other
        # resolution rule.
        default_home = tmp_path / ".Titan"
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("Titan_HOME", str(profile_home))
        monkeypatch.setenv("Titan_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        default_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("Titan_HOME", str(default_home))
        monkeypatch.setenv("Titan_KANBAN_HOME", "   ")
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # Titan_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile Titan_HOME and reads.
        monkeypatch.setenv("Titan_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_Titan_kanban_db_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # Titan_KANBAN_DB pins the file path directly and beats both
        # Titan_KANBAN_HOME and the `get_default_Titan_root()` path.
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("Titan_HOME", str(default_home))
        monkeypatch.setenv("Titan_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("Titan_KANBAN_DB", str(pinned_db))

        assert kb.kanban_db_path() == pinned_db
        # workspaces_root still follows Titan_KANBAN_HOME -- the pins
        # are independent.
        assert kb.workspaces_root() == umbrella / "kanban" / "workspaces"

    def test_Titan_kanban_workspaces_root_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # Titan_KANBAN_WORKSPACES_ROOT pins the workspaces root directly.
        default_home = tmp_path / ".Titan"
        default_home.mkdir()
        umbrella = tmp_path / "umbrella"
        umbrella.mkdir()
        ...
        monkeypatch.setenv("Titan_HOME", str(default_home))
        monkeypatch.setenv("Titan_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("Titan_KANBAN_WORKSPACES_ROOT", str(pinned_ws))

        assert kb.workspaces_root() == pinned_ws
        # kanban_db_path still follows Titan_KANBAN_HOME.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("Titan_HOME", str(default_home))
        monkeypatch.setenv("Titan_KANBAN_DB", "   ")
        monkeypatch.setenv("Titan_KANBAN_WORKSPACES_ROOT", "")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # The dispatcher's `_default_spawn` must inject Titan_KANBAN_DB
        # and Titan_KANBAN_WORKSPACES_ROOT into the worker env so the
        # worker converges on the dispatcher's paths even when the
        # `-p <profile>` flag rewrites Titan_HOME.
        default_home = tmp_path / ".Titan"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
        ...
        env = captured["env"]
        assert env["Titan_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["Titan_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["Titan_KANBAN_TASK"] == "t_dispatch_env"
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see Titan_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(kanban_home, caplog):
    ...
    with _patch("Titan_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="Titan_state"):
            conn = kb.connect()
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'Titan kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_Titan_argv()
#
# Workers spawned by the dispatcher must use a `Titan` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["Titan", ...]`
...
def test_resolve_Titan_argv_prefers_path_shim(monkeypatch):
    """When `Titan` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
    import Titan_cli.kanban_db as kb

    monkeypatch.setattr(shutil, "which", lambda name: "/usr/local/bin/Titan")
    argv = kb._resolve_Titan_argv()
    assert argv == ["/usr/local/bin/Titan"]


def test_resolve_Titan_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m Titan_cli.main`.

    Pins the correct module name (NOT `Titan` — there is no top-level
    `Titan` package). Regression for #23198: the original PR shipped
    `python -m Titan` which fails with `No module named Titan` on every
    invocation.
    """
    import shutil
    import sys
    ...
    argv = kb._resolve_Titan_argv()
    assert argv == [sys.executable, "-m", "Titan_cli.main"]


def test_resolve_Titan_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `Titan_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m Titan_cli.main --version`
    ...
    assert "Titan Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
