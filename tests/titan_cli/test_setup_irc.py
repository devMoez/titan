"""Tests for IRC gateway configuration via `Titan setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Titan install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Titan install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#Titan")
            monkeypatch.setenv("IRC_NICKNAME", "Titan-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `Titan setup gateway` experience with IRC present."""
