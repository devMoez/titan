"""Shared fixtures for the titan-agent test suite.
...
2. **Isolated Titan_HOME.** Titan_HOME points to a per-test tempdir so
   code reading ``~/.Titan/*`` via ``get_Titan_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".Titan"`` instead of the canonical
   ``get_Titan_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No Titan_SESSION_* inheritance** — the agent's current gateway
...
# Titan_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_Titan_BEHAVIORAL_VARS = frozenset({
    "Titan_YOLO_MODE",
    "Titan_INTERACTIVE",
    "Titan_QUIET",
    "Titan_TOOL_PROGRESS",
    ...
    "Titan_SESSION_CHAT_ID",
    "Titan_SESSION_CHAT_NAME",
    "Titan_SESSION_THREAD_ID",
    "Titan_SESSION_SOURCE",
    "Titan_SESSION_KEY",
    "Titan_GATEWAY_SESSION",
    "Titan_PLATFORM",
    "Titan_MODEL",
    "Titan_INFERENCE_MODEL",
    ...
    "Titan_TIMEZONE",
    "Titan_REDACT_SECRETS",
    "Titan_BACKGROUND_NOTIFICATIONS",
    "Titan_EXEC_ASK",
    "Titan_HOME_MODE",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.Titan/kanban.db instead of the per-test Titan_HOME.
    "Titan_KANBAN_DB",
    "Titan_KANBAN_BOARD",
    "Titan_KANBAN_WORKSPACES_ROOT",
    "Titan_KANBAN_LOGS_ROOT",
    "Titan_KANBAN_TASK",
    "Titan_KANBAN_WORKSPACE",
    "Titan_TENANT",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and Titan_HOME to per-test tempdirs so code that
    reads ``~/.Titan/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral Titan_* vars that could change test semantics.
    for name in _Titan_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect Titan_HOME to a per-test tempdir. Code that reads
    #    ``~/.Titan/*`` via ``get_Titan_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.Titan/*`` via
    #    ``Path.home() / ".Titan"`` instead of ``get_Titan_home()``
    #    is a bug to fix at the callsite.
    fake_Titan_home = tmp_path / "Titan_test"
    fake_Titan_home.mkdir()
    (fake_Titan_home / "sessions").mkdir()
    (fake_Titan_home / "cron").mkdir()
    (fake_Titan_home / "memories").mkdir()
    (fake_Titan_home / "skills").mkdir()
    monkeypatch.setenv("Titan_HOME", str(fake_Titan_home))
    ...
    #    ~/.Titan/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import Titan_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_Titan_home(_hermetic_environment):
...
def _reset_module_state():
    ...
    for _logger_name in ("tools", "run_agent", "trajectory_compressor", "cron", "Titan_cli"):
...
@pytest.fixture()
def mock_config():
    """Return a minimal Titan config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``Titan-gateway`` process
...
# correlated with a ``tests/Titan_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> Titan-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart Titan-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting Titan/python patterns are also blocked.
    """
    ...
    def _matches_Titan_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _Titan_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_Titan_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch Titan-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m Titan_cli.main".
                if (
                    "Titan" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live Titan-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting Titan/python could hit the live gateway. "
