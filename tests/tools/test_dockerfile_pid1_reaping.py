...
- Signal forwarding runs through the init so ``docker stop`` triggers
  Titan's own graceful-shutdown path.
"""
...
def test_dockerfile_installs_an_init_for_zombie_reaping(dockerfile_text):
    ...
    Without a PID-1 init that handles SIGCHLD, Titan accumulates zombie
    ...
    assert installed, (
        ...
        "orphaned subprocesses, Titan accumulates zombies in Docker "
...
def test_dockerfile_entrypoint_routes_through_the_init(dockerfile_text):
    ...
    directly, the shell becomes PID 1 and will ``exec`` into Titan,
    ...
    assert routes_through_init, (
        ...
        "If tini is only installed but not wired into ENTRYPOINT, Titan "
...
def test_dockerfile_installs_tui_dependencies(dockerfile_text):
    ...
    # resolve dependencies. The bundled ``Titan-ink`` workspace package is
    ...
    assert "ui-tui/packages/Titan-ink/" in dockerfile_text
    assert any(
...
def test_dockerfile_materializes_local_tui_ink_package(dockerfile_text):
    ...
    assert "ui-tui/packages/Titan-ink/" in dockerfile_text, (
        "Dockerfile must COPY the bundled Titan-ink workspace package "
        "so ``await import('@Titan/ink')`` resolves at runtime."
    )
