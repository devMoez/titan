...
class TestWriteDenyExactPaths:
    ...
    def test_Titan_env(self):
        # ``.env`` under the active Titan_HOME (profile-aware, not just
        # ``~/.Titan``) must be write-denied. The hermetic test conftest
        # points Titan_HOME at a tempdir — resolve via get_Titan_home()
        # to match the denylist.
        from Titan_constants import get_Titan_home
        path = str(get_Titan_home() / ".env")
        assert _is_write_denied(path) is True
...
class TestWriteAllowed:
    ...
    def test_Titan_config_not_env(self):
        path = os.path.join(str(Path.home()), ".Titan", "config.yaml")
