...
def _restore_tool_modules():
    original_Titan_home = os.environ.get("Titan_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_Titan_home is None:
            os.environ.pop("Titan_HOME", None)
        else:
            os.environ["Titan_HOME"] = original_Titan_home
        _reset_modules(("tools", "Titan_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    sys.modules["Titan_cli.config"] = types.SimpleNamespace(
        get_Titan_home=lambda: Titan_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="titan-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": Titan_home / "modal_snapshots.json",
