...
def test_Titan_provider_subclass_exists():
    """TitanMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _Titan_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _Titan_PROVIDER_CLS is not None
    assert issubclass(_Titan_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_Titan_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns TitanMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _Titan_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("Titan_HOME", str(tmp_path))
    ...
    assert _Titan_PROVIDER_CLS is not None
    assert isinstance(provider, _Titan_PROVIDER_CLS)
    assert provider._Titan_server_name == "srv"
