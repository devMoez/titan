...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._Titan_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_Titan_bin(self):
        """After _INSTALL_FAILED, manual install in Titan_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        Titan_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(Titan_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(Titan_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._Titan_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == Titan_bin
            assert _tirith_mod._resolved_path == Titan_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# Titan_HOME isolation
# ---------------------------------------------------------------------------

class TestTitanHomeIsolation:
    def test_Titan_bin_dir_respects_Titan_home(self):
        """_Titan_bin_dir must use Titan_HOME, not hardcoded ~/.Titan."""
        from tools.tirith_security import _Titan_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"Titan_HOME": tmpdir}):
            result = _Titan_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_Titan_home(self):
        """_failure_marker_path must use Titan_HOME, not hardcoded ~/.Titan."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"Titan_HOME": "/custom/Titan"}):
            result = _failure_marker_path()
        assert result == "/custom/Titan/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets Titan_HOME; verify it's active."""
        Titan_home = os.getenv("Titan_HOME")
        assert Titan_home is not None, "Titan_HOME should be set by conftest"
        assert "Titan_test" in Titan_home, "Should point to test temp dir"

    def test_get_Titan_home_fallback(self):
        """Without Titan_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_Titan_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove Titan_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".Titan")
            result = _get_Titan_home()
