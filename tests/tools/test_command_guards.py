...
def _clean_state():
    ...
    saved = {}
    for k in ("Titan_INTERACTIVE", "Titan_GATEWAY_SESSION", "Titan_EXEC_ASK", "Titan_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["Titan_INTERACTIVE"] = "1"
        session_key = os.getenv("Titan_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No Titan_INTERACTIVE or Titan_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
