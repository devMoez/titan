...
def _load_plugin_router():
    ...
    spec = importlib.util.spec_from_file_location(
        "Titan_dashboard_plugin_kanban_test", plugin_file,
    )
...
def test_board_auto_initializes_missing_db(tmp_path, monkeypatch):
    ...
    home.mkdir()
    monkeypatch.setenv("Titan_HOME", str(home))
    monkeypatch.delenv("Titan_KANBAN_BOARD", raising=False)
    monkeypatch.delenv("Titan_KANBAN_DB", raising=False)
...
def test_ws_events_rejects_when_token_required(tmp_path, monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "Titan_cli.web_server", stub)
    monkeypatch.setattr(Titan_cli, "web_server", stub, raising=False)
...
def test_create_task_includes_warning_when_no_dispatcher(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "Titan_cli.kanban._check_dispatcher_presence",
        lambda: (False, "No gateway is running — start `Titan gateway start`."),
    )
    r = client.post(
        "/api/plugins/kanban/tasks",
...
def test_create_task_probe_error_does_not_break_create(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "Titan_cli.kanban._check_dispatcher_presence", _raise,
    )
    r = client.post(
        "/api/plugins/kanban/tasks",
...
def test_board_surfaces_warnings_field_for_hallucinated_completions(client):
    ...
    raw event kind — see Titan_cli.kanban_diagnostics for the rule
    that produces it.
    """
    conn = kb.connect()
    try:
...
def test_dashboard_bulk_actions_include_reclaim_first():
    ...
    assert "Titan-kanban-bulk-reclaim-first" in dist
    assert '"→ todo"' in dist
...
def test_dashboard_failed_card_highlight_class_exists():
    ...
    assert "Titan-kanban-card--failed" in js
    assert "Titan-kanban-card--failed" in css
