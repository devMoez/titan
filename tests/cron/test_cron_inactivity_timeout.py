...
- Unlimited timeout (Titan_CRON_TIMEOUT=0)
- Backward compat: Titan_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """Titan_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """Titan_CRON_TIMEOUT env var is respected."""
        monkeypatch.setenv("Titan_CRON_TIMEOUT", "1200")
        raw = os.getenv("Titan_CRON_TIMEOUT", "").strip()
    ...
    def test_timeout_zero_means_unlimited(self, monkeypatch):
        """Titan_CRON_TIMEOUT=0 yields None (unlimited)."""
        monkeypatch.setenv("Titan_CRON_TIMEOUT", "0")
        raw = os.getenv("Titan_CRON_TIMEOUT", "").strip()
        _cron_timeout = self._parse_cron_timeout(raw)
        _cron_inactivity_limit = _cron_timeout if _cron_timeout > 0 else None
    ...
    def test_timeout_invalid_value_falls_back_to_default(self, monkeypatch):
        """Titan_CRON_TIMEOUT=abc should fall back to 600s, not raise ValueError."""
        monkeypatch.setenv("Titan_CRON_TIMEOUT", "abc")
        raw = os.getenv("Titan_CRON_TIMEOUT", "").strip()
        _cron_timeout = self._parse_cron_timeout(raw)
    ...
    def test_timeout_empty_string_uses_default(self, monkeypatch):
        """Titan_CRON_TIMEOUT='' (empty) should use the 600s default."""
        monkeypatch.setenv("Titan_CRON_TIMEOUT", "")
        raw = os.getenv("Titan_CRON_TIMEOUT", "").strip()
        _cron_timeout = self._parse_cron_timeout(raw)
        assert _cron_timeout == 600.0
...
class TestSysPathOrdering:
    ...
    def test_Titan_time_importable(self):
        """Titan_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _Titan_now
        assert callable(_Titan_now)

    def test_Titan_constants_importable(self):
        """Titan_constants should be importable from cron context."""
        from Titan_constants import get_Titan_home
        assert callable(get_Titan_home)
