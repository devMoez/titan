"""Tests for Titan_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Titan entry point
(Titan, titan-agent, Titan-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Titan entry point imports Titan_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported Titan_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("Titan_bootstrap", None)
    import Titan_bootstrap  # noqa: WPS433
    return Titan_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Titan CLI
...
class TestEntryPointsImportBootstrap:
    """Every Titan entry point must import Titan_bootstrap as its
    ...
    # Entry points that invoke Titan as a process.  Each one must
    # import Titan_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "Titan_cli/main.py",   # Titan CLI (console_script)
        "run_agent.py",          # titan-agent (console_script)
        "acp_adapter/entry.py",  # Titan-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import Titan_bootstrap' and that
        ...
        half-finished ``Titan update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``Titan_bootstrap`` as a top-level module) leaves Titan
        ...
        """
        # Resolve relative to the titan-agent repo root.  Tests live
        # at tests/test_Titan_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # Titan start even when Titan_bootstrap hasn't been
        ...
        assert first_import_name == "Titan_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'Titan_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import Titan_bootstrap' line to be the first import."
        )
