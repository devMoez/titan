"""
Tests for mcp_serve — Titan MCP server.
...
def _isolate_Titan_home(tmp_path, monkeypatch):
    """Redirect Titan_HOME to a temp directory."""
    monkeypatch.setenv("Titan_HOME", str(tmp_path))
    try:
        import Titan_constants
        monkeypatch.setattr(Titan_constants, "get_Titan_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking Titan_state schema."""
