"""Tests for Titan_constants module."""
...
from Titan_constants import (
    ...
    get_default_Titan_root,
    is_container,
    parse_reasoning_effort,
)


class TestGetDefaultTitanRoot:
    """Tests for get_default_Titan_root() — Docker/custom deployment awareness."""

    def test_no_Titan_home_returns_native(self, tmp_path, monkeypatch):
        """When Titan_HOME is not set, returns ~/.Titan."""
        monkeypatch.delenv("Titan_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_Titan_root() == tmp_path / ".Titan"

    def test_Titan_home_is_native(self, tmp_path, monkeypatch):
        """When Titan_HOME = ~/.Titan, returns ~/.Titan."""
        native = tmp_path / ".Titan"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("Titan_HOME", str(native))
        assert get_default_Titan_root() == native

    def test_Titan_home_is_profile(self, tmp_path, monkeypatch):
        """When Titan_HOME is a profile under ~/.Titan, returns ~/.Titan."""
        native = tmp_path / ".Titan"
    ...
    def test_Titan_home_is_docker(self, tmp_path, monkeypatch):
        """When Titan_HOME points outside ~/.Titan (Docker), returns Titan_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("Titan_HOME", str(docker_home))
        assert get_default_Titan_root() == docker_home

    def test_Titan_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any Titan_HOME outside ~/.Titan is treated as the root."""
        custom = tmp_path / "my-Titan-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("Titan_HOME", str(custom))
        assert get_default_Titan_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (Titan_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("Titan_HOME", str(profile))
        assert get_default_Titan_root() == docker_root
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(Titan_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(Titan_constants, "_container_detected", True)
        assert is_container() is True
