...
Lets the LLM write a Python script that calls Titan tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `Titan_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `Titan_tools.py` with file-based RPC stubs
...
# available on every platform Titan itself runs on.
...
def generate_Titan_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the Titan_tools.py stub module.
...
"""Auto-generated Titan tools RPC stubs."""
...
def _connect():
    ...
    Titan_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["Titan_RPC_SOCKET"]
...
"""Auto-generated Titan tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("Titan_RPC_DIR") or os.path.join(tempfile.gettempdir(), "Titan_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated Titan_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/Titan_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_Titan_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/Titan_tools.py", tools_src)
        ...
        env_prefix = (
            f"Titan_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("Titan_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
...
def execute_code(
    ...
    # --- Set up temp directory with Titan_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="Titan_sandbox_")
    ...
    # selector from Titan_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"Titan_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated Titan_tools module.
        ...
        tools_src = generate_Titan_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "Titan_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  Titan_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["Titan_RPC_SOCKET"] = rpc_endpoint
        ...
        # Ensure the titan-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from Titan_tools import ...`` resolves even
        ...
        _pp_parts = [tmpdir, _Titan_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # Titan_TIMEZONE is an internal Titan setting and must not leak
        # into child processes.
        _tz_name = os.getenv("Titan_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("Titan_TIMEZONE", None)

        # Per-profile HOME isolation: redirect system tool configs into
        # {Titan_HOME}/home/ when that directory exists.
        from Titan_constants import get_subprocess_home
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.Titan/.env')).
...
def _load_config() -> dict:
    ...
    ``Titan --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (titan-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``Titan tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and titan-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.Titan/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Titan tools programmatically. "
        ...
        f"Available via `from Titan_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into Titan_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from Titan_tools import {import_str}` "
