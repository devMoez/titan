#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Titan Skills Hub.
...
Used by Titan_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                Titan_meta = meta_block.get("Titan", {})
                if isinstance(Titan_meta, dict):
                    tags = Titan_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Titan centralized index source
# ---------------------------------------------------------------------------

Titan_INDEX_URL = "https://titan-agent.nousresearch.com/docs/api/skills-index.json"
Titan_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "Titan-index.json"
Titan_INDEX_TTL = 6 * 3600  # 6 hours


def _load_Titan_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for Titan_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if Titan_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - Titan_INDEX_CACHE_FILE.stat().st_mtime
            if age < Titan_INDEX_TTL:
                return json.loads(Titan_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(Titan_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Titan index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Titan index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        Titan_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        Titan_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class TitanIndexSource(SkillSource):
    """Skill source backed by the centralized Titan Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "Titan-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "Titan-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        TitanIndexSource(auth=auth), # Centralized index (search + resolved install paths)
