...
it is automatically downloaded from GitHub releases to $Titan_HOME/bin/tirith.
...
def _get_Titan_home() -> str:
    """Return the Titan home directory, respecting Titan_HOME env var."""
    return str(get_Titan_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_Titan_home(), ".tirith-install-failed")
...
def _Titan_bin_dir() -> str:
    """Return $Titan_HOME/bin, creating it if needed."""
    d = os.path.join(_get_Titan_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $Titan_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_Titan_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $Titan_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $Titan_HOME/bin/tirith
    ...
    Titan_bin = os.path.join(_Titan_bin_dir(), "tirith")
    if os.path.isfile(Titan_bin) and os.access(Titan_bin, os.X_OK):
        _resolved_path = Titan_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return Titan_bin
