...
Tracks per-skill usage metadata in a sidecar JSON file (~/.Titan/skills/.usage.json)
...
def _read_bundled_manifest_names() -> Set[str]:
    ...
    Reads ~/.Titan/skills/.bundled_manifest (format: "name:hash" per line).
...
def _read_hub_installed_names() -> Set[str]:
    ...
    Reads ~/.Titan/skills/.hub/lock.json (see tools/skills_hub.py :: HubLockFile).
    """
...
def list_archived_skill_names() -> List[str]:
    """Enumerate skills in ``~/.Titan/skills/.archive/``.

    Archive layout is flat (``.archive/<skill>/``) as set by ``archive_skill``,
    so the directory name is the skill name. Used by ``Titan curator
    list-archived`` to help users pass a name to ``Titan curator restore``.
    """
...
def archive_skill(skill_name: str) -> Tuple[bool, str]:
    """Move an agent-created skill directory to ~/.Titan/skills/.archive/.
...
def restore_skill(skill_name: str) -> Tuple[bool, str]:
    """Move an archived skill back to ~/.Titan/skills/. Restores to the flat
...
def _find_skill_dir(skill_name: str) -> Optional[Path]:
    ...
    Handles both flat (~/.Titan/skills/<skill>/SKILL.md) and category-nested
    (~/.Titan/skills/<category>/<skill>/SKILL.md) layouts.
    """
    base = _skills_dir()
    if not base.exists():
        return None
