...
This module provides the glue:
    - ``TitanTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Titan Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses Titan_HOME so each profile gets its own OAuth tokens.
    Layout: ``Titan_HOME/mcp-tokens/``
    """
    try:
        from Titan_constants import get_Titan_home
        base = Path(get_Titan_home())
    except ImportError:
        base = Path(os.environ.get("Titan_HOME", str(Path.home() / ".Titan")))
...
# ---------------------------------------------------------------------------
# TitanTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class TitanTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        Titan_HOME/mcp-tokens/<server_name>.json         -- tokens
        Titan_HOME/mcp-tokens/<server_name>.client.json   -- client info
        Titan_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Titan records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Titan.</p></body></html>"
            ) if code else (
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Titan Agent")
    scope = cfg.get("scope")
