# OpenAI-Compatible API Server for Titan Agent
...
SSE streaming. By exposing this endpoint, titan-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  titan-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.Titan/.env`
2. Run `Titan gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with titan-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns titan-agent as a model) |
...
Authorization: Bearer Titan-api-key-here
...
But titan-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "titan-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured titan-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of titan-agent's core prompt
...
Once implemented, titan-agent works as a drop-in backend for:
