...
There are two ways to add a platform to the Titan gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.Titan/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Titan code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `Titan gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `Titan_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Titan core.
...
And add it to the `Titan-gateway` composite:

```python
"Titan-gateway": {
    "includes": [..., "Titan-your-platform"]
}
```
...
## 12. Status Display (`Titan_cli/status.py`)
...
## 13. Gateway Setup Wizard (`Titan_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ Titan_cli/ toolsets.py \
  --include="*.py" -l | sort -u
