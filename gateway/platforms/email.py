"""
Email platform adapter for the Titan gateway.

Allows users to interact with Titan by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "titan-agent" "version" "{_Titan_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Titan Agent")
        ...
        msg_id = f"<Titan-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
