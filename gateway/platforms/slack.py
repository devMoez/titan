...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        tokens_file = get_Titan_home() / "slack_tokens.json"
        ...
        try:
            ...
            # /model work directly without /Titan prefix). A single regex
            ...
            # manifest (see `Titan slack manifest`). In Socket Mode, Slack
            ...
            from Titan_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/Titan$")

            @self._app.command(_slash_pattern)
            async def handle_Titan_command(ack, command):
            ...
            for _action_id in (
                "Titan_approve_once",
                "Titan_approve_session",
                "Titan_approve_always",
                "Titan_deny",
            ):
            ...
            for _action_id in (
                "Titan_confirm_once",
                "Titan_confirm_always",
                "Titan_confirm_cancel",
            ):
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = f":thread: Titan handoff — *{(name or 'session').strip()[:80]}*"
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Titan session — matching the per-thread behavior channels already
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "Titan_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "Titan_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "Titan_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "Titan_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "Titan_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "Titan_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "Titan_confirm_once": "once",
            "Titan_confirm_always": "always",
            "Titan_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "Titan_approve_once": "once",
            "Titan_approve_session": "session",
            "Titan_approve_always": "always",
            "Titan_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/Titan <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/Titan
        ...
        if slash_name in {"Titan", ""}:
            # Legacy /Titan <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from Titan_cli.commands import slack_subcommand_map
            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/Titan   ``).
        ...
        # "Running /cmd…" ack shown by handle_Titan_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/Titan <question>" must produce public replies so
