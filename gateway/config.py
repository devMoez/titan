...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_Titan_home() / "sessions")
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.Titan/config.yaml (primary user-facing config)
    3. ~/.Titan/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            if not isinstance(streaming_cfg, dict):
                ...
                # ``Titan config set gateway.streaming.*``
