...
from Titan_cli.timeouts import (
...
_loaded_env_paths = load_Titan_dotenv(Titan_home=_Titan_home, project_env=_project_env)
...
class _SafeWriter:
    ...
    When titan-agent runs as a systemd service, Docker container, or headless
...
def _routermint_headers() -> dict:
    ...
    return {
        "User-Agent": f"TitanAgent/{_Titan_VERSION}",
    }
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[titan-agent: tool call arguments were corrupted in this session and "
    ...
    def __init__(
        ...
        Args:
            ...
            load_soul_identity (bool): If True, still use ~/.Titan/SOUL.md as the primary
        ...
        self._cache_ttl = "5m"
        try:
            from Titan_cli.config import load_config as _load_pc_cfg
        ...
        # both live under ~/.Titan/logs/.  Idempotent, so gateway mode
        # (which creates a new AIAgent per message) won't duplicate handlers.
        from Titan_logging import setup_logging, setup_verbose_logging
        setup_logging(Titan_home=_Titan_home)
        ...
        if self.verbose_logging:
            ...
            # on the fact that Titan_logging.setup_logging() does not
            ...
            # handler level inside Titan_logging.py, not here.
            pass
        ...
        if self.api_mode == "anthropic_messages":
            ...
            try:
                from Titan_cli.config import load_config as _load_br_cfg
            ...
            if api_key and base_url:
                ...
                if base_url_host_matches(effective_base, "openrouter.ai"):
                    ...
                    from Titan_cli.models import copilot_default_headers
                    ...
                    if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                        ...
                        if not _fb_resolved:
                            raise RuntimeError(
                                ...
                                f"variable, or switch to a different provider with `Titan model`."
                            )
                    if not getattr(self, "_fallback_activated", False):
                        # No provider configured — reject with a clear message.
                        raise RuntimeError(
                            "No LLM provider configured. Run `Titan model` to "
                            "select a provider, or run `Titan setup` for first-time "
                            "configuration."
                        )
        ...
        os.environ["Titan_SESSION_ID"] = self.session_id
        try:
        ...
        # Session logs go into ~/.Titan/sessions/ alongside gateway sessions
        Titan_home = get_Titan_home()
        self.logs_dir = Titan_home / "sessions"
        ...
        try:
            from Titan_cli.config import load_config as _load_agent_config
                ...
                if _mem_provider_name:
                    ...
                    if self._memory_manager.providers:
                        ...
                        try:
                            ...
                            _init_kwargs["agent_workspace"] = "Titan"
                        except Exception:
                            pass
        ...
        if _config_context_length is None and _custom_providers:
            try:
                from Titan_cli.config import get_custom_provider_context_length
        ...
        if _engine_name != "compressor":
            ...
            if _selected_engine is None:
                try:
                    from Titan_cli.plugins import get_plugin_context_engine
        ...
        if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"by Titan Agent.  Choose a model with at least "
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            self._session_db.create_session(
                session_id=self.session_id,
                source=self.platform or os.environ.get("Titan_SESSION_SOURCE", "cli"),
                model=self.model,
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> None:
        """
        Preload the LM Studio model with at least Titan' minimum context.
        """
        if (self.provider or "").strip().lower() != "lmstudio":
            return
        try:
            from agent.model_metadata import MINIMUM_CONTEXT_LENGTH
            from Titan_cli.models import ensure_lmstudio_model_loaded
    ...
    def switch_model(self, new_model, new_provider, api_key='', base_url='', api_mode=''):
        ...
        turn-scoped).
        """
        from Titan_cli.providers import determine_api_mode
        ...
        if hasattr(self, "context_compressor") and self.context_compressor:
            ...
            try:
                from Titan_cli.config import load_config, get_compatible_custom_providers
                _sm_cfg = load_config()
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``Titan chat -q``. In that mode,
    ...
    def _emit_stream_drop(
        ...
        *,
        ...
        ``Titan logs --level WARNING | grep "Stream drop"`` to inspect.
        """
        ...
        try:
            ...
            if client is None or not aux_model:
                if _aux_cfg_provider and _aux_cfg_provider != "auto":
                    ...
                    msg = (
                        ...
                        "compression will drop middle turns without a summary. "
                        "Run `Titan setup` or set OPENROUTER_API_KEY."
                    )
            ...
            if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
                raise ValueError(
                    ...
                    f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by Titan "
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``Titan_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``Titan_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return float(os.getenv("Titan_API_TIMEOUT", 1800.0))
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``Titan_API_CALL_STALE_TIMEOUT`` env var
          4. 300.0s default
        ...
        env_timeout = os.getenv("Titan_API_CALL_STALE_TIMEOUT")
    ...
    def _provider_model_requires_responses_api(
        ...
        *,
        ...
        if normalized_provider == "copilot":
            try:
                from Titan_cli.models import _should_use_copilot_responses_api
        ...
        def _run_review():
            ...
            try:
                with open(os.devnull, "w", encoding="utf-8") as _devnull, \
                    ...
                    # skill_manage). Those tools require Titan' own dispatch,
                    # which the codex_app_server runtime bypasses entirely
                    # (it runs the turn inside codex's subprocess). So when
                    # the parent is on codex_app_server, downgrade the review
                    # fork to codex_responses — same auth/credentials, but
                    # talks to the OpenAI Responses API directly so Titan
                    ...
                    # scratch (fresh _Titan_now() timestamp, fresh
    ...
    def _build_memory_write_metadata(
        ...
        *,
        ...
        metadata: Dict[str, Any] = {
            ...
            "platform": self.platform or os.environ.get("Titan_SESSION_SOURCE", "cli"),
            "tool_name": "memory",
        }
        if task_id:
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use Titan against xai-oauth.  Lots of
    ...
    def _dump_api_request_debug(
        ...
        *,
        ...
        try:
            ...
            if env_var_enabled("Titan_DUMP_REQUEST_STDOUT"):
    ...
    def _build_system_prompt_parts(self, system_message: str = None) -> Dict[str, str]:
        ...
        AIAgent.  Titan never re-renders parts of this string mid-
        ...
        # Some execution modes (cron) still want Titan_HOME persona while keeping
        ...
        # Pointer to the titan-agent skill + docs for user questions about Titan itself.
        stable_parts.append(titan_agent_HELP_GUIDANCE)
        ...
        # Titan_KANBAN_TASK env var). Normal chat sessions never see
        # this block.
        ...
        if not self.skip_context_files:
            ...
            # mode).  The gateway process runs from the titan-agent install
        ...
        from Titan_time import now as _Titan_now
        now = _Titan_now()
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        one cached block — Titan never rebuilds or reinjects parts of it
    ...
    def _copilot_headers_for_request(self, *, is_vision: bool) -> dict:
        from Titan_cli.copilot_auth import copilot_request_headers
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``Titan auth add xai-oauth``) whose tokens belong to
        ...
        try:
            if self.provider == "openai-codex":
                ...
                from Titan_cli.auth import resolve_xai_oauth_runtime_credentials
    ...
    def _try_refresh_nous_client_credentials(self, *, force: bool = True) -> bool:
        ...
        try:
            from Titan_cli.auth import resolve_nous_runtime_credentials

            creds = resolve_nous_runtime_credentials(
                min_key_ttl_seconds=max(60, int(os.getenv("Titan_NOUS_MIN_KEY_TTL_SECONDS", "1800"))),
                timeout_seconds=float(os.getenv("Titan_NOUS_TIMEOUT_SECONDS", "15")),
                force_mint=force,
            )
        except Exception as exc:
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        try:
            from Titan_cli.copilot_auth import resolve_copilot_token
    ...
    def _interruptible_streaming_api_call(
        ...
        def _call_chat_completions():
            ...
            # wins over the Titan_API_TIMEOUT env default if the user set it.
            _provider_timeout_cfg = get_provider_request_timeout(self.provider, self.model)
            _base_timeout = (
                _provider_timeout_cfg
                if _provider_timeout_cfg is not None
                else float(os.getenv("Titan_API_TIMEOUT", 1800.0))
            )
            # Read timeout: config wins here too.  Otherwise use
            # Titan_STREAM_READ_TIMEOUT (default 120s) for cloud providers.
            if _provider_timeout_cfg is not None:
                ...
                # overrode Titan_STREAM_READ_TIMEOUT.
        ...
        def _call():
            ...
            _max_stream_retries = int(os.getenv("Titan_STREAM_RETRIES", 2))
        ...
        _stream_stale_timeout_base = float(os.getenv("Titan_STREAM_STALE_TIMEOUT", 180.0))
        # Local providers (Ollama, oMLX, llama-cpp) can take 300+ seconds
        # for prefill on large contexts.  Disable the stale detector unless
        # the user explicitly set Titan_STREAM_STALE_TIMEOUT.
    ...
    def _try_activate_fallback(self, reason: "FailoverReason | None" = None) -> bool:
        ...
        try:
            ...
            if not fb_api_key_hint:
                ...
                # _normalize_custom_provider_entry in Titan_cli/config.py).
            ...
            try:
                from Titan_cli.model_normalize import normalize_model_for_provider
        ...
        def _shrink_data_url(url: str) -> Optional[str]:
            ...
            try:
                ...
                tmp = tempfile.NamedTemporaryFile(
                    prefix="Titan_shrink_", suffix=suffix, delete=False,
                )
                try:
                    tmp.write(raw)
                    tmp.close()
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        if _is_qwen:
            _qwen_meta = {
                "sessionId": self.session_id or "Titan",
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        if (
            ...
            try:
                from Titan_cli.models import github_model_reasoning_efforts
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from Titan_cli.models import lmstudio_model_reasoning_options
    ...
    def _compress_context(self, messages: list, system_message: str, *, approx_tokens: int = None, task_id: str = "default", focus_topic: str = None) -> tuple:
        ...
        # compression (not a fresh /new). Plugin engines (e.g. Titan-lcm) use
        # boundary_reason="compression" to preserve DAG lineage across the
        # rollover instead of re-initializing fresh per-session state.
        # See Titan-lcm#68. Built-in ContextCompressor ignores kwargs.
        try:
    ...
    def _invoke_tool(self, function_name: str, function_args: dict, effective_task_id: str,
        ...
        if not pre_tool_block_checked:
            try:
                from Titan_cli.plugins import get_pre_tool_call_block_message
    ...
    def run_conversation(
        ...
        # ``Titan logs --session <id>`` can filter a single conversation.
        from Titan_logging import set_session_context
        ...
        # are reused.  The system prompt is Titan's territory; plugins
        ...
        # all run inside Codex). Default Titan path is bypassed entirely.
        ...
        while (api_call_count < self.max_iterations and self.iteration_budget.remaining > 0) or self._budget_grace_call:
            ...
            # cache prefix.  The system prompt is reserved for Titan internals.
            #
            # Titan invariant: the system prompt is built ONCE per session
            ...
            while retry_count < max_retries:
                ...
                try:
                    ...
                    if env_var_enabled("Titan_DUMP_REQUESTS"):
                    ...
                    if (
                        ...
                        print(f"{self.log_prefix}     • Re-authenticate: Titan login --provider nous")
                    ...
                    if (
                        ...
                        from Titan_constants import display_Titan_home as _dhh_fn
                        _dhh = _dhh_fn()
                        print(f"{self.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Titan-managed OAuth/setup tokens")
                        print(f"{self.log_prefix}     • Check ANTHROPIC_API_KEY in {_dhh}/.env for API keys or legacy token values")
                        print(f"{self.log_prefix}     • For API keys: verify at https://platform.claude.com/settings/keys")
                        print(f"{self.log_prefix}     • For Claude Code: run 'claude /login' to refresh, then retry")
                        print(f"{self.log_prefix}     • Legacy cleanup: Titan config set ANTHROPIC_TOKEN \"\"")
                        print(f"{self.log_prefix}     • Clear stale keys: Titan config set ANTHROPIC_API_KEY \"\"")
                    ...
                    # which Titan' system prompt + tool schemas alone exceed.
                    ...
                    if (
                        ...
                        self._vprint(
                            f"{self.log_prefix}      request at ~8K tokens. Titan' system prompt + tool schemas baseline",
                            force=True,
                        )
                        self._vprint(
                        ...
                        self._vprint(
                            f"{self.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`Titan",
                            force=True,
                        )
                        self._vprint(
                    ...
                    # providers (DeepSeek, Kimi, MiMo, Titan).  A 429 can
                        ...
                        if classified.is_auth or classified.reason == FailoverReason.billing:
                            if _provider in {"openai-codex", "xai-oauth"} and status_code == 401:
                                if _provider == "openai-codex":
                                    ...
                                    self._vprint(f"{self.log_prefix}      2. Then run `Titan auth` to re-authenticate.", force=True)
                                else:
                                    self._vprint(f"{self.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                    self._vprint(f"{self.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok Subscription) from `Titan model`.", force=True)
                            else:
                                self._vprint(f"{self.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                                self._vprint(f"{self.log_prefix}      • Is the key valid? Run: Titan setup", force=True)
        ...
        if final_response is None and (
            ...
            _kanban_task = os.environ.get("Titan_KANBAN_TASK")
            if _kanban_task:
                try:
                    handle_function_call(
                        "kanban_block",
    ...
    def _run_codex_app_server_turn(
        ...
        *,
        ...
        app-server` subprocess and projects its events back into Titan'
        ...
        if not hasattr(self, "_codex_session") or self._codex_session is None:
            ...
            # Approval callback: defer to Titan' standard prompt flow if a
