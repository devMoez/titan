...
from Titan_constants import get_config_path, get_skills_dir
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``Titan_PLATFORM`` or
            ``Titan_SESSION_PLATFORM`` env vars.  Falls back to the
...
# which becomes the dominant cost of ``Titan`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.Titan/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``Titan`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against Titan_HOME, not cwd
        if not p.is_absolute():
            p = (Titan_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.Titan/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    Titan = metadata.get("Titan") or {}
    if not isinstance(Titan, dict):
        Titan = {}
    return {
        "fallback_for_toolsets": Titan.get("fallback_for_toolsets", []),
        "requires_toolsets": Titan.get("requires_toolsets", []),
        "fallback_for_tools": Titan.get("fallback_for_tools", []),
        "requires_tools": Titan.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    Titan = metadata.get("Titan")
    if not isinstance(Titan, dict):
        return []
    raw = Titan.get("config")
    if not raw:
        return []
