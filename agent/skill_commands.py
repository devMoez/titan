...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``Titan_PLATFORM`` env var and
    ``Titan_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = os.getenv("Titan_PLATFORM")
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.Titan.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_Titan_home()}/config.yaml):"]
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.Titan/skills/ and return a mapping of /command -> skill info.
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.Titan/skills/`` and any ``skills.external_dirs`` so the
