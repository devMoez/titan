...
as ``kind: backend``) or ``~/.Titan/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
def _images_cache_dir() -> Path:
    """Return ``$Titan_HOME/cache/images/``, creating parents as needed."""
    from Titan_constants import get_Titan_home

    path = get_Titan_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$Titan_HOME/cache/images/``.
