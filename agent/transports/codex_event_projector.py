"""Projects codex app-server events into Titan' messages list.

The translator that lets Titan' memory/skill review keep working under the
...
Titan' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape Titan
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way Titan' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but Titan stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way Titan already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — Titan' messages store text only.
        text_parts: list[str] = []
