...
Owns one Codex thread per Titan session. Drives `turn/start`, consumes
...
# Titan' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_Titan_TO_CODEX_PERMISSION_PROFILE = {
...
class CodexAppServerSession:
    """One Codex thread per Titan session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _Titan_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("Titan_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="Titan",
            client_title="Titan Agent",
            client_version=_get_Titan_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Titan' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Titan' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own Titan-tools callback we
            # auto-accept — the user already approved Titan' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Titan approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Titan returns 'once', 'session', 'always', or 'deny'.
...
def _get_Titan_version() -> str:
    """Best-effort Titan version string for codex's userAgent line."""
    try:
        from importlib.metadata import version

        return version("titan-agent")
