...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - Titan_home (str): The active Titan_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.Titan``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "Titan").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'Titan memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], Titan_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'Titan memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``Titan_home`` is the active Titan_HOME directory path.
