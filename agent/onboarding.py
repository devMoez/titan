...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Titan starts and finds ``~/.openclaw/``.

    Points users at ``Titan claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``Titan claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Titan, run "
        "`Titan claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `Titan claw cleanup` (renames it to ~/.openclaw.pre-migration — "
