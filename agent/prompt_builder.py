...
from Titan_constants import get_Titan_home, get_skills_dir, is_wsl
...
_Titan_MD_NAMES = (".Titan.md", "Titan.md")


def _find_Titan_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.Titan.md`` or ``Titan.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Titan Agent, an intelligent AI assistant created by Nous Research. "
...
titan_agent_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using Titan Agent "
    "itself, load the `titan-agent` skill with skill_view(name='titan-agent') "
    "before answering. Docs: https://titan-agent.nousresearch.com/docs"
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.Titan/kanban.db`. Your task id is in "
    "`$Titan_KANBAN_TASK`; your workspace is `$Titan_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $Titan_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `Titan kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Titan WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Titan itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Titan runs on.
    """
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Titan itself is running. The host OS, home, and cwd "
                f"of the Titan process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Titan "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_Titan_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.Titan/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("Titan_PLATFORM")
        or get_session_env("Titan_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Titan Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `titan-agent` skill "
            "first. It has the actual commands (e.g. `Titan config set …`, `Titan tools`, "
            "`Titan setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: Titan setup, Titan setup tools, Titan setup terminal, Titan status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from Titan_HOME and return its content, or None.
    ...
    """
    try:
        from Titan_cli.config import ensure_Titan_home
        ensure_Titan_home()
    except Exception as e:
        logger.debug("Could not ensure Titan_HOME before loading SOUL.md: %s", e)

    soul_path = get_Titan_home() / "SOUL.md"
...
def _load_Titan_md(cwd_path: Path) -> str:
    """.Titan.md / Titan.md — walk to git root."""
    Titan_md_path = _find_Titan_md(cwd_path)
    if not Titan_md_path:
        return ""
    try:
        content = Titan_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = Titan_md_path.name
        try:
            rel = str(Titan_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".Titan.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", Titan_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .Titan.md / Titan.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from Titan_HOME is independent and always included when present.
    ...
    # SOUL.md from Titan_HOME only — skip when already loaded as identity
    if not skip_soul:
