...
Every Titan request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Titan Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=titan-agent",
        "client=Titan-client-v<__version__>",
    ]

The version is sourced live from ``Titan_cli.__version__`` so it auto-aligns
...
``Titan_cli.__version__`` is the canonical source of truth.
"""
...
def _Titan_version() -> str:
    """Return the current Titan release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``Titan_cli`` cannot be imported (should
...
def Titan_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=Titan-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=Titan-client-v{_Titan_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=titan-agent", Titan_client_tag()]
