...
Plugins built on Titan Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Titan
...
:class:`~Titan_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Titan' main config
...
handles every provider, fallback chain, and per-task override Titan
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`Titan_cli.plugins.PluginContext`
