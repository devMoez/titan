"""CLI entry point for the titan-agent ACP adapter.

Loads environment variables from ``~/.Titan/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from Titan_HOME (default ``~/.Titan``)."""
    from Titan_cli.env_loader import load_Titan_dotenv

    Titan_home = get_Titan_home()
    loaded = load_Titan_dotenv(Titan_home=Titan_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", Titan_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="Titan-acp",
        description="Run Titan Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Titan version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _run_check() -> None:
    ...
    from acp_adapter.server import TitanACPAgent  # noqa: F401

    print("Titan ACP check OK")


def _run_setup() -> None:
    from Titan_cli.main import main as Titan_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "Titan", "model"]
        Titan_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting titan-agent ACP adapter")
    ...
    import acp
    from .server import TitanACPAgent
