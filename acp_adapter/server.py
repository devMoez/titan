"""ACP agent server — exposes Titan Agent via the Agent Client Protocol."""
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Titan running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Titan cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Titan/OpenAI-compatible user content payload."""
...
class TitanACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Titan AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Titan version",
        },
    )
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from Titan_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from Titan_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Titan estimates ``used`` from the same
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["Titan-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="titan-agent", version=Titan_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Titan' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Titan setup/model selection out-of-band.
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications; merely restoring server-side state makes Titan remember
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Titan on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Titan' local kawaii waiting/status
        ...
        # not the executor's. Also set Titan_INTERACTIVE so approval.py
        # takes the CLI-interactive path (which calls the registered
        # callback via prompt_dangerous_approval) instead of the
        # non-interactive auto-approve branch (GHSA-96vc-wcxf-jjff).
        # ACP's conn.request_permission maps cleanly to the interactive
        # callback shape — not the gateway-queue Titan_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind Titan_SESSION_KEY for this session so per-session caches
            ...
            previous_interactive = os.environ.get("Titan_INTERACTIVE")
            ...
            try:
                ...
                # Restore Titan_INTERACTIVE.
                if previous_interactive is None:
                    os.environ.pop("Titan_INTERACTIVE", None)
                else:
                    os.environ["Titan_INTERACTIVE"] = previous_interactive
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            ...
            # stomp on each other's ContextVar writes (Titan_SESSION_KEY in
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Titan Agent v{Titan_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Titan has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
