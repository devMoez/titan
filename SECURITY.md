# Titan Agent Security Policy

This document describes Titan Agent's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/NousResearch/titan-agent/security/advisories/new)
or **security@nousresearch.com**. Do not open public issues for
security vulnerabilities. **Titan Agent does not operate a bug
bounty program.**
...
- Environment details (`Titan version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Titan Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Titan Agent's documentation
...
Titan Agent supports two OS-level isolation postures. They address
...
Titan Agent supports this in two ways:

- **Titan Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Titan Agent's in-process heuristics
...
Titan Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Titan Agent
itself. Bugs in Titan Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Titan Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Titan Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Titan Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  Titan-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Titan Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Titan Agent. Bugs in Titan
...
- Titan Agent includes supply-chain guards for MCP server
