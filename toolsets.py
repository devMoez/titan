...
_Titan_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (Titan_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `Titan tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (Titan_KANBAN_TASK env "
    ...
    # ==========================================================================
    # Full Titan toolsets (CLI + messaging platforms)
    #
    ...
    "Titan-cli": {
        ...
        "tools": _Titan_CORE_TOOLS,
        "includes": []
    },

    "Titan-cron": {
        # Mirrors Titan-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `Titan tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as Titan-cli; gated by `Titan tools`",
        "tools": _Titan_CORE_TOOLS,
        "includes": []
    },
    ...
    "Titan-email": {
        "description": "Email bot toolset - interact with Titan via email (IMAP/SMTP)",
        "tools": _Titan_CORE_TOOLS,
        "includes": []
    },
    ...
    "Titan-sms": {
        "description": "SMS bot toolset - interact with Titan via SMS (Twilio)",
        "tools": _Titan_CORE_TOOLS,
        "includes": []
    },
    ...
    "Titan-gateway": {
        ...
        "tools": [],
        "includes": ["Titan-telegram", "Titan-discord", "Titan-whatsapp", "Titan-slack", "Titan-signal", "Titan-bluebubbles", "Titan-homeassistant", "Titan-email", "Titan-sms", "Titan-mattermost", "Titan-matrix", "Titan-dingtalk", "Titan-feishu", "Titan-wecom", "Titan-wecom-callback", "Titan-weixin", "Titan-qqbot", "Titan-webhook", "Titan-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (Titan-<name>).
        # Gives them _Titan_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("Titan-"):
            platform_name = name[len("Titan-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_Titan_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
